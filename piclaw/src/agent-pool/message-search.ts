...
function applyDetailsLimit(row: MessageRow, maxChars: number | undefined): MessageResultRow {
  ...
  if (maxChars <= 0) {
    return {
      ...row,
      content: "",
      content_truncated: fullLength > 0,
  ...
  return {
    ...row,
    content: `${row.content.slice(0, sliceLength)}…`,
...
function fetchByRowId(db: ReturnType<typeof getDb>, rowId: number, chatJid: string | null): MessageRow | null {
  if (chatJid) {
    const row = db
      .prepare(
        "SELECT rowid, chat_jid, sender, sender_name, content, timestamp, is_bot_message FROM messages WHERE rowid = ? AND chat_jid = ?"
      )
      .get(rowId, chatJid) as MessageRow | undefined;
    return row ?? null;
  }
  const row = db
    .prepare(
      "SELECT rowid, chat_jid, sender, sender_name, content, timestamp, is_bot_message FROM messages WHERE rowid = ?"
    )
  ...
  if (trimmed.startsWith("#")) {
    ...
    if (chatJid) {
      return db
        ...
        .all(chatJid, pattern, limit, offset) as MessageRow[];
    }
    return db
      .prepare(
        "SELECT rowid, chat_jid, sender, sender_name, content, timestamp, is_bot_message FROM messages WHERE content LIKE ? COLLATE NOCASE ORDER BY rowid DESC LIMIT ? OFFSET ?"
      )
      .all(pattern, limit, offset) as MessageRow[];
  }
  ...
  try {
    if (chatJid) {
      return db
        ...
        .all(chatJid, trimmed, limit, offset) as MessageRow[];
    }
    return db
      .prepare(
        `SELECT messages.rowid, messages.chat_jid, messages.sender, messages.sender_name, messages.content, messages.timestamp, messages.is_bot_message
      ...
      .all(trimmed, limit, offset) as MessageRow[];
  } catch {
...
export function createMessageSearchTool(defaultChatJid: string): ToolDefinition<typeof SearchMessagesSchema, SearchMessagesDetails> {
  return {
    ...
    execute: async (
      ...
      _signal?: AbortSignal,
      _onUpdate?: AgentToolUpdateCallback<SearchMessagesDetails>,
      ...
      const chatJid = normalizeChatJid(params.chat_jid, defaultChatJid);
      ...
      if (!query) {
        return {
          content: [{ type: "text", text: "Provide a query or row_id." }],
          details: { count: 0, results: [] },
        };
      }
      ...
      const detailRows = detailsMaxChars === undefined
        ? rows
        : rows.map((row) => applyDetailsLimit(row, detailsMaxChars));
