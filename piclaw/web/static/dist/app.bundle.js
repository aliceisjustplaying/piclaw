...
`)){y_(!1),A_([]);return}let b=Q.toLowerCase().split(" ")[0];if(b.length<1){y_(!1),A_([]);return}let x=s9.filter((d)=>d.name.startsWith(b)||d.name.replace(/-/g,"").startsWith(b.replace(/-/g,"")));if(x.length>0&&!(x.length===1&&x[0].name===b))W0(!1),I_([]),A_(x),h_(0),y_(!0);else y_(!1),A_([])},F$=(Q)=>{let b=G_,x=b.indexOf(" "),d=x>=0?b.slice(x):"",X_=Q.name+d;C_(X_),y_(!1),A_([]),requestAnimationFrame(()=>{let w_=u_.current;if(!w_)return;let P_=X_.length;w_.selectionStart=P_,w_.selectionEnd=P_,w_.focus()})},u$=(Q)=>{if(a1(Q)==null){W0(!1),I_([]);return}let b=b3(d_,Q,{currentChatJid:Q_});if(b.length>0&&!(b.length===1&&t1(b[0].agent_name).trim().toLowerCase()===String(Q||"").trim().toLowerCase()))y_(!1),A_([]),I_(b),f_(0),W0(!0);else W0(!1),I_([])},m$=(Q)=>{let b=t1(Q?.agent_name);if(!b)return;C_(b),W0(!1),I_([]),requestAnimationFrame(()=>{let x=u_.current;if(!x)return;let d=b.length;x.selectionStart=d,x.selectionEnd=d,x.focus()})},M$=(Q)=>{if(j)D0(Q);else C_(Q),z2(Q),u$(Q);requestAnimationFrame(()=>Q0())},i_=(Q)=>{let b=j?v_:G_,x=b&&!b.endsWith(`
`)?`
`:"",d=`${b}${x}${Q}`.trimStart();M$(d)},I0=(Q)=>{let b=Q?.command?.model_label;if(b)return b;let x=Q?.command?.message;if(typeof x==="string"){let d=x.match(/•\s+([^\n]+?)\s+\(current\)/);if(d?.[1])return d[1].trim()}return null},R0=async(Q)=>{if(j||G0)return;e0(!0);try{let b=await V2("default",Q,null,[],null,Q_),x=I0(b);p0({model:x??u??null,thinking_level:b?.command?.thinking_level,supports_thinking:b?.command?.supports_thinking});try{let d=await E2(Q_);if(d)p0(d)}catch{}return _?.(),!0}catch(b){return console.error("Failed to switch model:",b),alert("Failed to switch model: "+b.message),!1}finally{e0(!1)}},d$=async()=>{await R0("/cycle-model")},s$=async(Q)=>{if(!Q||G0)return;if(await R0(`/model ${Q}`))S_(!1)},o$=(Q)=>{Q.preventDefault(),Q.stopPropagation(),S_((b)=>!b)},J$=(Q)=>{if(Q==="queue"||Q==="steer"||Q==="auto")return Q;return O_?"queue":null},y0=async(Q,b,x={})=>{let{includeMedia:d=!0,includeFileRefs:X_=!0,includeMessageRefs:w_=!0,clearAfterSubmit:P_=!0,recordHistory:W_=!0}=x||{},Z$=typeof Q==="string"?Q:Q&&typeof Q?.target?.value==="string"?Q.target.value:G_,g_=typeof Z$==="string"?Z$:"";if(!g_.trim()&&(d?Y_.length===0:!0)&&(X_?q.length===0:!0)&&(w_?Y.length===0:!0))return;y_(!1),A_([]),W0(!1),I_([]),F_(null);let h0=d?[...Y_]:[],E$=X_?[...q]:[],c$=w_?[...Y]:[],k$=g_.trim();if(W_&&k$){let b$=_$.current,r_=T_(b$.filter((S0)=>S0!==k$));if(r_.push(k$),r_.length>200)r_.splice(0,r_.length-200);_$.current=r_,T$(r_),X$.current=-1,B0.current=""}let t$=()=>{if(d)$_([...h0]);if(X_)U_?.(E$);if(w_)J_?.(c$);C_(k$),requestAnimationFrame(()=>Q0())};if(P_)C_(""),$_([]),B?.(),A?.();(async()=>{try{if(await r?.({content:k$,submitMode:b,fileRefs:E$,messageRefs:c$,mediaFiles:h0})){_?.();return}let r_=[];for(let v0 of h0){let O$=await u1(v0);r_.push(O$.id)}let S0=E$.length?`Files:
${E$.map((v0)=>`- ${v0}`).join(`
...
`),i0=await V2("default",o0,null,r_,J$(b),Q_);if(N_?.(i0),i0?.command){p0({model:i0.command.model_label??u??null,thinking_level:i0.command.thinking_level,supports_thinking:i0.command.supports_thinking});try{let v0=await E2(Q_);if(v0)p0(v0)}catch{}}_?.()}catch(b$){if(P_)t$();let r_=b$?.message||"Failed to send message.";F_(r_),R_?.(r_),console.error("Failed to post:",b$)}})()},V0=(Q)=>{n_?.(Q)},U0=(Q)=>{if(Q.isComposing)return;if(j&&Q.key==="Escape"){Q.preventDefault(),D0(""),G?.();return}if(A0&&b_.length>0){let b=u_.current?.value??(j?v_:G_);if(!String(b||"").match(/^@([a-zA-Z0-9_-]*)$/))W0(!1),I_([]);else{if(Q.key==="ArrowDown"){Q.preventDefault(),f_((x)=>(x+1)%b_.length);return}if(Q.key==="ArrowUp"){Q.preventDefault(),f_((x)=>(x-1+b_.length)%b_.length);return}if(Q.key==="Tab"||Q.key==="Enter"){Q.preventDefault(),m$(b_[z0]);return}if(Q.key==="Escape"){Q.preventDefault(),W0(!1),I_([]);return}}}if(j0&&E_.length>0){let b=u_.current?.value??(j?v_:G_);if(!String(b||"").startsWith("/"))y_(!1),A_([]);else{if(Q.key==="ArrowDown"){Q.preventDefault(),h_((x)=>(x+1)%E_.length);return}if(Q.key==="ArrowUp"){Q.preventDefault(),h_((x)=>(x-1+E_.length)%E_.length);return}if(Q.key==="Tab"){Q.preventDefault(),F$(E_[$0]);return}if(Q.key==="Enter"&&!Q.shiftKey){if(!(u_.current?.value??(j?v_:G_)).includes(" ")){Q.preventDefault();let X_=E_[$0];y_(!1),A_([]),y0(X_.name);return}}if(Q.key==="Escape"){Q.preventDefault(),y_(!1),A_([]);return}}}if(!j&&(Q.key==="ArrowUp"||Q.key==="ArrowDown")&&!Q.metaKey&&!Q.ctrlKey&&!Q.altKey&&!Q.shiftKey){let b=u_.current;if(!b)return;let x=b.value||"",d=b.selectionStart===0&&b.selectionEnd===0,X_=b.selectionStart===x.length&&b.selectionEnd===x.length;if(Q.key==="ArrowUp"&&d||Q.key==="ArrowDown"&&X_){let w_=_$.current;if(!w_.length)return;Q.preventDefault();let P_=X$.current;if(Q.key==="ArrowUp"){if(P_===-1)B0.current=x,P_=w_.length-1;else if(P_>0)P_-=1;X$.current=P_,M$(w_[P_]||"")}else{if(P_===-1)return;if(P_<w_.length-1)P_+=1,X$.current=P_,M$(w_[P_]||"");else X$.current=-1,M$(B0.current||""),B0.current=""}requestAnimationFrame(()=>{let W_=u_.current;if(!W_)return;let Z$=W_.value.length;W_.selectionStart=Z$,W_.selectionEnd=Z$});return}}if(Q.key==="Enter"&&!Q.shiftKey&&(Q.ctrlKey||Q.metaKey)){Q.preventDefault();let b=u_.current?.value??(j?v_:G_);if(j){if(b.trim())N?.(b.trim(),Z)}else y0(b,"steer");return}if(Q.key==="Enter"&&!Q.shiftKey){Q.preventDefault();let b=u_.current?.value??(j?v_:G_);if(j){if(b.trim())N?.(b.trim(),Z)}else y0(b)}},s0=(Q)=>{let b=Array.from(Q||[]).filter((x)=>x&&x.type&&x.type.startsWith("image/"));if(!b.length)return;$_((x)=>[...x,...b]),F_(null)},C$=(Q)=>{s0(Q.target.files),Q.target.value=""},r$=(Q)=>{if(j)return;Q.preventDefault(),Q.stopPropagation(),k_.current+=1,V_(!0)},H$=(Q)=>{if(j)return;if(Q.preventDefault(),Q.stopPropagation(),k_.current=Math.max(0,k_.current-1),k_.current===0)V_(!1)},$$=(Q)=>{if(j)return;if(Q.preventDefault(),Q.stopPropagation(),Q.dataTransfer)Q.dataTransfer.dropEffect="copy";V_(!0)},g$=(Q)=>{if(j)return;Q.preventDefault(),Q.stopPropagation(),k_.current=0,V_(!1),s0(Q.dataTransfer?.files||[])},j$=(Q)=>{if(j)return;let b=Q.clipboardData?.items;if(!b||!b.length)return;let x=[];for(let d of b){if(d.kind!=="file")continue;let X_=d.getAsFile?.();if(X_)x.push(X_)}if(x.length>0)Q.preventDefault(),s0(x)},w0=(Q)=>{$_((b)=>b.filter((x,d)=>d!==Q))},D$=()=>{F_(null),$_([]),B?.(),A?.()},a$=()=>{if(!navigator.geolocation){alert("Geolocation is not available in this browser.");return}navigator.geolocation.getCurrentPosition((Q)=>{let{latitude:b,longitude:x,accuracy:d}=Q.coords,X_=`${b.toFixed(5)}, ${x.toFixed(5)}`,w_=Number.isFinite(d)?` ±${Math.round(d)}m`:"",P_=`https://maps.google.com/?q=${b},${x}`,W_=`Location: ${X_}${w_} ${P_}`;i_(W_)},(Q)=>{let b=Q?.message||"Unable to retrieve location.";alert(`Location error: ${b}`)},{enableHighAccuracy:!0,timeout:1e4,maximumAge:0})};v(()=>{if(!K0)return;P0(!0),E2(Q_).then((Q)=>{let b=Array.isArray(Q?.models)?Q.models.filter((x)=>typeof x==="string"&&x.trim().length>0):[];b.sort((x,d)=>x.localeCompare(d,void 0,{sensitivity:"base"})),s_(b),p0(Q)}).catch((Q)=>{console.warn("Failed to load model list:",Q),s_([])}).finally(()=>{P0(!1)})},[K0,u]),v(()=>{if(j)S_(!1),y_(!1),A_([]),W0(!1),I_([])},[j]),v(()=>{if(!K0)return;let Q=(b)=>{let x=S.current,d=s.current,X_=b.target;if(x&&x.contains(X_))return;if(d&&d.contains(X_))return;S_(!1)};return document.addEventListener("pointerdown",Q),()=>document.removeEventListener("pointerdown",Q)},[K0]),v(()=>{let Q=()=>{let w_=K_.current?.clientWidth||0;E0((P_)=>P_===w_?P_:w_)};Q();let b=K_.current,x=0,d=()=>{if(x)cancelAnimationFrame(x);x=requestAnimationFrame(()=>{x=0,Q()})},X_=null;if(b&&typeof ResizeObserver<"u")X_=new ResizeObserver(()=>d()),X_.observe(b);if(typeof window<"u")window.addEventListener("resize",d);return()=>{if(x)cancelAnimationFrame(x);if(X_?.disconnect?.(),typeof window<"u")window.removeEventListener("resize",d)}},[j,u,d0.length,F?.percent]);let p$=(Q)=>{let b=Q.target.value;F_(null),Q0(Q.target),M$(b)};return v(()=>{requestAnimationFrame(()=>Q0())},[G_,v_,j]),v(()=>{if(j)return;u$(G_)},[d_,Q_,G_,j]),O`
        <div class="compose-box">
            ...
            >
                ${x_!=="connected"&&O`
                    <span class="compose-connection-status connection-status ${x_}" title=${L$}>
                        ${Z0}
                    </span>
                `}
...
`):E_,h_=Number.isFinite(V_)?V_:A_.reduce((b_,I_)=>b_+u(I_),0),j0=$0?$0.split(`
`).reduce((b_,I_)=>b_+u(I_),0):0,y_=Math.max(h_-j0,0);return{text:$0,omitted:y_,totalLines:h_,visibleLines:j0}},f=Y(j),E=Y(Z),F=Y($),I=Boolean(f.text)||f.totalLines>0,C=Boolean(E.text)||E.totalLines>0,i=Boolean(F.fullText?.trim()||F.text?.trim());if(!_&&!i&&!I&&!C&&!N&&!z)return null;let[l,e]=T(new Set),[n,z_]=T(()=>Date.now()),c=($_)=>e((M_)=>{let V_=new Set(M_),E_=!V_.has($_);if(E_)V_.add($_);else V_.delete($_);if(typeof q==="function")q($_,E_);return V_});v(()=>{e(new Set)},[K]);let q_=P2(_);v(()=>{if(!q_)return;z_(Date.now());let $_=setInterval(()=>z_(Date.now()),1000);return()=>clearInterval($_)},[q_,_?.started_at,_?.startedAt]);let n_=_?.turn_id||K,__=W6(n_),r=G?"turn-dot turn-dot-queued":"turn-dot",N_=($_)=>$_,j_=Boolean(_?.last_activity||_?.lastActivity),O_=($_)=>$_==="warning"?"#f59e0b":$_==="error"?"var(--danger-color)":$_==="success"?"var(--success-color)":__,d_=z?.kind||"info",Q_=O_(d_),x_=O_(_?.kind||(q_?"warning":"info")),U_="",J_=_?.title,R_=_?.status;if(_?.type==="plan")U_=J_?`Planning: ${J_}`:"Planning...";else if(_?.type==="tool_call")U_=J_?`Running: ${J_}`:"Running tool...";else if(_?.type==="tool_status")U_=J_?`${J_}: ${R_||"Working..."}`:R_||"Working...";else if(_?.type==="error")U_=J_||"Agent error";else U_=J_||R_||"Working...";if(j_)U_="Last activity just now";let G_=({panelTitle:$_,text:M_,fullText:V_,totalLines:E_,maxLines:A_,titleClass:$0,panelKey:h_})=>{let j0=l.has(h_),y_=V_||M_||"",b_=h_==="thought"||h_==="draft"?A(y_):y_,I_=typeof A_==="number",z0=j0&&I_,f_=I_?R(b_,A_,E_):{text:b_||"",omitted:0,totalLines:Number.isFinite(E_)?E_:0};if(!b_&&!(Number.isFinite(f_.totalLines)&&f_.totalLines>0))return null;let A0=`agent-thinking-body${I_?" agent-thinking-body-collapsible":""}`,W0=I_?`--agent-thinking-collapsed-lines: ${A_};`:"";return O`
            <div
            ...
            >
                ...
                <div
                    class=${A0}
                    style=${W0}
                    dangerouslySetInnerHTML=${{__html:N1(b_)}}
                />
                ${!j0&&f_.omitted>0&&O`
        ...
        `},C_=N?.tool_call?.title,v_=C_?`Awaiting approval: ${C_}`:"Awaiting approval",D0=q_?V6(_,n):null,Y_=($_,M_,V_=null)=>{let E_=G6($_);return O`
            <div
...
//# debugId=9D72B221D23E40D964756E2164756E21
