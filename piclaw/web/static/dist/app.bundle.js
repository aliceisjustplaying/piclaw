...
`);N=X.pop()||"";for(let O of X){let q=z4(O);if(q)$(q.event,q.data)}}N+=Z.decode();let K=z4(N);if(K)$(K.event,K.data)}async function _$(_=10,$=null,j=null){let Z=`/timeline?limit=${_}`;if($)Z+=`&before=${$}`;if(j)Z+=`&chat_jid=${encodeURIComponent(j)}`;return f_(Z)}async function V3(_,$=50,j=0,Z=null){let N=Z?`&chat_jid=${encodeURIComponent(Z)}`:"";return f_(`/hashtag/${encodeURIComponent(_)}?limit=${$}&offset=${j}${N}`)}async function q3(_,$=50,j=0,Z=null,N="current",K=null){let z=Z?`&chat_jid=${encodeURIComponent(Z)}`:"",G=N?`&scope=${encodeURIComponent(N)}`:"",X=K?`&root_chat_jid=${encodeURIComponent(K)}`:"";return f_(`/search?q=${encodeURIComponent(_)}&limit=${$}&offset=${j}${z}${G}${X}`)}async function U3(_,$=null){let j=$?`?chat_jid=${encodeURIComponent($)}`:"";return f_(`/thread/${_}${j}`)}async function a6(_,$=[],j=null){let Z=j?`?chat_jid=${encodeURIComponent(j)}`:"";return f_(`/post${Z}`,{method:"POST",body:JSON.stringify({content:_,media_ids:$})})}async function t6(_,$,j=[],Z=null){let N=Z?`?chat_jid=${encodeURIComponent(Z)}`:"";return f_(`/reply${N}`,{method:"POST",body:JSON.stringify({thread_id:_,content:$,media_ids:j})})}async function B3(_,$=!1,j=null){let Z=j?`&chat_jid=${encodeURIComponent(j)}`:"",N=`/post/${_}?cascade=${$?"true":"false"}${Z}`;return f_(N,{method:"DELETE"})}async function g2(_,$,j=null,Z=[],N=null,K=null){let z=K?`?chat_jid=${encodeURIComponent(K)}`:"";return f_(`/agent/${_}/message${z}`,{method:"POST",body:JSON.stringify({content:$,thread_id:j,media_ids:Z,mode:N})})}async function Q3(){return f_("/agent/active-chats")}async function e6(_=null){let $=_?`?root_chat_jid=${encodeURIComponent(_)}`:"";return f_(`/agent/branches${$}`)}async function w$(_,$={}){return f_("/agent/branch-fork",{method:"POST",body:JSON.stringify({source_chat_jid:_,...$?.agentName?{agent_name:$.agentName}:{},...$?.displayName?{display_name:$.displayName}:{}})})}async function _5(_,$={}){return f_("/agent/branch-rename",{method:"POST",body:JSON.stringify({chat_jid:_,...$&&Object.prototype.hasOwnProperty.call($,"agentName")?{agent_name:$.agentName}:{},...$&&Object.prototype.hasOwnProperty.call($,"displayName")?{display_name:$.displayName}:{}})})}async function $5(_){return f_("/agent/branch-prune",{method:"POST",body:JSON.stringify({chat_jid:_})})}async function j5(_,$,j,Z="auto",N={}){let K={source_chat_jid:_,content:j,mode:Z,...N?.sourceAgentName?{source_agent_name:N.sourceAgentName}:{},...N?.targetBy==="agent_name"?{target_agent_name:$}:{target_chat_jid:$}};return f_("/agent/peer-message",{method:"POST",body:JSON.stringify(K)})}async function F3(){return f_("/agents")}async function H3(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return f_(`/agent/status${$}`)}async function Z5(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return f_(`/agent/context${$}`)}async function N5(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return f_(`/agent/queue-state${$}`)}async function K5(_,$=null){let j=await fetch("/agent/queue-remove",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row_id:_,chat_jid:$||void 0})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Failed to remove queued item"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function z5(_,$=null){let j=await fetch("/agent/queue-steer",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({row_id:_,chat_jid:$||void 0})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Failed to steer queued item"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function $$(_=null){let $=_?`?chat_jid=${encodeURIComponent(_)}`:"";return f_(`/agent/models${$}`)}async function J3(_){let $=new FormData;$.append("file",_);let j=await fetch("/media/upload",{method:"POST",body:$});if(!j.ok){let Z=await j.json().catch(()=>({error:"Upload failed"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function A$(_,$,j=null){let Z=await fetch("/agent/respond",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({request_id:_,outcome:$,chat_jid:j||void 0})});if(!Z.ok){let N=await Z.json().catch(()=>({error:"Failed to respond"}));throw Error(N.error||`HTTP ${Z.status}`)}return Z.json()}async function y3(_){let $=await fetch("/agent/card-action",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(_)});if(!$.ok){let j=await $.json().catch(()=>({error:"Adaptive Card action failed"}));throw Error(j.error||`HTTP ${$.status}`)}return $.json()}async function Y5(_,$={}){let j=await fetch("/agent/side-prompt/stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:_,system_prompt:$.systemPrompt||void 0,chat_jid:$.chatJid||void 0}),signal:$.signal});if(!j.ok){let K=await j.json().catch(()=>({error:"Side prompt failed"}));throw Error(K.error||`HTTP ${j.status}`)}let Z=null,N=null;if(await r6(j,(K,z)=>{if($.onEvent?.(K,z),K==="side_prompt_thinking_delta")$.onThinkingDelta?.(z?.delta||"");else if(K==="side_prompt_text_delta")$.onTextDelta?.(z?.delta||"");else if(K==="side_prompt_done")Z=z;else if(K==="side_prompt_error")N=z}),N){let K=Error(N?.error||"Side prompt failed");throw K.payload=N,K}return Z}async function D3(_,$){let j=await fetch("/agent/whitelist",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pattern:_,description:$})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Failed to add to whitelist"}));throw Error(Z.error||`HTTP ${j.status}`)}return j.json()}async function E3(_,$="thought"){let j=`/agent/thought?turn_id=${encodeURIComponent(_)}&panel=${encodeURIComponent($)}`;return f_(j)}async function k3(_,$,j){return f_("/agent/thought/visibility",{method:"POST",body:JSON.stringify({turn_id:_,panel:$,expanded:Boolean(j)})})}function Z2(_){return`/media/${_}`}function b3(_){return`/media/${_}/thumbnail`}async function c2(_){let $=await fetch(`/media/${_}/info`);if(!$.ok)throw Error("Failed to get media info");return $.json()}async function w3(_){let $=await fetch(`/media/${_}`);if(!$.ok)throw Error("Failed to load media text");return $.text()}async function A3(_){let $=await fetch(`/media/${_}`);if(!$.ok)throw Error("Failed to load media blob");return $.blob()}async function j$(_="",$=2,j=!1){let Z=`/workspace/tree?path=${encodeURIComponent(_)}&depth=${$}&show_hidden=${j?"1":"0"}`;return f_(Z)}async function G5(_=""){let $=`/workspace/branch?path=${encodeURIComponent(_||"")}`;return f_($)}async function P3(_,$=20000,j=null){let Z=j?`&mode=${encodeURIComponent(j)}`:"",N=`/workspace/file?path=${encodeURIComponent(_)}&max=${$}${Z}`;return f_(N)}async function O5(_,$){return f_("/workspace/file",{method:"PUT",body:JSON.stringify({path:_,content:$})})}async function M3(_){return f_("/workspace/attach",{method:"POST",body:JSON.stringify({path:_})})}async function P$(_,$="",j={}){let Z=new FormData;Z.append("file",_);let N=new URLSearchParams;if($)N.set("path",$);if(j.overwrite)N.set("overwrite","1");let K=N.toString(),z=K?`/workspace/upload?${K}`:"/workspace/upload",G=await fetch(""+z,{method:"POST",body:Z});if(!G.ok){let X=await G.json().catch(()=>({error:"Upload failed"})),O=Error(X.error||`HTTP ${G.status}`);throw O.status=G.status,O.code=X.code,O}return G.json()}async function C3(_,$,j=""){let Z=await fetch("/workspace/file",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:_,name:$,content:j})});if(!Z.ok){let N=await Z.json().catch(()=>({error:"Create failed"})),K=Error(N.error||`HTTP ${Z.status}`);throw K.status=Z.status,K.code=N.code,K}return Z.json()}async function x3(_,$){let j=await fetch("/workspace/rename",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:_,name:$})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Rename failed"})),N=Error(Z.error||`HTTP ${j.status}`);throw N.status=j.status,N.code=Z.code,N}return j.json()}async function S3(_,$){let j=await fetch("/workspace/move",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:_,target:$})});if(!j.ok){let Z=await j.json().catch(()=>({error:"Move failed"})),N=Error(Z.error||`HTTP ${j.status}`);throw N.status=j.status,N.code=Z.code,N}return j.json()}async function f3(_){let $=`/workspace/file?path=${encodeURIComponent(_||"")}`;return f_($,{method:"DELETE"})}async function Z$(_,$=!1){return f_("/workspace/visibility",{method:"POST",body:JSON.stringify({visible:Boolean(_),show_hidden:Boolean($)})})}function M$(_){return`/workspace/raw?path=${encodeURIComponent(_)}`}function T3(_,$=!1){return`/workspace/download?${`path=${encodeURIComponent(_||"")}&show_hidden=${$?"1":"0"}`}`}class C${constructor(_,$,j={}){this.onEvent=_,this.onStatusChange=$,this.chatJid=typeof j?.chatJid==="string"&&j.chatJid.trim()?j.chatJid.trim():null,this.eventSource=null,this.reconnectTimeout=null,this.reconnectDelay=1000,this.status="disconnected",this.reconnectAttempts=0,this.cooldownUntil=0,this.connecting=!1}connect(){if(this.connecting)return;if(this.eventSource&&this.status==="connected")return;if(this.connecting=!0,this.eventSource)this.eventSource.close();let _=this.chatJid?`?chat_jid=${encodeURIComponent(this.chatJid)}`:"";this.eventSource=new EventSource("/sse/stream"+_),this.eventSource.onopen=()=>{this.connecting=!1,this.reconnectDelay=1000,this.reconnectAttempts=0,this.cooldownUntil=0,this.status="connected",this.onStatusChange("connected")},this.eventSource.onerror=()=>{this.connecting=!1,this.status="disconnected",this.onStatusChange("disconnected"),this.reconnectAttempts+=1,this.scheduleReconnect()},this.eventSource.addEventListener("connected",()=>{console.log("SSE connected"),this.onEvent("connected",{})}),this.eventSource.addEventListener("new_post",($)=>{this.onEvent("new_post",JSON.parse($.data))}),this.eventSource.addEventListener("new_reply",($)=>{this.onEvent("new_reply",JSON.parse($.data))}),this.eventSource.addEventListener("agent_response",($)=>{this.onEvent("agent_response",JSON.parse($.data))}),this.eventSource.addEventListener("interaction_updated",($)=>{this.onEvent("interaction_updated",JSON.parse($.data))}),this.eventSource.addEventListener("interaction_deleted",($)=>{this.onEvent("interaction_deleted",JSON.parse($.data))}),this.eventSource.addEventListener("agent_status",($)=>{this.onEvent("agent_status",JSON.parse($.data))}),this.eventSource.addEventListener("agent_steer_queued",($)=>{this.onEvent("agent_steer_queued",JSON.parse($.data))}),this.eventSource.addEventListener("agent_followup_queued",($)=>{this.onEvent("agent_followup_queued",JSON.parse($.data))}),this.eventSource.addEventListener("agent_followup_consumed",($)=>{this.onEvent("agent_followup_consumed",JSON.parse($.data))}),this.eventSource.addEventListener("agent_followup_removed",($)=>{this.onEvent("agent_followup_removed",JSON.parse($.data))}),this.eventSource.addEventListener("agent_request",($)=>{this.onEvent("agent_request",JSON.parse($.data))}),this.eventSource.addEventListener("workspace_update",($)=>{this.onEvent("workspace_update",JSON.parse($.data))}),this.eventSource.addEventListener("agent_request_timeout",($)=>{this.onEvent("agent_request_timeout",JSON.parse($.data))}),this.eventSource.addEventListener("agent_draft",($)=>{this.onEvent("agent_draft",JSON.parse($.data))}),this.eventSource.addEventListener("agent_draft_delta",($)=>{this.onEvent("agent_draft_delta",JSON.parse($.data))}),this.eventSource.addEventListener("agent_thought",($)=>{this.onEvent("agent_thought",JSON.parse($.data))}),this.eventSource.addEventListener("agent_thought_delta",($)=>{this.onEvent("agent_thought_delta",JSON.parse($.data))}),this.eventSource.addEventListener("model_changed",($)=>{this.onEvent("model_changed",JSON.parse($.data))}),this.eventSource.addEventListener("ui_theme",($)=>{this.onEvent("ui_theme",JSON.parse($.data))})}scheduleReconnect(){if(this.reconnectTimeout)clearTimeout(this.reconnectTimeout);let _=10,$=60000,j=Date.now();if(this.reconnectAttempts>=_)this.cooldownUntil=Math.max(this.cooldownUntil,j+$),this.reconnectAttempts=0;let Z=Math.max(this.cooldownUntil-j,0),N=Math.max(this.reconnectDelay,Z);this.reconnectTimeout=setTimeout(()=>{console.log("Reconnecting SSE..."),this.connect()},N),this.reconnectDelay=Math.min(this.reconnectDelay*2,30000)}reconnectIfNeeded(){if(this.status==="connected")return;let _=Date.now();if(this.cooldownUntil&&_<this.cooldownUntil)return;if(this.reconnectTimeout)clearTimeout(this.reconnectTimeout),this.reconnectTimeout=null;this.connect()}disconnect(){if(this.connecting=!1,this.eventSource)this.eventSource.close(),this.eventSource=null;if(this.reconnectTimeout)clearTimeout(this.reconnectTimeout),this.reconnectTimeout=null}}function s0(_){if(typeof window>"u"||!window.localStorage)return null;try{return window.localStorage.getItem(_)}catch{return null}}function s_(_,$){if(typeof window>"u"||!window.localStorage)return;try{window.localStorage.setItem(_,$)}catch{}}function p2(_,$=!1){let j=s0(_);if(j===null)return $;return j==="true"}function h2(_,$=null){let j=s0(_);if(j===null)return $;let Z=parseInt(j,10);return Number.isFinite(Z)?Z:$}function x$(_){return String(_||"").trim().toLowerCase()}function I3(_){let $=String(_||"").match(/^@([a-zA-Z0-9_-]*)$/);if(!$)return null;return x$($[1]||"")}function Y4(_){let $=new Set,j=[];for(let Z of Array.isArray(_)?_:[]){let N=x$(Z?.agent_name);if(!N||$.has(N))continue;$.add(N),j.push(Z)}return j}function G4(_,$,j={}){let Z=I3($);if(Z==null)return[];let N=typeof j?.currentChatJid==="string"?j.currentChatJid:null;return Y4(_).filter((K)=>{if(N&&K?.chat_jid===N)return!1;return x$(K?.agent_name).startsWith(Z)})}function m3(_){let $=x$(_);return $?`@${$} `:""}function O4(_,$={}){let j=typeof $?.currentChatJid==="string"?$.currentChatJid:null,Z=Number.isFinite($?.limit)?Math.max(0,$.limit):4;return Y4(_).filter((N)=>!(j&&N?.chat_jid===j)).slice(0,Z)}function L4({footerWidth:_=0,visibleAgentCount:$=0,hasContextIndicator:j=!1}={}){let Z=Number(_||0),N=Math.max(0,Math.min(Number($||0),4));if(!Number.isFinite(Z)||Z<=0)return!1;if(N<=0)return!1;let K=460+N*68+(j?40:0);return Z>=K}function Q2({prefix:_="file",label:$,title:j,onRemove:Z,onClick:N,removeTitle:K="Remove",icon:z="file"}){let G=`${_}-file-pill`,X=`${_}-file-name`,O=`${_}-file-remove`,q=z==="message"?U`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  `}var L5=[{name:"/model",description:"Select model or list available models"},{name:"/cycle-model",description:"Cycle to the next available model"},{name:"/thinking",description:"Show or set thinking level"},{name:"/cycle-thinking",description:"Cycle thinking level"},{name:"/theme",description:"Set UI theme (use /theme list for options)"},{name:"/tint",description:"Tint default light/dark UI (usage: /tint #hex or /tint off)"},{name:"/test-card",description:"Emit a built-in Adaptive Card test message (/test-card list for variants)"},{name:"/btw",description:"Open a side conversation panel without interrupting the main chat"},{name:"/state",description:"Show current session state"},{name:"/stats",description:"Show session token and cost stats"},{name:"/context",description:"Show context window usage"},{name:"/last",description:"Show last assistant response"},{name:"/compact",description:"Manually compact the session"},{name:"/auto-compact",description:"Toggle auto-compaction"},{name:"/auto-retry",description:"Toggle auto-retry"},{name:"/abort",description:"Abort the current response"},{name:"/abort-retry",description:"Abort retry backoff"},{name:"/abort-bash",description:"Abort running bash command"},{name:"/shell",description:"Run a shell command and return output"},{name:"/bash",description:"Run a shell command and add output to context"},{name:"/queue",description:"Queue a follow-up message (one-at-a-time)"},{name:"/queue-all",description:"Queue a follow-up message (batch all)"},{name:"/steer",description:"Steer the current response"},{name:"/steering-mode",description:"Set steering mode (all|one)"},{name:"/followup-mode",description:"Set follow-up mode (all|one)"},{name:"/session-name",description:"Set or show the session name"},{name:"/new-session",description:"Start a new session"},{name:"/switch-session",description:"Switch to a session file"},{name:"/fork",description:"Fork from a previous message"},{name:"/forks",description:"List forkable messages"},{name:"/tree",description:"List the session tree"},{name:"/label",description:"Set or clear a label on a tree entry"},{name:"/labels",description:"List labeled entries"},{name:"/agent-name",description:"Set or show the agent display name"},{name:"/agent-avatar",description:"Set or show the agent avatar URL"},{name:"/user-name",description:"Set or show your display name"},{name:"/user-avatar",description:"Set or show your avatar URL"},{name:"/user-github",description:"Set name/avatar from GitHub profile"},{name:"/export-html",description:"Export session to HTML"},{name:"/passkey",description:"Manage passkeys (enrol/list/delete)"},{name:"/totp",description:"Show a TOTP enrolment QR code"},{name:"/qr",description:"Generate a QR code for text or URL"},{name:"/search",description:"Search notes and skills in the workspace"},{name:"/restart",description:"Restart the agent and stop subprocesses"},{name:"/commands",description:"List available commands"}];function X5({usage:_}){let $=Math.min(100,Math.max(0,_.percent||0)),j=_.tokens,Z=_.contextWindow,N=j!=null?`Context: ${X4(j)} / ${X4(Z)} tokens (${$.toFixed(0)}%)`:`Context: ${$.toFixed(0)}%`,K=7,z=2*Math.PI*7,G=$/100*z,X=$>90?"var(--context-red, #ef4444)":$>75?"var(--context-amber, #f59e0b)":"var(--context-green, #22c55e)";return U`
    ...
    `}function X4(_){if(_==null)return"?";if(_>=1e6)return(_/1e6).toFixed(1)+"M";if(_>=1000)return(_/1000).toFixed(0)+"K";return String(_)}function W4({onPost:_,onFocus:$,searchMode:j,searchScope:Z="current",onSearch:N,onSearchScopeChange:K,onEnterSearch:z,onExitSearch:G,fileRefs:X=[],onRemoveFileRef:O,onClearFileRefs:q,messageRefs:V=[],onRemoveMessageRef:H,onClearMessageRefs:b,activeModel:f=null,modelUsage:T=null,thinkingLevel:v=null,supportsThinking:x=!1,contextUsage:F=null,notificationsEnabled:A=!1,notificationPermission:C="default",onToggleNotifications:c,onModelChange:a,onModelStateChange:t,activeEditorPath:l=null,onAttachEditorFile:N_,onOpenFilePill:i,followupQueueItems:Q_=[],onInjectQueuedFollowup:U_,onRemoveQueuedFollowup:s,onSubmitIntercept:__,onMessageResponse:K_,onPopOutChat:j_,isAgentActive:G_=!1,activeChatAgents:X_=[],currentChatJid:J_="web:default"}){let[Z_,D_]=I(""),[k_,r_]=I(""),[E_,u_]=I([]),[b_,l_]=I(!1),[p,L_]=I([]),[W_,V_]=I(0),[y_,w_]=I(!1),[v_,F_]=I([]),[C_,S_]=I(0),[Y0,n_]=I(!1),[e_,B0]=I(!1),[x_,_0]=I(!1),[Q0,D0]=I([]),[F0,G0]=I(!1),[E0,o_]=I(0),T_=y(null),M0=y(null),c_=y(null),d_=y(null),k0=y(null),B_=y(null),M=y(0),e=200,A_=(B)=>{let S=new Set,n=[];for(let Y_ of B||[]){if(typeof Y_!=="string")continue;let P_=Y_.trim();if(!P_||S.has(P_))continue;S.add(P_),n.push(P_)}return n},R_=()=>{let B=s0("piclaw_compose_history");if(!B)return[];try{let S=JSON.parse(B);if(!Array.isArray(S))return[];return A_(S)}catch{return[]}},b0=(B)=>{s_("piclaw_compose_history",JSON.stringify(B))},a0=y(R_()),$0=y(-1),O0=y(""),W0=Z_.trim()||E_.length>0||X.length>0||V.length>0,t0=typeof window<"u"&&typeof navigator<"u"&&Boolean(navigator.geolocation)&&Boolean(window.isSecureContext),C0=typeof window<"u"&&typeof Notification<"u",R0=typeof window<"u"?Boolean(window.isSecureContext):!1,n2=C0&&R0&&C!=="denied",b2=C==="granted"&&A,c0=b2?"Disable notifications":"Enable notifications",j0=O4(X_,{currentChatJid:J_,limit:4}),K2=j0.length>0,L0=!j&&L4({footerWidth:E0,visibleAgentCount:j0.length,hasContextIndicator:Boolean(F&&F.percent!=null)}),Z0=f||"",I_=x&&v?` (${v})`:"",I0=I_.trim()?`${v}`:"",p0=typeof T?.hint_short==="string"?T.hint_short.trim():"",z2=[I0||null,p0||null].filter(Boolean).join(" • "),J2=[Z0?`Current model: ${Z0}${I_}`:null,T?.plan?`Plan: ${T.plan}`:null,p0||null,T?.primary?.reset_description||null,T?.secondary?.reset_description||null].filter(Boolean),w2=e_?"Switching model…":J2.join(" • ")||`Current model: ${Z0}${I_} (tap to open model picker)`,H0=(B)=>{if(!B||typeof B!=="object")return;let S=B.model??B.current;if(typeof t==="function")t({model:S??null,thinking_level:B.thinking_level??null,supports_thinking:B.supports_thinking,provider_usage:B.provider_usage??null});if(S&&typeof a==="function")a(S)},A2=(B)=>{let S=B||T_.current;if(!S)return;S.style.height="auto",S.style.height=`${S.scrollHeight}px`,S.style.overflowY="hidden"},o2=(B)=>{if(!B.startsWith("/")||B.includes(`
`)){w_(!1),L_([]);return}let S=B.toLowerCase().split(" ")[0];if(S.length<1){w_(!1),L_([]);return}let n=L5.filter((Y_)=>Y_.name.startsWith(S)||Y_.name.replace(/-/g,"").startsWith(S.replace(/-/g,"")));if(n.length>0&&!(n.length===1&&n[0].name===S))n_(!1),F_([]),L_(n),V_(0),w_(!0);else w_(!1),L_([])},y2=(B)=>{let S=Z_,n=S.indexOf(" "),Y_=n>=0?S.slice(n):"",P_=B.name+Y_;D_(P_),w_(!1),L_([]),requestAnimationFrame(()=>{let t_=T_.current;if(!t_)return;let M_=P_.length;t_.selectionStart=M_,t_.selectionEnd=M_,t_.focus()})},m2=(B)=>{if(I3(B)==null){n_(!1),F_([]);return}let S=G4(X_,B,{currentChatJid:J_});if(S.length>0&&!(S.length===1&&m3(S[0].agent_name).trim().toLowerCase()===String(B||"").trim().toLowerCase()))w_(!1),L_([]),F_(S),S_(0),n_(!0);else n_(!1),F_([])},q2=(B)=>{let S=m3(B?.agent_name);if(!S)return;D_(S),n_(!1),F_([]),requestAnimationFrame(()=>{let n=T_.current;if(!n)return;let Y_=S.length;n.selectionStart=Y_,n.selectionEnd=Y_,n.focus()})},a_=(B)=>{if(j)r_(B);else D_(B),o2(B),m2(B);requestAnimationFrame(()=>A2())},P2=(B)=>{let S=j?k_:Z_,n=S&&!S.endsWith(`
`)?`
`:"",Y_=`${S}${n}${B}`.trimStart();a_(Y_)},x0=(B)=>{let S=B?.command?.model_label;if(S)return S;let n=B?.command?.message;if(typeof n==="string"){let Y_=n.match(/•\s+([^\n]+?)\s+\(current\)/);if(Y_?.[1])return Y_[1].trim()}return null},S0=async(B)=>{if(j||e_)return;B0(!0);try{let S=await g2("default",B,null,[],null,J_),n=x0(S);H0({model:n??f??null,thinking_level:S?.command?.thinking_level,supports_thinking:S?.command?.supports_thinking});try{let Y_=await $$(J_);if(Y_)H0(Y_)}catch{}return _?.(),!0}catch(S){return console.error("Failed to switch model:",S),alert("Failed to switch model: "+S.message),!1}finally{B0(!1)}},Y2=async()=>{await S0("/cycle-model")},X$=async(B)=>{if(!B||e_)return;if(await S0(`/model ${B}`))_0(!1)},U0=(B)=>{B.preventDefault(),B.stopPropagation(),_0((S)=>!S)},M2=(B)=>{if(B==="queue"||B==="steer"||B==="auto")return B;return G_?"queue":null},G2=async(B,S,n={})=>{let{includeMedia:Y_=!0,includeFileRefs:P_=!0,includeMessageRefs:t_=!0,clearAfterSubmit:M_=!0,recordHistory:N0=!0}=n||{},i0=typeof B==="string"?B:B&&typeof B?.target?.value==="string"?B.target.value:Z_,D2=typeof i0==="string"?i0:"";if(!D2.trim()&&(Y_?E_.length===0:!0)&&(P_?X.length===0:!0)&&(t_?V.length===0:!0))return;w_(!1),L_([]),n_(!1),F_([]);let m0=Y_?[...E_]:[],L2=P_?[...X]:[],X2=t_?[...V]:[],l0=D2.trim();if(N0&&l0){let W2=a0.current,J0=A_(W2.filter((f2)=>f2!==l0));if(J0.push(l0),J0.length>200)J0.splice(0,J0.length-200);a0.current=J0,b0(J0),$0.current=-1,O0.current=""}if(M_)D_(""),u_([]),q?.(),b?.();(async()=>{try{if(await __?.({content:l0,submitMode:S,fileRefs:L2,messageRefs:X2,mediaFiles:m0})){_?.();return}let J0=[];for(let m of m0){let d=await J3(m);J0.push(d.id)}let f2=L2.length?`Files:
${L2.map((m)=>`- ${m}`).join(`
        ...
        <div class="compose-box">
            ...
            >
                <div class="compose-input-main">
                    ${(X.length>0||E_.length>0||V.length>0)&&U`
                        <div class="compose-file-refs">
                            ${V.map((B)=>{return U`
                                    ...
                                    />
                                `})}
                            ${X.map((B)=>{let S=B.split("/").pop()||B;return U`
                                    <${Q2}
                                        prefix="compose"
                                        label=${S}
                                        title=${B}
...
`),blocks:Z}}function P5(_){if(!_)return _;return u$(_,5)}function M5(_){let $=new TextEncoder().encode(String(_||"")),j="";for(let Z of $)j+=String.fromCharCode(Z);return btoa(j)}function C5(_){let $=atob(String(_||"")),j=new Uint8Array($.length);for(let Z=0;Z<$.length;Z+=1)j[Z]=$.charCodeAt(Z);return new TextDecoder().decode(j)}function x5(_,$){if(!_||!$||$.length===0)return _;return _.replace(/@@MERMAID_BLOCK_(\d+)@@/g,(j,Z)=>{let N=Number(Z),K=$[N]??"",z=P5(K);return`<div class="mermaid-container" data-mermaid="${M5(z)}"><div class="mermaid-loading">Loading diagram...</div></div>`})}function b4(_){if(!_)return _;return _.replace(/<code>([\s\S]*?)<\/code>/gi,($,j)=>{if(j.includes(`
`))return`
\`\`\`
${j}
\`\`\`
`;return`\`${j}\``})}function w4(_){if(!_)return _;return _.replace(/&lt;([\s\S]*?)&gt;/g,($,j)=>{let Z=j.trim(),N=Z.startsWith("/"),K=N?Z.slice(1).trim():Z,X=(K.endsWith("/")?K.slice(0,-1).trim():K).split(/\s+/)[0]?.toLowerCase();if(!X||!D5.has(X))return $;if(X==="br")return N?"":"<br>";if(N)return`</${X}>`;return`<${X}>`})}function A4(_){if(!_)return _;let $=(j)=>j.replace(/&amp;lt;/g,"&lt;").replace(/&amp;gt;/g,"&gt;").replace(/&amp;quot;/g,"&quot;").replace(/&amp;#39;/g,"&#39;").replace(/&amp;amp;/g,"&amp;");return _.replace(/<pre><code>([\s\S]*?)<\/code><\/pre>/g,(j,Z)=>`<pre><code>${$(Z)}</code></pre>`).replace(/<code>([\s\S]*?)<\/code>/g,(j,Z)=>`<code>${$(Z)}</code>`)}function P4(_){if(!_)return _;let $=new DOMParser().parseFromString(_,"text/html"),j=$.createTreeWalker($.body,NodeFilter.SHOW_TEXT),Z=(K)=>K.replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&amp;/g,"&"),N;while(N=j.nextNode()){if(!N.nodeValue)continue;let K=Z(N.nodeValue);if(K!==N.nodeValue)N.nodeValue=K}return $.body.innerHTML}function S5(_){if(!window.katex)return _;let $=(z)=>k4(z).replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&amp;/g,"&").replace(/<br\s*\/?\s*>/gi,`
`),j=(z)=>{let G=[],X=z.replace(/<pre\b[^>]*>\s*<code\b[^>]*>[\s\S]*?<\/code>\s*<\/pre>/gi,(O)=>{let q=G.length;return G.push(O),`@@CODE_BLOCK_${q}@@`});return X=X.replace(/<code\b[^>]*>[\s\S]*?<\/code>/gi,(O)=>{let q=G.length;return G.push(O),`@@CODE_INLINE_${q}@@`}),{html:X,blocks:G}},Z=(z,G)=>{if(!G.length)return z;return z.replace(/@@CODE_(?:BLOCK|INLINE)_(\d+)@@/g,(X,O)=>{let q=Number(O);return G[q]??""})},N=j(_),K=N.html;return K=K.replace(/(^|\n|<br\s*\/?\s*>|<p>|<\/p>)\s*\$\$([\s\S]+?)\$\$\s*(?=\n|<br\s*\/?\s*>|<\/p>|$)/gi,(z,G,X)=>{try{let O=katex.renderToString($(X.trim()),{displayMode:!0,throwOnError:!1});return`${G}${O}`}catch(O){return`<span class="math-error" title="${D4(O.message)}">${z}</span>`}}),K=K.replace(/(^|[^\\$])\$(?!\s)([^\n$]+?)\$/g,(z,G,X)=>{if(/\s$/.test(X))return z;try{let O=katex.renderToString($(X),{displayMode:!1,throwOnError:!1});return`${G}${O}`}catch(O){return`${G}<span class="math-error" title="${D4(O.message)}">$${X}$</span>`}}),Z(K,N.blocks)}function f5(_){if(!_)return _;let $=new DOMParser().parseFromString(_,"text/html"),j=$.createTreeWalker($.body,NodeFilter.SHOW_TEXT),Z=[],N;while(N=j.nextNode())Z.push(N);for(let K of Z){let z=K.nodeValue;if(!z)continue;if(m$.lastIndex=0,!m$.test(z))continue;m$.lastIndex=0;let G=K.parentElement;if(G&&(G.closest("a")||G.closest("code")||G.closest("pre")))continue;let X=z.split(m$);if(X.length<=1)continue;let O=$.createDocumentFragment();X.forEach((q,V)=>{if(V%2===1){let H=$.createElement("a");H.setAttribute("href","#"),H.className="hashtag",H.setAttribute("data-hashtag",q),H.textContent=`#${q}`,O.appendChild(H)}else O.appendChild($.createTextNode(q))}),K.parentNode?.replaceChild(O,K)}return $.body.innerHTML}function T5(_){if(!_)return _;let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
...
`)}function v0(_,$,j={}){if(!_)return"";let Z=T5(_),{text:N,blocks:K}=A5(Z),z=u$(N,2),X=b4(z).replace(/</g,"&lt;").replace(/>/g,"&gt;"),O=w4(X),q=window.marked?marked.parse(O,{headerIds:!1,mangle:!1}):O.replace(/\n/g,"<br>");return q=A4(q),q=P4(q),q=S5(q),q=f5(q),q=x5(q,K),q=E4(q,j),q}function v$(_){if(!_)return"";let $=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`),j=u$($,2),N=b4(j).replace(/</g,"&lt;").replace(/>/g,"&gt;"),K=w4(N),z=window.marked?marked.parse(K):K.replace(/\n/g,"<br>");return z=A4(z),z=P4(z),z=E4(z),z}async function F2(_){if(!window.beautifulMermaid)return;let{renderMermaid:$,THEMES:j}=window.beautifulMermaid,N=y4()==="dark"?j["tokyo-night"]:j["github-light"],K=_.querySelectorAll(".mermaid-container[data-mermaid]");for(let z of K)try{let G=z.dataset.mermaid,X=C5(G||""),O=u$(X,2),q=await $(O,{...N,transparent:!0});z.innerHTML=q,z.removeAttribute("data-mermaid")}catch(G){console.error("Mermaid render error:",G);let X=document.createElement("pre");X.className="mermaid-error",X.textContent=`Diagram error: ${G.message}`,z.innerHTML="",z.appendChild(X),z.removeAttribute("data-mermaid")}}function M4(_){let $=String(_||"").trim();if(!$.startsWith("/btw"))return null;let j=$.slice(4).trim();if(!j)return{type:"help"};if(j==="clear"||j==="close")return{type:"clear"};return{type:"ask",question:j}}function C4(_){return String(_||"").trim()||"web:default"}function x4(_){if(!_)return!1;let $=String(_.answer||"").trim();return _.status!=="running"&&Boolean($)}function S4(_){if(!_)return!1;return _.status!=="running"}function f4(_){let $=String(_?.question||"").trim(),j=String(_?.answer||"").trim();if(!$&&!j)return"";return["BTW side conversation",$?`Question: ${$}`:null,j?`Answer:
${j}`:null].filter(Boolean).join(`

`)}function T4({session:_,onClose:$,onInject:j,onRetry:Z}){let N=y(null),K=y(null),z=_?.thinking?v$(_.thinking):"",G=_?.answer?v0(_.answer,null,{sanitize:!1}):"";if(u(()=>{if(N.current&&z)F2(N.current).catch(()=>{})},[z]),u(()=>{if(K.current&&G)F2(K.current).catch(()=>{})},[G]),!_)return null;let X=_.status==="running",O=Boolean(String(_.answer||"").trim()),q=Boolean(String(_.thinking||"").trim()),V=x4(_),H=S4(_),b=!X&&O,f=X?"Thinking…":_.status==="error"?"Error":"Done";return U`
    ...
    `}var I4="PiClaw";function p3(_,$){let j=_||"PiClaw",Z=j.charAt(0).toUpperCase(),N=["#FF6B6B","#4ECDC4","#45B7D1","#FFA07A","#98D8C8","#F7DC6F","#BB8FCE","#85C1E2","#F8B195","#6C5CE7","#00B894","#FDCB6E","#E17055","#74B9FF","#A29BFE","#FD79A8","#00CEC9","#FFEAA7","#DFE6E9","#FF7675","#55EFC4","#81ECEC","#FAB1A0","#74B9FF","#A29BFE","#FD79A8"],K=Z.charCodeAt(0)%N.length,z=N[K],G=j.trim().toLowerCase(),X=typeof $==="string"?$.trim():"",q=(X?X:null)||(G==="PiClaw".toLowerCase()||G==="pi"?"/static/icon-192.png":null);return{letter:Z,color:z,image:q}}function m4(_,$){if(!_)return"PiClaw";let j=$[_]?.name||_;return j?j.charAt(0).toUpperCase()+j.slice(1):"PiClaw"}function u4(_,$){if(!_)return null;let j=$[_]||{};return j.avatar_url||j.avatarUrl||j.avatar||null}function v4(_){if(!_)return null;if(typeof document<"u"){let K=document.documentElement,z=K?.dataset?.colorTheme||"",G=K?.dataset?.tint||"",X=getComputedStyle(K).getPropertyValue("--accent-color")?.trim();if(X&&(G||z&&z!=="default"))return X}let $=["#4ECDC4","#FF6B6B","#45B7D1","#BB8FCE","#FDCB6E","#00B894","#74B9FF","#FD79A8","#81ECEC","#FFA07A"],j=String(_),Z=0;for(let K=0;K<j.length;K+=1)Z=(Z*31+j.charCodeAt(K))%2147483647;let N=Math.abs(Z)%$.length;return $[N]}function I5(_){if(!_||typeof _!=="object")return null;let $=_.started_at??_.startedAt;if(typeof $!=="string"||!$)return null;let j=Date.parse($);return Number.isFinite(j)?j:null}function R$(_){if(!_||typeof _!=="object")return!1;let $=_.intent_key??_.intentKey;return _.type==="intent"&&$==="compaction"}function m5(_){let $=Math.max(0,Math.floor(_/1000)),j=$%60,Z=Math.floor($/60)%60,N=Math.floor($/3600);if(N>0)return`${N}:${String(Z).padStart(2,"0")}:${String(j).padStart(2,"0")}`;return`${Z}:${String(j).padStart(2,"0")}`}function R4(_,$=Date.now()){let j=I5(_);if(j===null)return null;return m5(Math.max(0,$-j))}function g4({status:_,draft:$,plan:j,thought:Z,pendingRequest:N,intent:K,turnId:z,steerQueued:G,onPanelToggle:X}){let V=(p)=>{if(!p)return{text:"",totalLines:0,fullText:""};if(typeof p==="string"){let y_=p,w_=y_?y_.replace(/\r\n/g,`
`).split(`
`).length:0;return{text:y_,totalLines:w_,fullText:y_}}let L_=p.text||"",W_=p.fullText||p.full_text||L_,V_=Number.isFinite(p.totalLines)?p.totalLines:W_?W_.replace(/\r\n/g,`
`).split(`
`).length:0;return{text:L_,totalLines:V_,fullText:W_}},H=160,b=(p)=>{if(!p)return 1;return Math.max(1,Math.ceil(p.length/160))},f=(p,L_,W_)=>{let V_=(p||"").replace(/\r\n/g,`
`).replace(/\r/g,`
`);if(!V_)return{text:"",omitted:0,totalLines:Number.isFinite(W_)?W_:0,visibleLines:0};let y_=V_.split(`
`),w_=y_.length>L_?y_.slice(0,L_).join(`
`):V_,v_=Number.isFinite(W_)?W_:y_.reduce((S_,Y0)=>S_+b(Y0),0),F_=w_?w_.split(`
`).reduce((S_,Y0)=>S_+b(Y0),0):0,C_=Math.max(v_-F_,0);return{text:w_,omitted:C_,totalLines:v_,visibleLines:F_}},T=V(j),v=V(Z),x=V($),F=Boolean(T.text)||T.totalLines>0,A=Boolean(v.text)||v.totalLines>0,C=Boolean(x.fullText?.trim()||x.text?.trim());if(!_&&!C&&!F&&!A&&!N&&!K)return null;let[c,a]=I(new Set),[t,l]=I(()=>Date.now()),N_=(p)=>a((L_)=>{let W_=new Set(L_),V_=!W_.has(p);if(V_)W_.add(p);else W_.delete(p);if(typeof X==="function")X(p,V_);return W_});u(()=>{a(new Set)},[z]);let i=R$(_);u(()=>{if(!i)return;l(Date.now());let p=setInterval(()=>l(Date.now()),1000);return()=>clearInterval(p)},[i,_?.started_at,_?.startedAt]);let Q_=_?.turn_id||z,U_=v4(Q_),s=G?"turn-dot turn-dot-queued":"turn-dot",__=(p)=>p,K_=Boolean(_?.last_activity||_?.lastActivity),j_=(p)=>p==="warning"?"#f59e0b":p==="error"?"var(--danger-color)":p==="success"?"var(--success-color)":U_,G_=K?.kind||"info",X_=j_(G_),J_=j_(_?.kind||(i?"warning":"info")),Z_="",D_=_?.title,k_=_?.status;if(_?.type==="plan")Z_=D_?`Planning: ${D_}`:"Planning...";else if(_?.type==="tool_call")Z_=D_?`Running: ${D_}`:"Running tool...";else if(_?.type==="tool_status")Z_=D_?`${D_}: ${k_||"Working..."}`:k_||"Working...";else if(_?.type==="error")Z_=D_||"Agent error";else Z_=D_||k_||"Working...";if(K_)Z_="Last activity just now";let r_=({panelTitle:p,text:L_,fullText:W_,totalLines:V_,maxLines:y_,titleClass:w_,panelKey:v_})=>{let F_=c.has(v_),C_=W_||L_||"",S_=typeof y_==="number",Y0=F_&&S_,n_=S_?f(C_,y_,V_):{text:C_||"",omitted:0,totalLines:Number.isFinite(V_)?V_:0};if(!C_&&!(Number.isFinite(n_.totalLines)&&n_.totalLines>0))return null;let e_=`agent-thinking-body${S_?" agent-thinking-body-collapsible":""}`,B0=S_?`--agent-thinking-collapsed-lines: ${y_};`:"";return U`
            <div
            ...
            >
                <div class="agent-thinking-title ${w_||""}">
                    ${U_&&U`<span class=${s} aria-hidden="true"></span>`}
                    ${p}
                    ${Y0&&U`
                        <button
        ...
        `},E_=N?.tool_call?.title,u_=E_?`Awaiting approval: ${E_}`:"Awaiting approval",b_=i?R4(_,t):null,l_=(p,L_,W_=null)=>U`
        <div
        ...
        >
            <div class="agent-thinking-title intent">
                ${L_&&U`<span class=${s} aria-hidden="true"></span>`}
    ...
    `;return U`
        <div class="agent-status-panel">
            ...
            `}
            ${F&&r_({panelTitle:__("Planning"),text:T.text,fullText:T.fullText,totalLines:T.totalLines,panelKey:"plan"})}
            ${A&&r_({panelTitle:__("Thoughts"),text:v.text,fullText:v.fullText,totalLines:v.totalLines,maxLines:8,titleClass:"thought",panelKey:"thought"})}
            ${C&&r_({panelTitle:__("Draft"),text:x.text,fullText:x.fullText,totalLines:x.totalLines,maxLines:8,titleClass:"thought",panelKey:"draft"})}
            ${_&&_?.type!=="intent"&&U`
    ...
    `}function c4({request:_,onRespond:$}){if(!_)return null;let{request_id:j,tool_call:Z,options:N,chat_jid:K}=_,z=Z?.title||"Agent Request",G=Z?.kind||"other",X=Z?.rawInput||{},O=X.command||X.commands&&X.commands[0]||null,q=X.diff||null,V=X.fileName||X.path||null,H=Z?.description||X.description||X.explanation||null,f=(Array.isArray(Z?.locations)?Z.locations:[]).map((A)=>A?.path).filter((A)=>Boolean(A)),T=Array.from(new Set([V,...f].filter(Boolean)));console.log("AgentRequestModal:",{request_id:j,tool_call:Z,options:N});let v=async(A)=>{try{await A$(j,A,K||null),$()}catch(C){console.error("Failed to respond to agent request:",C)}},x=async()=>{try{await D3(z,`Auto-approved: ${z}`),await A$(j,"approved",K||null),$()}catch(A){console.error("Failed to add to whitelist:",A)}},F=N&&N.length>0;return U`
    ...
    `}function z9({mediaId:_,onPreview:$}){let[j,Z]=I(null);if(u(()=>{c2(_).then(Z).catch(()=>{})},[_]),!j)return null;let N=j.filename||"file",K=j.metadata?.size,z=K?r0(K):"",X=z$(j.content_type)==="unsupported"?"Details":"Preview";return U`
    ...
    `}function Y9({attachment:_,onPreview:$}){let j=Number(_?.id),[Z,N]=I(null);u(()=>{if(!Number.isFinite(j))return;c2(j).then(N).catch(()=>{});return},[j]);let K=Z?.filename||_.label||`attachment-${_.id}`,z=Number.isFinite(j)?Z2(j):null,X=z$(Z?.content_type)==="unsupported"?"Details":"Preview";return U`
    ...
    `}function G9({block:_}){let $=_.title||_.name||_.uri,j=_.description,Z=_.size?r0(_.size):"",N=_.mime_type||"",K=L9(N),z=T2(_.uri);return U`
        <a
            href=${z||"#"}
            class="resource-link"
    ...
    </svg>`;async function Q9(_){let $=typeof _==="string"?_:"";if(!$)return!1;if(navigator.clipboard?.writeText)try{return await navigator.clipboard.writeText($),!0}catch{}try{let j=document.createElement("textarea");j.value=$,j.setAttribute("readonly",""),j.style.position="fixed",j.style.opacity="0",j.style.pointerEvents="none",document.body.appendChild(j),j.select(),j.setSelectionRange(0,j.value.length);let Z=document.execCommand("copy");return document.body.removeChild(j),Z}catch{return!1}}function F9(_){if(!_)return()=>{};let $=Array.from(_.querySelectorAll("pre")).filter((K)=>K.querySelector("code"));if($.length===0)return()=>{};let j=new Map,Z=[],N=(K,z)=>{let G=z||"idle";if(K.dataset.copyState=G,G==="success")K.innerHTML=U9,K.setAttribute("aria-label","Copied"),K.setAttribute("title","Copied"),K.classList.add("is-success"),K.classList.remove("is-error");else if(G==="error")K.innerHTML=B9,K.setAttribute("aria-label","Copy failed"),K.setAttribute("title","Copy failed"),K.classList.add("is-error"),K.classList.remove("is-success");else K.innerHTML=q9,K.setAttribute("aria-label","Copy code"),K.setAttribute("title","Copy code"),K.classList.remove("is-success","is-error")};return $.forEach((K)=>{let z=document.createElement("div");z.className="post-code-block",K.parentNode?.insertBefore(z,K),z.appendChild(K);let G=document.createElement("button");G.type="button",G.className="post-code-copy-btn",N(G,"idle"),z.appendChild(G);let X=async(O)=>{O.preventDefault(),O.stopPropagation();let V=K.querySelector("code")?.textContent||"",H=await Q9(V);N(G,H?"success":"error");let b=j.get(G);if(b)clearTimeout(b);let f=setTimeout(()=>{N(G,"idle"),j.delete(G)},V9);j.set(G,f)};G.addEventListener("click",X),Z.push(()=>{G.removeEventListener("click",X);let O=j.get(G);if(O)clearTimeout(O);if(z.parentNode)z.parentNode.insertBefore(K,z),z.remove()})}),()=>{Z.forEach((K)=>K())}}function H9(_){if(!_)return{content:_,fileRefs:[]};let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`),Z=-1;for(let O=0;O<j.length;O+=1)if(j[O].trim()==="Files:"&&j[O+1]&&/^\s*-\s+/.test(j[O+1])){Z=O;break}if(Z===-1)return{content:_,fileRefs:[]};let N=[],K=Z+1;for(;K<j.length;K+=1){let O=j[K];if(/^\s*-\s+/.test(O))N.push(O.replace(/^\s*-\s+/,"").trim());else if(!O.trim())break;else break}if(N.length===0)return{content:_,fileRefs:[]};let z=j.slice(0,Z),G=j.slice(K),X=[...z,...G].join(`
`);return X=X.replace(/\n{3,}/g,`

`).trim(),{content:X,fileRefs:N}}function J9(_){if(!_)return{content:_,messageRefs:[]};let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`),Z=-1;for(let O=0;O<j.length;O+=1)if(j[O].trim()==="Referenced messages:"&&j[O+1]&&/^\s*-\s+/.test(j[O+1])){Z=O;break}if(Z===-1)return{content:_,messageRefs:[]};let N=[],K=Z+1;for(;K<j.length;K+=1){let O=j[K];if(/^\s*-\s+/.test(O)){let V=O.replace(/^\s*-\s+/,"").trim().match(/^message:(\S+)$/i);if(V)N.push(V[1])}else if(!O.trim())break;else break}if(N.length===0)return{content:_,messageRefs:[]};let z=j.slice(0,Z),G=j.slice(K),X=[...z,...G].join(`
`);return X=X.replace(/\n{3,}/g,`

`).trim(),{content:X,messageRefs:N}}function y9(_){if(!_)return{content:_,attachments:[]};let j=_.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
`),Z=-1;for(let O=0;O<j.length;O+=1)if(j[O].trim()==="Images:"&&j[O+1]&&/^\s*-\s+/.test(j[O+1])){Z=O;break}if(Z===-1)return{content:_,attachments:[]};let N=[],K=Z+1;for(;K<j.length;K+=1){let O=j[K];if(/^\s*-\s+/.test(O)){let q=O.replace(/^\s*-\s+/,"").trim(),V=q.match(/^attachment:([^\s)]+)\s*(?:\((.+)\))?$/i)||q.match(/^attachment:([^\s]+)\s+(.+)$/i);if(V){let H=V[1],b=(V[2]||"").trim()||H;N.push({id:H,label:b,raw:q})}else N.push({id:null,label:q,raw:q})}else if(!O.trim())break;else break}if(N.length===0)return{content:_,attachments:[]};let z=j.slice(0,Z),G=j.slice(K),X=[...z,...G].join(`
`);return X=X.replace(/\n{3,}/g,`

`).trim(),{content:X,attachments:N}}function D9(_){return _.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function E9(_,$){if(!_||!$)return _;let j=String($).trim().split(/\s+/).filter(Boolean);if(j.length===0)return _;let Z=j.map(D9).sort((q,V)=>V.length-q.length),N=new RegExp(`(${Z.join("|")})`,"gi"),K=new RegExp(`^(${Z.join("|")})$`,"i"),z=new DOMParser().parseFromString(_,"text/html"),G=z.createTreeWalker(z.body,NodeFilter.SHOW_TEXT),X=[],O;while(O=G.nextNode())X.push(O);for(let q of X){let V=q.nodeValue;if(!V||!N.test(V)){N.lastIndex=0;continue}N.lastIndex=0;let H=q.parentElement;if(H&&H.closest("code, pre, script, style"))continue;let b=V.split(N).filter((T)=>T!=="");if(b.length===0)continue;let f=z.createDocumentFragment();for(let T of b)if(K.test(T)){let v=z.createElement("mark");v.className="search-highlight-term",v.textContent=T,f.appendChild(v)}else f.appendChild(z.createTextNode(T));q.parentNode.replaceChild(f,q)}return z.body.innerHTML}function $8({post:_,onClick:$,onHashtagClick:j,onMessageRef:Z,onScrollToMessage:N,agentName:K,agentAvatarUrl:z,userName:G,userAvatarUrl:X,userAvatarBackground:O,onDelete:q,isThreadReply:V,isThreadPrev:H,isThreadNext:b,isRemoving:f,highlightQuery:T,onFileRef:v}){let[x,F]=I(null),A=y(null),C=_.data,c=C.type==="agent_response",a=G||"You",t=c?K||I4:a,l=c?p3(K,z):p3(a,X),N_=typeof O==="string"?O.trim().toLowerCase():"",i=!c&&l.image&&(N_==="clear"||N_==="transparent"),Q_=c&&Boolean(l.image),U_=`background-color: ${i||Q_?"transparent":l.color}`,s=C.content_meta,__=Boolean(s?.truncated),K_=Boolean(s?.preview),j_=__&&!K_,G_=__?{originalLength:Number.isFinite(s?.original_length)?s.original_length:C.content?C.content.length:0,maxLength:Number.isFinite(s?.max_length)?s.max_length:0}:null,X_=C.content_blocks||[],J_=C.media_ids||[],Z_=W9(C.content,C.link_previews),{content:D_,fileRefs:k_}=H9(Z_),{content:r_,messageRefs:E_}=J9(D_),{content:u_,attachments:b_}=y9(r_);Z_=u_;let l_=n3(X_),p=o3(X_),L_=l_.length===1&&typeof l_[0]?.fallback_text==="string"?l_[0].fallback_text.trim():"",W_=p.length===1?a4(p[0]).trim():"",V_=Boolean(L_)&&Z_?.trim()===L_||Boolean(W_)&&Z_?.trim()===W_,y_=Boolean(Z_)&&!j_&&!V_,w_=typeof T==="string"?T.trim():"",v_=i_(()=>{if(!Z_||V_)return"";let M=v0(Z_,j);return w_?E9(M,w_):M},[Z_,V_,w_]),F_=(M,e)=>{M.stopPropagation(),F(Z2(e))},[C_,S_]=I(null),Y0=(M)=>{S_(M)},n_=(M)=>{M.stopPropagation(),q?.(_)},e_=(M,e)=>{let A_=new Set;if(!M||e.length===0)return{content:M,usedIds:A_};return{content:M.replace(/attachment:([^\s)"']+)/g,(b0,a0,$0,O0)=>{let W0=a0.replace(/^\/+/,""),C0=e.find((g0)=>g0.name&&g0.name.toLowerCase()===W0.toLowerCase()&&!A_.has(g0.id))||e.find((g0)=>!A_.has(g0.id));if(!C0)return b0;if(A_.add(C0.id),O0.slice(Math.max(0,$0-2),$0)==="](")return`/media/${C0.id}`;return C0.name||"attachment"}),usedIds:A_}},B0=[],x_=[],_0=[],Q0=[],D0=[],F0=[],G0=0;if(X_.length>0)X_.forEach((M)=>{if(M?.type==="text"&&M.annotations)F0.push(M.annotations);if(M?.type==="resource_link")Q0.push(M);else if(M?.type==="resource")D0.push(M);else if(M?.type==="file"){let e=J_[G0++];if(e)x_.push(e),_0.push({id:e,name:M?.name||M?.filename||M?.title})}else if(M?.type==="image"||!M?.type){let e=J_[G0++];if(e){let A_=typeof M?.mime_type==="string"?M.mime_type:void 0;B0.push({id:e,annotations:M?.annotations,mimeType:A_}),_0.push({id:e,name:M?.name||M?.filename||M?.title})}}});else if(J_.length>0)J_.forEach((M)=>{B0.push({id:M,annotations:null}),_0.push({id:M,name:null})});if(b_.length>0)b_.forEach((M)=>{if(!M?.id)return;let e=_0.find((A_)=>String(A_.id)===String(M.id));if(e&&!e.name)e.name=M.label});let{content:E0,usedIds:o_}=e_(Z_,_0);Z_=E0;let T_=B0.filter(({id:M})=>!o_.has(M)),M0=x_.filter((M)=>!o_.has(M)),c_=b_.length>0?b_.map((M,e)=>({id:M.id||`attachment-${e+1}`,label:M.label||`attachment-${e+1}`})):_0.map((M,e)=>({id:M.id,label:M.name||`attachment-${e+1}`})),d_=i_(()=>n3(X_),[X_]),k0=i_(()=>o3(X_),[X_]);u(()=>{if(!A.current)return;return F2(A.current),F9(A.current)},[v_]);let B_=y(null);return u(()=>{if(!B_.current||d_.length===0)return;let M=B_.current;M.innerHTML="";for(let e of d_){let A_=document.createElement("div");M.appendChild(A_),r4(A_,e,{onAction:async(R_)=>{if(R_.type==="Action.OpenUrl"){let b0=T2(R_.url||"");if(!b0)throw Error("Invalid URL");window.open(b0,"_blank","noopener,noreferrer");return}if(R_.type==="Action.Submit"){await y3({post_id:_.id,thread_id:C.thread_id||_.id,chat_jid:_.chat_jid||null,card_id:e.card_id,action:{type:R_.type,title:R_.title||"",data:R_.data}});return}console.warn("[post] unsupported adaptive card action:",R_.type,R_)}}).catch((R_)=>{console.error("[post] adaptive card render error:",R_),A_.textContent=e.fallback_text||"Card failed to render."})}},[d_,C.thread_id,_.id]),U`
        ...
        ${C_&&U`
            ...
            />
        `}
    `}function j8({posts:_,hasMore:$,onLoadMore:j,onPostClick:Z,onHashtagClick:N,onMessageRef:K,onScrollToMessage:z,onFileRef:G,emptyMessage:X,timelineRef:O,agents:q,user:V,onDeletePost:H,reverse:b=!0,removingPostIds:f,searchQuery:T}){let[v,x]=I(!1),F=y(null),A=typeof IntersectionObserver<"u",C=P(async()=>{if(!j||!$||v)return;x(!0);try{await j({preserveScroll:!0,preserveMode:"top"})}finally{x(!1)}},[$,v,j]),c=P((s)=>{let{scrollTop:__,scrollHeight:K_,clientHeight:j_}=s.target,G_=b?K_-j_-__:__,X_=Math.max(300,j_);if(G_<X_)C()},[b,C]);u(()=>{if(!A)return;let s=F.current,__=O?.current;if(!s||!__)return;let K_=300,j_=new IntersectionObserver((G_)=>{for(let X_ of G_){if(!X_.isIntersecting)continue;C()}},{root:__,rootMargin:`${K_}px 0px ${K_}px 0px`,threshold:0});return j_.observe(s),()=>j_.disconnect()},[A,$,j,O,C]);let a=y(C);if(a.current=C,u(()=>{if(A)return;if(!O?.current)return;let{scrollTop:s,scrollHeight:__,clientHeight:K_}=O.current,j_=b?__-K_-s:s,G_=Math.max(300,K_);if(j_<G_)a.current?.()},[A,_,$,b,O]),u(()=>{if(!O?.current)return;if(!$||v)return;let{scrollTop:s,scrollHeight:__,clientHeight:K_}=O.current,j_=b?__-K_-s:s,G_=Math.max(300,K_);if(__<=K_+1||j_<G_)a.current?.()},[_,$,v,b,O]),!_)return U`<div class="loading"><div class="spinner"></div></div>`;if(_.length===0)return U`
        ...
        `;let t=_.slice().sort((s,__)=>s.id-__.id),l=(s)=>{let __=s?.data?.thread_id;if(__===null||__===void 0||__==="")return null;let K_=Number(__);return Number.isFinite(K_)?K_:null},N_=new Map;for(let s=0;s<t.length;s+=1){let __=t[s],K_=Number(__?.id),j_=l(__);if(j_!==null){let G_=N_.get(j_)||{anchorIndex:-1,replyIndexes:[]};G_.replyIndexes.push(s),N_.set(j_,G_)}else if(Number.isFinite(K_)){let G_=N_.get(K_)||{anchorIndex:-1,replyIndexes:[]};G_.anchorIndex=s,N_.set(K_,G_)}}let i=new Map;for(let[s,__]of N_.entries()){let K_=new Set;if(__.anchorIndex>=0)K_.add(__.anchorIndex);for(let j_ of __.replyIndexes)K_.add(j_);i.set(s,Array.from(K_).sort((j_,G_)=>j_-G_))}let Q_=t.map((s,__)=>{let K_=l(s);if(K_===null)return{hasThreadPrev:!1,hasThreadNext:!1};let j_=i.get(K_);if(!j_||j_.length===0)return{hasThreadPrev:!1,hasThreadNext:!1};let G_=j_.indexOf(__);if(G_<0)return{hasThreadPrev:!1,hasThreadNext:!1};return{hasThreadPrev:G_>0,hasThreadNext:G_<j_.length-1}}),U_=U`<div class="timeline-sentinel" ref=${F}></div>`;return U`
        <div class="timeline ${b?"reverse":"normal"}" ref=${O} onScroll=${c}>
            <div class="timeline-content">
                ${b?U_:null}
                ${t.map((s,__)=>{let K_=Boolean(s.data?.thread_id&&s.data.thread_id!==s.id),j_=f?.has?.(s.id),G_=Q_[__]||{};return U`
                    <${$8}
                        key=${s.id}
                        post=${s}
                        isThreadReply=${K_}
                        ...
                        isRemoving=${j_}
                        highlightQuery=${T}
                        agentName=${m4(s.data?.agent_id,q||{})}
                        agentAvatarUrl=${u4(s.data?.agent_id,q||{})}
...
`),this.setStatus("Unavailable")}}sendResize(){if(!this.socket||this.socket.readyState!==WebSocket.OPEN||!this.fitAddon?.proposeDimensions)return;let _=this.fitAddon.proposeDimensions();if(!_)return;this.socket.send(JSON.stringify({type:"resize",cols:_.cols,rows:_.rows}))}getContent(){return}isDirty(){return!1}focus(){if(this.terminal?.focus){this.terminal.focus();return}this.termEl?.focus()}resize(){this.syncHostLayout();try{this.terminal?.renderer?.remeasureFont?.()}catch{}try{this.fitAddon?.fit?.()}catch{}try{this.terminal?.refresh?.()}catch{}this.syncHostLayout(),this.sendResize()}dispose(){if(this.disposed)return;this.disposed=!0;try{if(this.resizeFrame)cancelAnimationFrame(this.resizeFrame),this.resizeFrame=0}catch{}try{if(this.themeChangeListener)window.removeEventListener("piclaw-theme-change",this.themeChangeListener)}catch{}try{if(this.mediaQuery&&this.mediaQueryListener){if(this.mediaQuery.removeEventListener)this.mediaQuery.removeEventListener("change",this.mediaQueryListener);else if(this.mediaQuery.removeListener)this.mediaQuery.removeListener(this.mediaQueryListener)}}catch{}try{if(this.dockResizeListener)window.removeEventListener("dock-resize",this.dockResizeListener);if(this.windowResizeListener)window.removeEventListener("resize",this.windowResizeListener)}catch{}try{this.themeObserver?.disconnect?.()}catch{}try{this.resizeObserver?.disconnect?.()}catch{}try{this.socket?.close?.()}catch{}try{this.fitAddon?.dispose?.()}catch{}try{this.terminal?.dispose?.()}catch{}this.termEl?.remove()}}var t3={id:"terminal",label:"Terminal",icon:"terminal",capabilities:["terminal"],placement:"dock",mount(_,$){return new Y8(_,$)}};function k2(_){return String(_||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function T9(_,$){let j=String(_||"").trim();if(!j)return j;if(/^[a-zA-Z][a-zA-Z\d+.-]*:/.test(j)||j.startsWith("#")||j.startsWith("data:")||j.startsWith("blob:"))return j;let Z=j.match(/^([^?#]*)(\?[^#]*)?(#.*)?$/),N=Z?.[1]||j,K=Z?.[2]||"",z=Z?.[3]||"",G=String($||"").split("/").slice(0,-1).join("/"),O=N.startsWith("/")?N:`${G?`${G}/`:""}${N}`,q=[];for(let H of O.split("/")){if(!H||H===".")continue;if(H===".."){if(q.length>0)q.pop();continue}q.push(H)}let V=q.join("/");return`${M$(V)}${K}${z}`}function Y$(_){return _?.preview||null}function I9(_){let $=String(_||""),j=Math.max($.lastIndexOf("/"),$.lastIndexOf("\\")),Z=j>=0?$.slice(j+1):$,N=Z.lastIndexOf(".");if(N<=0||N===Z.length-1)return"none";return Z.slice(N+1)}function m9(_){if(!_)return"unknown";if(_.kind==="image")return"image";if(_.kind==="text")return _.content_type==="text/markdown"?"markdown":"text";if(_.kind==="binary")return"binary";return String(_.kind||"unknown")}function u9(_,$){let j=$?.path||_?.path||"",Z=[];if($?.content_type)Z.push(`<span><strong>type:</strong> ${k2($.content_type)}</span>`);if(typeof $?.size==="number")Z.push(`<span><strong>size:</strong> ${k2(r0($.size))}</span>`);if($?.mtime)Z.push(`<span><strong>modified:</strong> ${k2(I2($.mtime))}</span>`);if(Z.push(`<span><strong>kind:</strong> ${k2(m9($))}</span>`),Z.push(`<span><strong>extension:</strong> ${k2(I9(j))}</span>`),j)Z.push(`<span><strong>path:</strong> ${k2(j)}</span>`);if($?.truncated)Z.push("<span><strong>content:</strong> truncated</span>");return`<div class="workspace-preview-meta workspace-preview-meta-inline">${Z.join("")}</div>`}function v9(_){let $=Y$(_);if(!$)return'<div class="workspace-preview-text">No preview available.</div>';let j=u9(_,$);if($.kind==="image"){let Z=$.url||($.path?M$($.path):"");return`${j}
    ...
    `}function d9({mediaId:_}){let[$,j]=I(null);if(u(()=>{if(!_)return;c2(_).then(j).catch(()=>{})},[_]),!$)return null;let Z=$.filename||"file",N=$.metadata?.size?r0($.metadata.size):"";return U`
    ...
    `}function E8({onFileSelect:_,visible:$=!0,active:j=void 0,onOpenEditor:Z}){let[N,K]=I(null),[z,G]=I(new Set(["."])),[X,O]=I(null),[q,V]=I(null),[H,b]=I(""),[f,T]=I(null),[v,x]=I(null),[F,A]=I(!0),[C,c]=I(!1),[a,t]=I(null),[l,N_]=I(()=>p2("workspaceShowHidden",!1)),[i,Q_]=I(!1),[U_,s]=I(null),[__,K_]=I(null),[j_,G_]=I(null),[X_,J_]=I(!1),[Z_,D_]=I(null),[k_,r_]=I(()=>W8()),[E_,u_]=I(()=>Z1({stored:s0(l$),...j1()})),b_=y(z),l_=y(""),p=y(null),L_=y(0),W_=y(new Set),V_=y(null),y_=y(new Map),w_=y(_),v_=y(Z),F_=y(null),C_=y(null),S_=y(null),Y0=y(null),n_=y(null),e_=y(null),B0=y("."),x_=y(null),_0=y({path:null,dragging:!1,startX:0,startY:0}),Q0=y({path:null,dragging:!1,startX:0,startY:0}),D0=y({path:null,timer:0}),F0=y(!1),G0=y(0),E0=y(new Map),o_=y(null),T_=y(null),M0=y(null),c_=y(null),d_=y(l),k0=y($),B_=y(j??$),M=y(0),e=y(j_),A_=y(i),R_=y(U_),b0=y(null),a0=y({x:0,y:0}),$0=y(0),O0=y(null),W0=y(X),t0=y(q),C0=y(null),R0=y(null),g0=y(f);w_.current=_,v_.current=Z,u(()=>{b_.current=z},[z]),u(()=>{d_.current=l},[l]),u(()=>{k0.current=$},[$]),u(()=>{B_.current=j??$},[j,$]),u(()=>{e.current=j_},[j_]),u(()=>{if(typeof window>"u")return;let W=()=>{u_(Z1({stored:s0(l$),...j1()}))};W();let Q=()=>W(),E=()=>W(),D=(g)=>{if(!g||g.key===null||g.key===l$)W()};window.addEventListener("resize",Q),window.addEventListener("focus",E),window.addEventListener("storage",D);let m=window.matchMedia?.("(pointer: coarse)"),d=window.matchMedia?.("(hover: none)"),r=(g,z_)=>{if(!g)return;if(g.addEventListener)g.addEventListener("change",z_);else if(g.addListener)g.addListener(z_)},o=(g,z_)=>{if(!g)return;if(g.removeEventListener)g.removeEventListener("change",z_);else if(g.removeListener)g.removeListener(z_)};return r(m,Q),r(d,Q),()=>{window.removeEventListener("resize",Q),window.removeEventListener("focus",E),window.removeEventListener("storage",D),o(m,Q),o(d,Q)}},[]),u(()=>{let W=(Q)=>{let E=Q?.detail?.path;if(!E)return;let D=E.split("/"),m=[];for(let d=1;d<D.length;d++)m.push(D.slice(0,d).join("/"));if(m.length)G((d)=>{let r=new Set(d);r.add(".");for(let o of m)r.add(o);return r});O(E),requestAnimationFrame(()=>{let d=document.querySelector(`[data-path="${CSS.escape(E)}"]`);if(d)d.scrollIntoView({block:"nearest",behavior:"smooth"})})};return window.addEventListener("workspace-reveal-path",W),()=>window.removeEventListener("workspace-reveal-path",W)},[]),u(()=>{A_.current=i},[i]),u(()=>{R_.current=U_},[U_]),u(()=>{W0.current=X},[X]),u(()=>{t0.current=q},[q]),u(()=>{g0.current=f},[f]),u(()=>{if(typeof window>"u"||typeof document>"u")return;let W=()=>r_(W8());W();let Q=window.matchMedia?.("(prefers-color-scheme: dark)"),E=()=>W();if(Q?.addEventListener)Q.addEventListener("change",E);else if(Q?.addListener)Q.addListener(E);let D=typeof MutationObserver<"u"?new MutationObserver(()=>W()):null;if(D?.observe(document.documentElement,{attributes:!0,attributeFilter:["class","data-theme"]}),document.body)D?.observe(document.body,{attributes:!0,attributeFilter:["class","data-theme"]});return()=>{if(Q?.removeEventListener)Q.removeEventListener("change",E);else if(Q?.removeListener)Q.removeListener(E);D?.disconnect()}},[]),u(()=>{if(!q)return;let W=n_.current;if(!W)return;let Q=requestAnimationFrame(()=>{try{W.focus(),W.select()}catch{}});return()=>cancelAnimationFrame(Q)},[q]);let n2=async(W)=>{c(!0),T(null),x(null);try{let Q=await P3(W,20000);T(Q)}catch(Q){T({error:Q.message||"Failed to load preview"})}finally{c(!1)}};F_.current=n2;let b2=async()=>{if(!k0.current)return;try{let W=await j$("",1,d_.current),Q=X8(W.root,b_.current,d_.current);if(Q===l_.current){A(!1);return}if(l_.current=Q,p.current=W.root,!L_.current)L_.current=requestAnimationFrame(()=>{L_.current=0,K((E)=>Y1(E,p.current)),A(!1)})}catch(W){t(W.message||"Failed to load workspace"),A(!1)}},c0=async(W)=>{if(!W)return;if(W_.current.has(W))return;W_.current.add(W);try{let Q=await j$(W,1,d_.current);K((E)=>K1(E,W,Q.root))}catch(Q){t(Q.message||"Failed to load workspace")}finally{W_.current.delete(W)}};C_.current=c0;let j0=P(()=>{let W=X;if(!W)return".";let Q=y_.current?.get(W);if(Q&&Q.type==="dir")return Q.path;if(W==="."||!W.includes("/"))return".";let E=W.split("/");return E.pop(),E.join("/")||"."},[X]),K2=P((W)=>{let Q=W?.closest?.(".workspace-row");if(!Q)return null;let E=Q.dataset.path,D=Q.dataset.type;if(!E)return null;if(D==="dir")return E;if(E.includes("/")){let m=E.split("/");return m.pop(),m.join("/")||"."}return"."},[]),L0=P((W)=>{return K2(W?.target||null)},[K2]),Z0=P((W)=>{e.current=W,G_(W)},[]),I_=P(()=>{let W=D0.current;if(W?.timer)clearTimeout(W.timer);D0.current={path:null,timer:0}},[]),I0=P((W)=>{if(!W||W==="."){I_();return}let Q=y_.current?.get(W);if(!Q||Q.type!=="dir"){I_();return}if(b_.current?.has(W)){I_();return}if(D0.current?.path===W)return;I_();let E=setTimeout(()=>{D0.current={path:null,timer:0},C_.current?.(W),G((D)=>{let m=new Set(D);return m.add(W),m})},600);D0.current={path:W,timer:E}},[I_]),p0=P((W,Q)=>{if(a0.current={x:W,y:Q},$0.current)return;$0.current=requestAnimationFrame(()=>{$0.current=0;let E=b0.current;if(!E)return;let D=a0.current;E.style.transform=`translate(${D.x+12}px, ${D.y+12}px)`})},[]),z2=P((W)=>{if(!W)return;let E=(y_.current?.get(W)?.name||W.split("/").pop()||W).trim();if(!E)return;K_({path:W,label:E})},[]),J2=P(()=>{if(K_(null),$0.current)cancelAnimationFrame($0.current),$0.current=0;if(b0.current)b0.current.style.transform="translate(-9999px, -9999px)"},[]),w2=P((W)=>{if(!W)return".";let Q=y_.current?.get(W);if(Q&&Q.type==="dir")return Q.path;if(W==="."||!W.includes("/"))return".";let E=W.split("/");return E.pop(),E.join("/")||"."},[]),H0=P(()=>{V(null),b("")},[]),A2=P((W)=>{if(!W)return;let E=(y_.current?.get(W)?.name||W.split("/").pop()||W).trim();if(!E||W===".")return;V(W),b(E)},[]),o2=P(async()=>{let W=t0.current;if(!W)return;let Q=(H||"").trim();if(!Q){H0();return}let E=y_.current?.get(W),D=(E?.name||W.split("/").pop()||W).trim();if(Q===D){H0();return}try{let d=(await x3(W,Q))?.path||W,r=W.includes("/")?W.split("/").slice(0,-1).join("/")||".":".";if(H0(),t(null),window.dispatchEvent(new CustomEvent("workspace-file-renamed",{detail:{oldPath:W,newPath:d,type:E?.type||"file"}})),E?.type==="dir")G((o)=>{let g=new Set;for(let z_ of o)if(z_===W)g.add(d);else if(z_.startsWith(`${W}/`))g.add(`${d}${z_.slice(W.length)}`);else g.add(z_);return g});if(O(d),E?.type==="dir")T(null),c(!1),x(null);else F_.current?.(d);C_.current?.(r)}catch(m){t(m?.message||"Failed to rename file")}},[H0,H]),y2=P(async(W)=>{let D=W||".";for(let m=0;m<50;m+=1){let r=`untitled${m===0?"":`-${m}`}.md`;try{let g=(await C3(D,r,""))?.path||(D==="."?r:`${D}/${r}`);if(D&&D!==".")G((z_)=>new Set([...z_,D]));O(g),t(null),C_.current?.(D),F_.current?.(g);return}catch(o){if(o?.status===409||o?.code==="file_exists")continue;t(o?.message||"Failed to create file");return}}t("Failed to create file (untitled name already in use).")},[]),m2=P((W)=>{if(W?.stopPropagation?.(),X_)return;let Q=w2(W0.current);y2(Q)},[X_,w2,y2]);u(()=>{if(typeof window>"u")return;let W=(Q)=>{let E=Q?.detail?.updates||[];if(!Array.isArray(E)||E.length===0)return;K((o)=>{let g=o;for(let z_ of E){if(!z_?.root)continue;if(!g||z_.path==="."||!z_.path)g=z_.root;else g=K1(g,z_.path,z_.root)}if(g)l_.current=X8(g,b_.current,d_.current);return A(!1),g});let D=W0.current;if(Boolean(D)&&E.some((o)=>{let g=o?.path||"";if(!g||g===".")return!0;return D===g||D.startsWith(`${g}/`)||g.startsWith(`${D}/`)}))E0.current.clear();if(!D||!g0.current)return;let d=y_.current?.get(D);if(d&&d.type==="dir")return;if(E.some((o)=>{let g=o?.path||"";if(!g||g===".")return!0;return D===g||D.startsWith(`${g}/`)}))F_.current?.(D)};return window.addEventListener("workspace-update",W),()=>window.removeEventListener("workspace-update",W)},[]),V_.current=b2;let q2=y(()=>{if(typeof window>"u")return;let W=window.matchMedia("(min-width: 1024px) and (orientation: landscape)"),Q=B_.current??k0.current,E=document.visibilityState!=="hidden"&&(Q||W.matches&&k0.current);Z$(E,d_.current).catch(()=>{})}).current,a_=y(0),P2=y(()=>{if(a_.current)clearTimeout(a_.current);a_.current=setTimeout(()=>{a_.current=0,q2()},250)}).current;u(()=>{if(k0.current)V_.current?.();P2()},[$,j]),u(()=>{V_.current(),q2();let W=setInterval(()=>V_.current(),i9),Q=h2("previewHeight",null),E=Number.isFinite(Q)?Math.min(Math.max(Q,80),600):280;if(G0.current=E,S_.current)S_.current.style.setProperty("--preview-height",`${E}px`);let D=window.matchMedia("(min-width: 1024px) and (orientation: landscape)"),m=()=>P2();if(D.addEventListener)D.addEventListener("change",m);else if(D.addListener)D.addListener(m);return document.addEventListener("visibilitychange",m),()=>{if(clearInterval(W),L_.current)cancelAnimationFrame(L_.current),L_.current=0;if(D.removeEventListener)D.removeEventListener("change",m);else if(D.removeListener)D.removeListener(m);if(document.removeEventListener("visibilitychange",m),a_.current)clearTimeout(a_.current),a_.current=0;if(x_.current)clearTimeout(x_.current),x_.current=null;Z$(!1,d_.current).catch(()=>{})}},[]);let x0=i_(()=>U8(N,z,l),[N,z,l]),S0=i_(()=>new Map(x0.map((W)=>[W.node.path,W.node])),[x0]),Y2=i_(()=>L8(E_),[E_]);y_.current=S0;let U0=(X?y_.current.get(X):null)?.type==="dir";u(()=>{if(!X||!U0){D_(null),o_.current=null,T_.current=null;return}let W=X,Q=`${l?"hidden":"visible"}:${X}`,E=E0.current,D=E.get(Q);if(D?.root){E.delete(Q),E.set(Q,D);let r=V8(D.root,Boolean(D.truncated),k_);if(r)o_.current=r,T_.current=X,D_({loading:!1,error:null,payload:r});return}let m=o_.current,d=T_.current;D_({loading:!0,error:null,payload:d===X?m:null}),j$(X,l9,l).then((r)=>{if(W0.current!==W)return;let o={root:r?.root,truncated:Boolean(r?.truncated)};E.delete(Q),E.set(Q,o);while(E.size>n9){let z_=E.keys().next().value;if(!z_)break;E.delete(z_)}let g=V8(o.root,o.truncated,k_);o_.current=g,T_.current=X,D_({loading:!1,error:null,payload:g})}).catch((r)=>{if(W0.current!==W)return;D_({loading:!1,error:r?.message||"Failed to load folder size chart",payload:d===X?m:null})})},[X,U0,l,k_]);let M2=Boolean(f&&f.kind==="text"&&!U0&&(!f.size||f.size<=262144)),G2=M2?"Open in editor":f?.size>262144?"File too large to edit":"File is not editable";u(()=>{let W=M0.current;if(c_.current)c_.current.dispose(),c_.current=null;if(!W)return;if(W.innerHTML="",!X||U0||!f||f.error)return;let Q={path:X,content:typeof f.text==="string"?f.text:void 0,mtime:f.mtime,size:f.size,preview:f,mode:"view"},E=X0.resolve(Q)||X0.get("workspace-preview-default");if(!E)return;let D=E.mount(W,Q);return c_.current=D,()=>{if(c_.current===D)D.dispose(),c_.current=null;W.innerHTML=""}},[X,U0,f]);let O2=(W)=>{let Q=W?.target;if(Q instanceof Element)return Q;return Q?.parentElement||null},C2=(W)=>{return Boolean(W?.closest?.(".workspace-node-icon, .workspace-label-text"))},h0=y((W)=>{if(R0.current)clearTimeout(R0.current),R0.current=null;let E=O2(W)?.closest?.("[data-path]");if(!E)return;let D=E.dataset.path;if(E.dataset.type==="dir"||!D)return;if(t0.current===D)H0();v_.current?.(D)}).current,e0=y((W)=>{if(F0.current){F0.current=!1;return}let Q=O2(W),E=Q?.closest?.("[data-path]");if(Y0.current?.focus?.(),!E)return;let D=E.dataset.path,m=E.dataset.type,d=Boolean(Q?.closest?.(".workspace-caret")),r=Boolean(Q?.closest?.("button"))||Boolean(Q?.closest?.("a"))||Boolean(Q?.closest?.("input")),o=W0.current===D,g=t0.current;if(g){if(g===D)return;H0()}let z_=m==="file"&&C0.current===D&&!d&&!r;if(o&&!d&&!r&&D!=="."&&!z_){if(R0.current)clearTimeout(R0.current);R0.current=setTimeout(()=>{R0.current=null,A2(D)},350);return}if(m==="dir"){if(C0.current=null,O(D),T(null),x(null),c(!1),!b_.current.has(D))C_.current?.(D);if(o&&!d)return;G((V0)=>{let A0=new Set(V0);if(A0.has(D))A0.delete(D);else A0.add(D);return A0})}else{C0.current=null,O(D);let y0=y_.current.get(D);if(y0)w_.current?.(y0.path,y0);F_.current?.(D)}}).current,f0=y(()=>{l_.current="",V_.current(),Array.from(b_.current||[]).filter((Q)=>Q&&Q!==".").forEach((Q)=>C_.current?.(Q))}).current,_2=y(()=>{C0.current=null,O(null),T(null),x(null),c(!1)}).current,x2=y(()=>{N_((W)=>{let Q=!W;if(typeof window<"u")s_("workspaceShowHidden",String(Q));return d_.current=Q,Z$(!0,Q).catch(()=>{}),l_.current="",V_.current?.(),Array.from(b_.current||[]).filter((D)=>D&&D!==".").forEach((D)=>C_.current?.(D)),Q})}).current,S2=y((W)=>{if(O2(W)?.closest?.("[data-path]"))return;_2()}).current,$2=P(async(W)=>{if(!W)return;let Q=W.split("/").pop()||W;if(!window.confirm(`Delete "${Q}"? This cannot be undone.`))return;try{await f3(W);let D=W.includes("/")?W.split("/").slice(0,-1).join("/")||".":".";if(W0.current===W)_2();C_.current?.(D),t(null)}catch(D){T((m)=>({...m||{},error:D.message||"Failed to delete file"}))}},[_2]),U2=P((W)=>{let Q=Y0.current;if(!Q||!W||typeof CSS>"u"||typeof CSS.escape!=="function")return;Q.querySelector(`[data-path="${CSS.escape(W)}"]`)?.scrollIntoView?.({block:"nearest"})},[]),d2=P((W)=>{let Q=x0;if(!Q||Q.length===0)return;let E=X?Q.findIndex((D)=>D.node.path===X):-1;if(W.key==="ArrowDown"){W.preventDefault();let D=Math.min(E+1,Q.length-1),m=Q[D];if(!m)return;if(O(m.node.path),m.node.type!=="dir")w_.current?.(m.node.path,m.node),F_.current?.(m.node.path);else T(null),c(!1),x(null);U2(m.node.path);return}if(W.key==="ArrowUp"){W.preventDefault();let D=E<=0?0:E-1,m=Q[D];if(!m)return;if(O(m.node.path),m.node.type!=="dir")w_.current?.(m.node.path,m.node),F_.current?.(m.node.path);else T(null),c(!1),x(null);U2(m.node.path);return}if(W.key==="ArrowRight"&&E>=0){let D=Q[E];if(D?.node?.type==="dir"&&!z.has(D.node.path))W.preventDefault(),C_.current?.(D.node.path),G((m)=>new Set([...m,D.node.path]));return}if(W.key==="ArrowLeft"&&E>=0){let D=Q[E];if(D?.node?.type==="dir"&&z.has(D.node.path))W.preventDefault(),G((m)=>{let d=new Set(m);return d.delete(D.node.path),d});return}if(W.key==="Enter"&&E>=0){W.preventDefault();let D=Q[E];if(!D)return;let m=D.node.path;if(D.node.type==="dir"){if(!b_.current.has(m))C_.current?.(m);G((r)=>{let o=new Set(r);if(o.has(m))o.delete(m);else o.add(m);return o}),T(null),x(null),c(!1)}else w_.current?.(m,D.node),F_.current?.(m);return}if((W.key==="Delete"||W.key==="Backspace")&&E>=0){let D=Q[E];if(!D||D.node.type==="dir")return;W.preventDefault(),$2(D.node.path);return}if(W.key==="Escape")W.preventDefault(),_2()},[_2,$2,z,x0,U2,X]),w0=P((W)=>{let Q=O2(W),E=Q?.closest?.(".workspace-row");if(!E)return;let D=E.dataset.type,m=E.dataset.path;if(!m||m===".")return;if(t0.current===m)return;let d=W?.touches?.[0];if(!d)return;if(_0.current={path:C2(Q)?m:null,dragging:!1,startX:d.clientX,startY:d.clientY},D!=="file")return;if(x_.current)clearTimeout(x_.current);x_.current=setTimeout(()=>{if(x_.current=null,_0.current?.dragging)return;$2(m)},600)},[$2]),B=P(()=>{if(x_.current)clearTimeout(x_.current),x_.current=null;let W=_0.current;if(W?.dragging&&W.path){let Q=e.current||j0(),E=O0.current;if(typeof E==="function")E(W.path,Q)}_0.current={path:null,dragging:!1,startX:0,startY:0},M.current=0,Q_(!1),s(null),Z0(null),I_(),J2()},[j0,J2,Z0,I_]),S=P((W)=>{let Q=_0.current,E=W?.touches?.[0];if(!E||!Q?.path){if(x_.current)clearTimeout(x_.current),x_.current=null;return}let D=Math.abs(E.clientX-Q.startX),m=Math.abs(E.clientY-Q.startY),d=D>8||m>8;if(d&&x_.current)clearTimeout(x_.current),x_.current=null;if(!Q.dragging&&d)Q.dragging=!0,Q_(!0),s("move"),z2(Q.path);if(Q.dragging){W.preventDefault(),p0(E.clientX,E.clientY);let r=document.elementFromPoint(E.clientX,E.clientY),o=K2(r)||j0();if(e.current!==o)Z0(o);I0(o)}},[K2,j0,z2,p0,Z0,I0]),n=y((W)=>{W.preventDefault();let Q=S_.current;if(!Q)return;let E=W.clientY,D=G0.current||280,m=W.currentTarget;m.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let d=E,r=(g)=>{d=g.clientY;let z_=Q.clientHeight-80,y0=Math.min(Math.max(D-(g.clientY-E),80),z_);Q.style.setProperty("--preview-height",`${y0}px`),G0.current=y0},o=()=>{let g=Q.clientHeight-80,z_=Math.min(Math.max(D-(d-E),80),g);G0.current=z_,m.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",s_("previewHeight",String(Math.round(z_))),document.removeEventListener("mousemove",r),document.removeEventListener("mouseup",o)};document.addEventListener("mousemove",r),document.addEventListener("mouseup",o)}).current,Y_=y((W)=>{W.preventDefault();let Q=S_.current;if(!Q)return;let E=W.touches[0];if(!E)return;let D=E.clientY,m=G0.current||280,d=W.currentTarget;d.classList.add("dragging"),document.body.style.userSelect="none";let r=(g)=>{let z_=g.touches[0];if(!z_)return;g.preventDefault();let y0=Q.clientHeight-80,V0=Math.min(Math.max(m-(z_.clientY-D),80),y0);Q.style.setProperty("--preview-height",`${V0}px`),G0.current=V0},o=()=>{d.classList.remove("dragging"),document.body.style.userSelect="",s_("previewHeight",String(Math.round(G0.current||m))),document.removeEventListener("touchmove",r),document.removeEventListener("touchend",o),document.removeEventListener("touchcancel",o)};document.addEventListener("touchmove",r,{passive:!1}),document.addEventListener("touchend",o),document.addEventListener("touchcancel",o)}).current,P_=async()=>{if(!X)return;try{let W=await M3(X);if(W.media_id)x(W.media_id)}catch(W){T((Q)=>({...Q||{},error:W.message||"Failed to attach"}))}},t_=async()=>{if(!X||U0)return;await $2(X)},M_=(W)=>{return Array.from(W?.dataTransfer?.types||[]).includes("Files")},N0=P((W)=>{if(!M_(W))return;if(W.preventDefault(),M.current+=1,!A_.current)Q_(!0);s("upload");let Q=L0(W)||j0();Z0(Q),I0(Q)},[j0,L0,Z0,I0]),i0=P((W)=>{if(!M_(W))return;if(W.preventDefault(),W.dataTransfer)W.dataTransfer.dropEffect="copy";if(!A_.current)Q_(!0);if(R_.current!=="upload")s("upload");let Q=L0(W)||j0();if(e.current!==Q)Z0(Q);I0(Q)},[j0,L0,Z0,I0]),D2=P((W)=>{if(!M_(W))return;if(W.preventDefault(),M.current=Math.max(0,M.current-1),M.current===0)Q_(!1),s(null),Z0(null),I_()},[Z0,I_]),m0=P(async(W,Q=".")=>{let E=Array.from(W||[]);if(E.length===0)return;let D=Q&&Q!==""?Q:".",m=D!=="."?D:"workspace root";J_(!0);try{let d=null;for(let r of E)try{d=await P$(r,D)}catch(o){let g=o?.status,z_=o?.code;if(g===409||z_==="file_exists"){let y0=r?.name||"file";if(!window.confirm(`"${y0}" already exists in ${m}. Overwrite?`))continue;d=await P$(r,D,{overwrite:!0})}else throw o}if(d?.path)C0.current=d.path,O(d.path),F_.current?.(d.path);C_.current?.(D)}catch(d){t(d.message||"Failed to upload file")}finally{J_(!1)}},[]),L2=P(async(W,Q)=>{if(!W)return;let E=y_.current?.get(W);if(!E)return;let D=Q&&Q!==""?Q:".",m=W.includes("/")?W.split("/").slice(0,-1).join("/")||".":".";if(D===m)return;try{let r=(await S3(W,D))?.path||W;if(E.type==="dir")G((o)=>{let g=new Set;for(let z_ of o)if(z_===W)g.add(r);else if(z_.startsWith(`${W}/`))g.add(`${r}${z_.slice(W.length)}`);else g.add(z_);return g});if(O(r),E.type==="dir")T(null),c(!1),x(null);else F_.current?.(r);C_.current?.(m),C_.current?.(D)}catch(d){t(d?.message||"Failed to move entry")}},[]);O0.current=L2;let X2=P(async(W)=>{if(!M_(W))return;W.preventDefault(),M.current=0,Q_(!1),s(null),G_(null),I_();let Q=Array.from(W?.dataTransfer?.files||[]);if(Q.length===0)return;let E=e.current||L0(W)||j0();await m0(Q,E)},[j0,L0,m0]),l0=P((W)=>{if(W?.stopPropagation?.(),X_)return;let Q=W?.currentTarget?.dataset?.uploadTarget||".";B0.current=Q,e_.current?.click()},[X_]),W2=P(()=>{if(X_)return;let W=W0.current,Q=W?y_.current?.get(W):null;B0.current=Q?.type==="dir"?Q.path:".",e_.current?.click()},[X_]),J0=P((W)=>{if(!W||W.button!==0)return;let Q=W.currentTarget;if(!Q||!Q.dataset)return;let E=Q.dataset.path;if(!E||E===".")return;if(t0.current===E)return;let D=O2(W);if(D?.closest?.("button, a, input, .workspace-caret"))return;if(!C2(D))return;W.preventDefault(),Q0.current={path:E,dragging:!1,startX:W.clientX,startY:W.clientY};let m=(r)=>{let o=Q0.current;if(!o?.path)return;let g=Math.abs(r.clientX-o.startX),z_=Math.abs(r.clientY-o.startY),y0=g>4||z_>4;if(!o.dragging&&y0)o.dragging=!0,F0.current=!0,Q_(!0),s("move"),z2(o.path),p0(r.clientX,r.clientY),document.body.style.userSelect="none",document.body.style.cursor="grabbing";if(o.dragging){r.preventDefault(),p0(r.clientX,r.clientY);let V0=document.elementFromPoint(r.clientX,r.clientY),A0=K2(V0)||j0();if(e.current!==A0)Z0(A0);I0(A0)}},d=()=>{document.removeEventListener("mousemove",m),document.removeEventListener("mouseup",d);let r=Q0.current;if(r?.dragging&&r.path){let o=e.current||j0(),g=O0.current;if(typeof g==="function")g(r.path,o)}Q0.current={path:null,dragging:!1,startX:0,startY:0},M.current=0,Q_(!1),s(null),Z0(null),I_(),J2(),document.body.style.userSelect="",document.body.style.cursor="",setTimeout(()=>{F0.current=!1},0)};document.addEventListener("mousemove",m),document.addEventListener("mouseup",d)},[K2,j0,z2,p0,J2,Z0,I0,I_]),f2=P(async(W)=>{let Q=Array.from(W?.target?.files||[]);if(Q.length===0)return;let E=B0.current||".";if(await m0(Q,E),B0.current=".",W?.target)W.target.value=""},[m0]);return U`
        <aside
            ...
            <div class="workspace-tree" onClick=${S2}>
                ...
                ${N&&U`
                    <div
                        ...
                        onTouchCancel=${B}
                    >
                        ${x0.map(({node:W,depth:Q})=>{let E=W.type==="dir",D=W.path===X,m=W.path===q,d=E&&z.has(W.path),r=j_&&W.path===j_,o=Array.isArray(W.children)&&W.children.length>0?W.children.length:Number(W.child_count)||0;return U`
                                <div
                                    key=${W.path}
                    ...
                    <div class="workspace-preview-header">
                        ...
                        <div class="workspace-preview-actions">
                            ...
                            ${U0?U`
                                    ...
                                    <a class="workspace-download" href=${T3(X,l)}
    ...
    `}function k8({tabs:_,activeId:$,onActivate:j,onClose:Z,onCloseOthers:N,onCloseAll:K,onTogglePin:z,onTogglePreview:G,previewTabs:X,onToggleDock:O,dockVisible:q}){let[V,H]=I(null),b=y(null);u(()=>{if(!V)return;let F=(A)=>{if(A.type==="keydown"&&A.key!=="Escape")return;H(null)};return document.addEventListener("click",F),document.addEventListener("keydown",F),()=>{document.removeEventListener("click",F),document.removeEventListener("keydown",F)}},[V]),u(()=>{let F=(A)=>{if(A.ctrlKey&&A.key==="Tab"){if(A.preventDefault(),!_.length)return;let C=_.findIndex((c)=>c.id===$);if(A.shiftKey){let c=_[(C-1+_.length)%_.length];j?.(c.id)}else{let c=_[(C+1)%_.length];j?.(c.id)}return}if((A.ctrlKey||A.metaKey)&&A.key==="w"){let C=document.querySelector(".editor-pane");if(C&&C.contains(document.activeElement)){if(A.preventDefault(),$)Z?.($)}}};return document.addEventListener("keydown",F),()=>document.removeEventListener("keydown",F)},[_,$,j,Z]);let f=P((F,A)=>{if(F.button===1){F.preventDefault(),Z?.(A);return}if(F.button===0)j?.(A)},[j,Z]),T=P((F,A)=>{F.preventDefault(),H({id:A,x:F.clientX,y:F.clientY})},[]),v=P((F)=>{F.preventDefault(),F.stopPropagation()},[]),x=P((F,A)=>{F.preventDefault(),F.stopPropagation(),Z?.(A)},[Z]);if(u(()=>{if(!$||!b.current)return;let F=b.current.querySelector(".tab-item.active");if(F)F.scrollIntoView({block:"nearest",inline:"nearest",behavior:"smooth"})},[$]),!_.length)return null;return U`
        ...
        `}
    `}var s9=400,O1=60,b8=220,L1="mdPreviewHeight";function r9(){try{let _=localStorage.getItem(L1),$=_?Number(_):NaN;return Number.isFinite($)&&$>=O1?$:b8}catch{return b8}}function w8({getContent:_,path:$,onClose:j}){let[Z,N]=I(""),[K,z]=I(r9),G=y(null),X=y(null),O=y(""),q=y(_);return q.current=_,u(()=>{let b=()=>{let T=q.current?.()||"";if(T===O.current)return;O.current=T;try{let v=v0(T,null,{sanitize:!1});N(v)}catch{N('<p style="color:var(--text-secondary)">Preview unavailable</p>')}};b();let f=setInterval(b,s9);return()=>clearInterval(f)},[]),u(()=>{if(G.current&&Z)F2(G.current).catch(()=>{})},[Z]),U`
        <div
            class="md-preview-splitter"
            onMouseDown=${(b)=>{b.preventDefault();let f=b.clientY,T=X.current?.offsetHeight||K,v=X.current?.parentElement,x=v?v.offsetHeight*0.7:500,F=b.currentTarget;F.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let A=(c)=>{let a=Math.min(Math.max(T-(c.clientY-f),O1),x);z(a)},C=()=>{F.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="";try{localStorage.setItem(L1,String(Math.round(X.current?.offsetHeight||K)))}catch{}document.removeEventListener("mousemove",A),document.removeEventListener("mouseup",C)};document.addEventListener("mousemove",A),document.addEventListener("mouseup",C)}}
            onTouchStart=${(b)=>{b.preventDefault();let f=b.touches[0];if(!f)return;let T=f.clientY,v=X.current?.offsetHeight||K,x=X.current?.parentElement,F=x?x.offsetHeight*0.7:500,A=b.currentTarget;A.classList.add("dragging"),document.body.style.userSelect="none";let C=(a)=>{let t=a.touches[0];if(!t)return;a.preventDefault();let l=Math.min(Math.max(v-(t.clientY-T),O1),F);z(l)},c=()=>{A.classList.remove("dragging"),document.body.style.userSelect="";try{localStorage.setItem(L1,String(Math.round(X.current?.offsetHeight||K)))}catch{}document.removeEventListener("touchmove",C),document.removeEventListener("touchend",c),document.removeEventListener("touchcancel",c)};document.addEventListener("touchmove",C,{passive:!1}),document.addEventListener("touchend",c),document.addEventListener("touchcancel",c)}}
        ></div>
    ...
    `}function A8({handleSseEvent:_,handleConnectionStatusChange:$,loadPosts:j,onWake:Z,chatJid:N}){let K=y(_);K.current=_;let z=y($);z.current=$;let G=y(j);G.current=j;let X=y(Z);X.current=Z,u(()=>{G.current();let O=new C$((V,H)=>K.current(V,H),(V)=>z.current(V),{chatJid:N});O.connect();let q=()=>{O.reconnectIfNeeded();let V=typeof document<"u"?document:null;if(!V||V.visibilityState==="visible")X.current?.()};return window.addEventListener("focus",q),document.addEventListener("visibilitychange",q),()=>{window.removeEventListener("focus",q),document.removeEventListener("visibilitychange",q),O.disconnect()}},[N])}function P8(){let[_,$]=I(!1),[j,Z]=I("default"),N=y(!1);u(()=>{let X=p2("notificationsEnabled",!1);if(N.current=X,$(X),typeof Notification<"u")Z(Notification.permission)},[]),u(()=>{N.current=_},[_]);let K=P(()=>{if(typeof Notification>"u")return Promise.resolve("denied");try{let X=Notification.requestPermission();if(X&&typeof X.then==="function")return X;return Promise.resolve(X)}catch{return Promise.resolve("default")}},[]),z=P(async()=>{if(typeof window>"u"||typeof Notification>"u")return;if(!window.isSecureContext){alert("Notifications require a secure context (HTTPS or installed app).");return}if(Notification.permission==="denied"){Z("denied"),alert("Browser notifications are blocked. Enable them in your browser settings.");return}if(Notification.permission==="default"){let O=await K();if(Z(O||"default"),O!=="granted"){N.current=!1,$(!1),s_("notificationsEnabled","false");return}}let X=!N.current;N.current=X,$(X),s_("notificationsEnabled",String(X))},[K]),G=P((X,O)=>{if(!N.current)return!1;if(typeof Notification>"u")return!1;if(Notification.permission!=="granted")return!1;try{let q=new Notification(X,{body:O});return q.onclick=()=>{try{window.focus()}catch{}},!0}catch{return!1}},[]);return{notificationsEnabled:_,notificationPermission:j,toggleNotifications:z,notify:G}}var G$=(_)=>{let $=new Set;return(_||[]).filter((j)=>{if(!j||$.has(j.id))return!1;return $.add(j.id),!0})};function M8({preserveTimelineScroll:_,preserveTimelineScrollTop:$,chatJid:j=null}){let[Z,N]=I(null),[K,z]=I(!1),G=y(!1),X=y(null),O=y(!1),q=y(null),V=y(null),H=y(0);u(()=>{G.current=K},[K]),u(()=>{V.current=Z},[Z]),u(()=>{H.current+=1,V.current=null,q.current=null,O.current=!1,G.current=!1,N(null),z(!1)},[j]);let b=P(async(v=null)=>{let x=H.current;try{if(v){let F=await V3(v,50,0,j);if(x!==H.current)return;N(F.posts),z(!1)}else{let F=await _$(10,null,j);if(x!==H.current)return;N(F.posts),z(F.has_more)}}catch(F){if(x!==H.current)return;console.error("Failed to load posts:",F)}},[j]),f=P(async()=>{let v=H.current;try{let x=await _$(10,null,j);if(v!==H.current)return;N((F)=>{if(!F||F.length===0)return x.posts;return G$([...x.posts,...F])}),z((F)=>F||x.has_more)}catch(x){if(v!==H.current)return;console.error("Failed to refresh timeline:",x)}},[j]),T=P(async(v={})=>{let x=H.current,F=V.current;if(!F||F.length===0)return;if(O.current)return;let{preserveScroll:A=!0,preserveMode:C="top",allowRepeat:c=!1}=v,a=(N_)=>{if(!A){N_();return}if(C==="top")$(N_);else _(N_)},l=F.slice().sort((N_,i)=>N_.id-i.id)[0]?.id;if(!Number.isFinite(l))return;if(!c&&q.current===l)return;O.current=!0,q.current=l;try{let N_=await _$(10,l,j);if(x!==H.current)return;if(N_.posts.length>0)a(()=>{N((i)=>G$([...N_.posts,...i||[]])),z(N_.has_more)});else z(!1)}catch(N_){if(x!==H.current)return;console.error("Failed to load more posts:",N_)}finally{if(x===H.current)O.current=!1}},[j,_,$]);return u(()=>{X.current=T},[T]),{posts:Z,setPosts:N,hasMore:K,setHasMore:z,hasMoreRef:G,loadPosts:b,refreshTimeline:f,loadMore:T,loadMoreRef:X,loadingMoreRef:O,lastBeforeIdRef:q}}function C8(){let[_,$]=I(null),[j,Z]=I({text:"",totalLines:0}),[N,K]=I(""),[z,G]=I({text:"",totalLines:0}),[X,O]=I(null),[q,V]=I(null),[H,b]=I(null),f=y(null),T=y(0),v=y(!1),x=y(""),F=y(""),A=y(null),C=y(null),c=y(null),a=y(null),t=y(!1),l=y(!1);return{agentStatus:_,setAgentStatus:$,agentDraft:j,setAgentDraft:Z,agentPlan:N,setAgentPlan:K,agentThought:z,setAgentThought:G,pendingRequest:X,setPendingRequest:O,currentTurnId:q,setCurrentTurnId:V,steerQueuedTurnId:H,setSteerQueuedTurnId:b,lastAgentEventRef:f,lastSilenceNoticeRef:T,isAgentRunningRef:v,draftBufferRef:x,thoughtBufferRef:F,pendingRequestRef:A,stalledPostIdRef:C,currentTurnIdRef:c,steerQueuedTurnIdRef:a,thoughtExpandedRef:t,draftExpandedRef:l}}function x8({appShellRef:_,sidebarWidthRef:$,editorWidthRef:j,dockHeightRef:Z}){let N=y((q)=>{q.preventDefault();let V=_.current;if(!V)return;let H=q.clientX,b=$.current||280,f=q.currentTarget;f.classList.add("dragging"),V.classList.add("sidebar-resizing"),document.body.style.cursor="col-resize",document.body.style.userSelect="none";let T=H,v=(F)=>{T=F.clientX;let A=Math.min(Math.max(b+(F.clientX-H),160),600);V.style.setProperty("--sidebar-width",`${A}px`),$.current=A},x=()=>{let F=Math.min(Math.max(b+(T-H),160),600);$.current=F,f.classList.remove("dragging"),V.classList.remove("sidebar-resizing"),document.body.style.cursor="",document.body.style.userSelect="",s_("sidebarWidth",String(Math.round(F))),document.removeEventListener("mousemove",v),document.removeEventListener("mouseup",x)};document.addEventListener("mousemove",v),document.addEventListener("mouseup",x)}).current,K=y((q)=>{q.preventDefault();let V=_.current;if(!V)return;let H=q.touches[0];if(!H)return;let b=H.clientX,f=$.current||280,T=q.currentTarget;T.classList.add("dragging"),V.classList.add("sidebar-resizing"),document.body.style.userSelect="none";let v=(F)=>{let A=F.touches[0];if(!A)return;F.preventDefault();let C=Math.min(Math.max(f+(A.clientX-b),160),600);V.style.setProperty("--sidebar-width",`${C}px`),$.current=C},x=()=>{T.classList.remove("dragging"),V.classList.remove("sidebar-resizing"),document.body.style.userSelect="",s_("sidebarWidth",String(Math.round($.current||f))),document.removeEventListener("touchmove",v),document.removeEventListener("touchend",x),document.removeEventListener("touchcancel",x)};document.addEventListener("touchmove",v,{passive:!1}),document.addEventListener("touchend",x),document.addEventListener("touchcancel",x)}).current,z=y((q)=>{q.preventDefault();let V=_.current;if(!V)return;let H=q.clientX,b=j.current||$.current||280,f=q.currentTarget;f.classList.add("dragging"),document.body.style.cursor="col-resize",document.body.style.userSelect="none";let T=H,v=(F)=>{T=F.clientX;let A=Math.min(Math.max(b+(F.clientX-H),200),800);V.style.setProperty("--editor-width",`${A}px`),j.current=A},x=()=>{let F=Math.min(Math.max(b+(T-H),200),800);j.current=F,f.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",s_("editorWidth",String(Math.round(F))),document.removeEventListener("mousemove",v),document.removeEventListener("mouseup",x)};document.addEventListener("mousemove",v),document.addEventListener("mouseup",x)}).current,G=y((q)=>{q.preventDefault();let V=_.current;if(!V)return;let H=q.touches[0];if(!H)return;let b=H.clientX,f=j.current||$.current||280,T=q.currentTarget;T.classList.add("dragging"),document.body.style.userSelect="none";let v=(F)=>{let A=F.touches[0];if(!A)return;F.preventDefault();let C=Math.min(Math.max(f+(A.clientX-b),200),800);V.style.setProperty("--editor-width",`${C}px`),j.current=C},x=()=>{T.classList.remove("dragging"),document.body.style.userSelect="",s_("editorWidth",String(Math.round(j.current||f))),document.removeEventListener("touchmove",v),document.removeEventListener("touchend",x),document.removeEventListener("touchcancel",x)};document.addEventListener("touchmove",v,{passive:!1}),document.addEventListener("touchend",x),document.addEventListener("touchcancel",x)}).current,X=y((q)=>{q.preventDefault();let V=_.current;if(!V)return;let H=q.clientY,b=Z?.current||200,f=q.currentTarget;f.classList.add("dragging"),document.body.style.cursor="row-resize",document.body.style.userSelect="none";let T=H,v=(F)=>{T=F.clientY;let A=Math.min(Math.max(b-(F.clientY-H),100),window.innerHeight*0.5);if(V.style.setProperty("--dock-height",`${A}px`),Z)Z.current=A;window.dispatchEvent(new CustomEvent("dock-resize"))},x=()=>{let F=Math.min(Math.max(b-(T-H),100),window.innerHeight*0.5);if(Z)Z.current=F;f.classList.remove("dragging"),document.body.style.cursor="",document.body.style.userSelect="",s_("dockHeight",String(Math.round(F))),window.dispatchEvent(new CustomEvent("dock-resize")),document.removeEventListener("mousemove",v),document.removeEventListener("mouseup",x)};document.addEventListener("mousemove",v),document.addEventListener("mouseup",x)}).current,O=y((q)=>{q.preventDefault();let V=_.current;if(!V)return;let H=q.touches[0];if(!H)return;let b=H.clientY,f=Z?.current||200,T=q.currentTarget;T.classList.add("dragging"),document.body.style.userSelect="none";let v=(F)=>{let A=F.touches[0];if(!A)return;F.preventDefault();let C=Math.min(Math.max(f-(A.clientY-b),100),window.innerHeight*0.5);if(V.style.setProperty("--dock-height",`${C}px`),Z)Z.current=C;window.dispatchEvent(new CustomEvent("dock-resize"))},x=()=>{T.classList.remove("dragging"),document.body.style.userSelect="",s_("dockHeight",String(Math.round(Z?.current||f))),window.dispatchEvent(new CustomEvent("dock-resize")),document.removeEventListener("touchmove",v),document.removeEventListener("touchend",x),document.removeEventListener("touchcancel",x)};document.addEventListener("touchmove",v,{passive:!1}),document.addEventListener("touchend",x),document.addEventListener("touchcancel",x)}).current;return{handleSplitterMouseDown:N,handleSplitterTouchStart:K,handleEditorSplitterMouseDown:z,handleEditorSplitterTouchStart:G,handleDockSplitterMouseDown:X,handleDockSplitterTouchStart:O}}function S8({onTabClosed:_}={}){let $=y(_);$.current=_;let[j,Z]=I(()=>m_.getTabs()),[N,K]=I(()=>m_.getActiveId()),[z,G]=I(()=>m_.getTabs().length>0);u(()=>{return m_.onChange((C,c)=>{Z(C),K(c),G(C.length>0)})},[]);let[X,O]=I(()=>new Set),q=P((C)=>{O((c)=>{let a=new Set(c);if(a.has(C))a.delete(C);else a.add(C);return a})},[]),V=P((C)=>{O((c)=>{if(!c.has(C))return c;let a=new Set(c);return a.delete(C),a})},[]),H=P((C)=>{if(!C)return;let c={path:C,mode:"edit"};try{if(!X0.resolve(c)){if(!X0.get("editor")){console.warn(`[openEditor] No pane handler for: ${C}`);return}}}catch(a){console.warn(`[openEditor] paneRegistry.resolve() error for "${C}":`,a)}m_.open(C)},[]),b=P(()=>{let C=m_.getActiveId();if(C){let c=m_.get(C);if(c?.dirty){if(!window.confirm(`"${c.label}" has unsaved changes. Close anyway?`))return}m_.close(C),V(C),$.current?.(C)}},[V]),f=P((C)=>{let c=m_.get(C);if(c?.dirty){if(!window.confirm(`"${c.label}" has unsaved changes. Close anyway?`))return}m_.close(C),V(C),$.current?.(C)},[V]),T=P((C)=>{m_.activate(C)},[]),v=P((C)=>{let c=m_.getTabs().filter((l)=>l.id!==C&&!l.pinned),a=c.filter((l)=>l.dirty).length;if(a>0){if(!window.confirm(`${a} unsaved tab${a>1?"s":""} will be closed. Continue?`))return}let t=c.map((l)=>l.id);m_.closeOthers(C),t.forEach((l)=>{V(l),$.current?.(l)})},[V]),x=P(()=>{let C=m_.getTabs().filter((t)=>!t.pinned),c=C.filter((t)=>t.dirty).length;if(c>0){if(!window.confirm(`${c} unsaved tab${c>1?"s":""} will be closed. Continue?`))return}let a=C.map((t)=>t.id);m_.closeAll(),a.forEach((t)=>{V(t),$.current?.(t)})},[V]),F=P((C)=>{m_.togglePin(C)},[]),A=P(()=>{let C=m_.getActiveId();if(C)window.dispatchEvent(new CustomEvent("workspace-reveal-path",{detail:{path:C}}))},[]);return u(()=>{let C=(c)=>{let{oldPath:a,newPath:t,type:l}=c.detail||{};if(!a||!t)return;if(l==="dir"){for(let N_ of m_.getTabs())if(N_.path===a||N_.path.startsWith(`${a}/`)){let i=`${t}${N_.path.slice(a.length)}`;m_.rename(N_.id,i)}}else m_.rename(a,t)};return window.addEventListener("workspace-file-renamed",C),()=>window.removeEventListener("workspace-file-renamed",C)},[]),u(()=>{let C=(c)=>{if(m_.hasUnsaved())c.preventDefault(),c.returnValue=""};return window.addEventListener("beforeunload",C),()=>window.removeEventListener("beforeunload",C)},[]),{editorOpen:z,tabStripTabs:j,tabStripActiveId:N,previewTabs:X,openEditor:H,closeEditor:b,handleTabClose:f,handleTabActivate:T,handleTabCloseOthers:v,handleTabCloseAll:x,handleTabTogglePin:F,handleTabTogglePreview:q,revealInExplorer:A}}function X1(_,$){try{if(typeof window>"u")return $;let j=window.__PICLAW_SILENCE||{},Z=`__PICLAW_SILENCE_${_.toUpperCase()}_MS`,N=j[_]??window[Z],K=Number(N);return Number.isFinite(K)?K:$}catch{return $}}var W1=X1("warning",30000),f8=X1("finalize",120000),V1=X1("refresh",30000),T8=30000;function I8(_){let $={};return(_?.agents||[]).forEach((j)=>{$[j.id]=j}),$}function m8(){if(/iPad|iPhone/.test(navigator.userAgent))return!0;return navigator.platform==="MacIntel"&&navigator.maxTouchPoints>1}function u8(_=30000){let[,$]=I(0);u(()=>{let j=setInterval(()=>$((Z)=>Z+1),_);return()=>clearInterval(j)},[_])}function q1(_,$=160){let j=String(_||"").replace(/\r\n/g,`
...
`).reduce((Z,N)=>Z+Math.max(1,Math.ceil(N.length/$)),0)}function v8(_,$){if(typeof _!=="string")return{kind:"ignore"};let j=_.trim();if(!j)return{kind:"toast",title:"No file selected",detail:"Use a valid file path from a file pill.",level:"warning"};if(!$.editorOpen)return{kind:"toast",title:"Editor pane is not open",detail:"Open the editor pane to open files from pills.",level:"warning"};if(/^[a-z][a-z0-9+.-]*:/i.test(j))return{kind:"toast",title:"Cannot open external path from file pill",detail:"Use an in-workspace file path.",level:"warning"};try{if(!$.resolvePane({path:j,mode:"edit"}))return{kind:"toast",title:"No editor available",detail:`No editor can open: ${j}`,level:"warning"}}catch{return{kind:"toast",title:"No editor available",detail:`No editor can open: ${j}`,level:"warning"}}return{kind:"open",path:j}}function O$(_={}){let $=_.window??(typeof window<"u"?window:null),j=_.navigator??(typeof navigator<"u"?navigator:null);if(j&&j.standalone===!0)return!0;if(!$||typeof $.matchMedia!=="function")return!1;return["(display-mode: standalone)","(display-mode: minimal-ui)","(display-mode: fullscreen)"].some((N)=>{try{return Boolean($.matchMedia(N)?.matches)}catch{return!1}})}function U1(_={}){let $=_.window??(typeof window<"u"?window:null),j=_.navigator??(typeof navigator<"u"?navigator:null);if(!$&&!j)return!1;let Z=String(j?.userAgent||""),N=Number(j?.maxTouchPoints||0),K=/Android|webOS|iPhone|iPad|iPod|Mobile|Windows Phone/i.test(Z),z=(()=>{if(!$||typeof $.matchMedia!=="function")return!1;try{return Boolean($.matchMedia("(pointer: coarse)")?.matches||$.matchMedia("(any-pointer: coarse)")?.matches)}catch{return!1}})();return Boolean(K||N>1||z)}function R8(_,$={}){if(O$($))return null;if(U1($))return{target:"_blank",features:void 0,mode:"tab"};return{target:a9(_),features:"popup=yes,width=900,height=960,resizable=yes,scrollbars=yes",mode:"popup"}}function g8(_,$={}){let j=$.window??(typeof window<"u"?window:null);if(!j||!_)return null;try{return _.features?j.open("about:blank",_.target,_.features):j.open("about:blank",_.target)}catch{return null}}function c8(_,$={}){if(!_||!_.document)return;try{let j=String($.title||"Opening branch…"),Z=String($.message||"Preparing a new branch window…");_.document.title=j,_.document.body.innerHTML=`
        ...
        `}catch{}}function p8(_,$){if(!_||!$)return;try{if(_.location&&typeof _.location.replace==="function"){_.location.replace(String($));return}_.location=String($)}catch{}}function h8(_){if(!_||typeof _.close!=="function")return;try{_.close()}catch{}}function L$(_,$,j={}){let Z=new URL(String(_||"http://localhost/")),N=String($||"").trim()||"web:default";if(Z.searchParams.set("chat_jid",N),Z.searchParams.delete("branch_loader"),Z.searchParams.delete("branch_source_chat_jid"),j.chatOnly!==!1)Z.searchParams.set("chat_only","1");return Z.toString()}function i8(_,$,j={}){let Z=new URL(String(_||"http://localhost/")),N=String($||"").trim()||"web:default";if(Z.searchParams.set("branch_loader","1"),Z.searchParams.set("branch_source_chat_jid",N),j.chatOnly!==!1)Z.searchParams.set("chat_only","1");return Z.toString()}function a9(_){return`piclaw-chat-${String(_||"web:default").trim().toLowerCase().replace(/[^a-z0-9_-]+/g,"-")||"default"}`}function B1(_){let $=_ instanceof Error?_.message:String(_||"").trim(),j=String($||"").trim();if(!j)return"PiClaw could not open a new branch window.";let Z=j.toLowerCase();if(Z.includes("no stable turn boundary"))return"This chat is still in flight and does not yet have a stable turn boundary to fork from.";if(Z.includes("cannot fork a branch while the source chat is still active"))return"This chat is still active. Please wait for the current turn to settle, then try again.";if(Z.includes("cancelled"))return"Branch creation was cancelled before a new chat window could be opened.";if(Z.includes("did not return a chat id"))return"PiClaw created no usable branch id for the new window. Please try again.";if(Z.includes("failed to fork branch")||Z.includes("failed to fork chat branch"))return"PiClaw could not create the new branch. Please try again.";return j}function Q1(_){return String(_||"").trim()||"web:default"}function l8({remainingQueueCount:_=0,currentTurnId:$=null,isAgentTurnActive:j=!1}={}){return Number(_||0)<=0&&!$&&!j}function n8(_={}){return O$(_)&&U1(_)}function t9(_={}){let $=_.window??(typeof window<"u"?window:null),j=Number($?.visualViewport?.height||0);if(Number.isFinite(j)&&j>0)return Math.round(j);let Z=Number($?.innerHeight||0);if(Number.isFinite(Z)&&Z>0)return Math.round(Z);return null}function e9(_={}){if(!n8(_))return null;let $=_.window??(typeof window<"u"?window:null),j=_.document??(typeof document<"u"?document:null);if(!$||!j?.documentElement)return null;let Z=t9({window:$});if(Z&&Z>0)j.documentElement.style.setProperty("--app-height",`${Z}px`);try{if(typeof $.scrollTo==="function")$.scrollTo(0,0)}catch{}try{if(j.scrollingElement)j.scrollingElement.scrollTop=0,j.scrollingElement.scrollLeft=0;if(j.documentElement)j.documentElement.scrollTop=0,j.documentElement.scrollLeft=0;if(j.body)j.body.scrollTop=0,j.body.scrollLeft=0}catch{}return Z}function o8(_={}){if(!n8(_))return()=>{};let $=_.window??(typeof window<"u"?window:null),j=_.document??(typeof document<"u"?document:null);if(!$||!j)return()=>{};let Z=0,N=new Set,K=()=>{if(Z)$.cancelAnimationFrame?.(Z),Z=0;for(let V of N)$.clearTimeout?.(V);N.clear()},z=()=>{Z=0,e9({window:$,document:j})},G=()=>{if(Z)$.cancelAnimationFrame?.(Z);Z=$.requestAnimationFrame?.(z)??0},X=()=>{G();for(let V of[80,220,420]){let H=$.setTimeout?.(()=>{N.delete(H),G()},V);if(H!=null)N.add(H)}},O=()=>{if(j.visibilityState&&j.visibilityState!=="visible")return;X()},q=$.visualViewport;return X(),$.addEventListener("focus",X),$.addEventListener("pageshow",X),$.addEventListener("resize",X),$.addEventListener("orientationchange",X),j.addEventListener("visibilitychange",O),j.addEventListener("focusin",X,!0),q?.addEventListener?.("resize",X),q?.addEventListener?.("scroll",X),()=>{K(),$.removeEventListener("focus",X),$.removeEventListener("pageshow",X),$.removeEventListener("resize",X),$.removeEventListener("orientationchange",X),j.removeEventListener("visibilitychange",O),j.removeEventListener("focusin",X,!0),q?.removeEventListener?.("resize",X),q?.removeEventListener?.("scroll",X)}}function _7(_,$){if(typeof window<"u")console.warn(`[app] API export missing: ${_}. Using fallback behavior.`);return async()=>$}function N2(_,$,j){let Z=_?.[$];return typeof Z==="function"?Z:_7($,j)}var $7=["(display-mode: standalone)","(display-mode: minimal-ui)","(display-mode: fullscreen)"];function d8(_,$={}){let j=$.window??(typeof window<"u"?window:null),Z=$.navigator??(typeof navigator<"u"?navigator:null);if(!j||typeof _!=="function")return()=>{};let N=()=>{_(O$({window:j,navigator:Z}))};N();let z=$7.map((G)=>{try{return j.matchMedia?.(G)??null}catch{return null}}).filter(Boolean).map((G)=>{if(typeof G.addEventListener==="function")return G.addEventListener("change",N),()=>G.removeEventListener("change",N);if(typeof G.addListener==="function")return G.addListener(N),()=>G.removeListener(N);return()=>{}});return j.addEventListener?.("focus",N),j.addEventListener?.("pageshow",N),()=>{for(let G of z)G();j.removeEventListener?.("focus",N),j.removeEventListener?.("pageshow",N)}}function s8(_,$={}){let j=$.window??(typeof window<"u"?window:null),Z=$.document??(typeof document<"u"?document:null);if(!j||!Z||typeof _!=="function")return()=>{};let N=()=>{if(Z.visibilityState&&Z.visibilityState!=="visible")return;_()};return j.addEventListener?.("focus",N),j.addEventListener?.("pageshow",N),Z.addEventListener?.("visibilitychange",N),()=>{j.removeEventListener?.("focus",N),j.removeEventListener?.("pageshow",N),Z.removeEventListener?.("visibilitychange",N)}}var H1="piclaw_btw_session";function j7(_){if(_==="root")return"Branch family";if(_==="all")return"All chats";return"Current branch"}function Z7(){let _=s0(H1);if(!_)return null;try{let $=JSON.parse(_);if(!$||typeof $!=="object")return null;let j=typeof $.question==="string"?$.question:"",Z=typeof $.answer==="string"?$.answer:"",N=typeof $.thinking==="string"?$.thinking:"",K=typeof $.error==="string"&&$.error.trim()?$.error:null,z=$.status==="running"?"error":$.status==="success"||$.status==="error"?$.status:"success";return{question:j,answer:Z,thinking:N,error:z==="error"?K||"BTW stream interrupted. You can retry.":K,model:null,status:z}}catch{return null}}var r8=q3,a8=B3,N7=F3,t8=E3,e8=k3,_6=H3,F1=N2(d0,"getAgentContext",null),K7=N2(d0,"getAgentModels",{current:null,models:[]}),z7=N2(d0,"getActiveChatAgents",{chats:[]}),$6=N2(d0,"getChatBranches",{chats:[]}),Y7=N2(d0,"renameChatBranch",null),G7=N2(d0,"pruneChatBranch",null),O7=N2(d0,"getAgentQueueState",{count:0}),L7=N2(d0,"steerAgentQueueItem",{removed:!1,queued:"steer"}),X7=N2(d0,"removeAgentQueueItem",{removed:!1}),W7=N2(d0,"streamSidePrompt",null);if(window.marked)marked.setOptions({breaks:!0,gfm:!0});X0.register(s3);X0.register($1);X0.register(_1);r3();X0.register(t3);function V7({locationParams:_}){let $=i_(()=>{let L=_.get("chat_jid");return L&&L.trim()?L.trim():"web:default"},[_]),j=i_(()=>{let L=(_.get("chat_only")||_.get("chat-only")||"").trim().toLowerCase();return L==="1"||L==="true"||L==="yes"},[_]),Z=i_(()=>{let L=(_.get("branch_loader")||"").trim().toLowerCase();return L==="1"||L==="true"||L==="yes"},[_]),N=i_(()=>{let L=_.get("branch_source_chat_jid");return L&&L.trim()?L.trim():$},[$,_]),[K,z]=I("disconnected"),[G,X]=I(()=>isStandaloneWebAppMode()),[O,q]=I(null),[V,H]=I(null),[b,f]=I(!1),[T,v]=I("current"),[x,F]=I([]),[A,C]=I([]),[c,a]=I(null),{agentStatus:t,setAgentStatus:l,agentDraft:N_,setAgentDraft:i,agentPlan:Q_,setAgentPlan:U_,agentThought:s,setAgentThought:__,pendingRequest:K_,setPendingRequest:j_,currentTurnId:G_,setCurrentTurnId:X_,steerQueuedTurnId:J_,setSteerQueuedTurnId:Z_,lastAgentEventRef:D_,lastSilenceNoticeRef:k_,isAgentRunningRef:r_,draftBufferRef:E_,thoughtBufferRef:u_,pendingRequestRef:b_,stalledPostIdRef:l_,currentTurnIdRef:p,steerQueuedTurnIdRef:L_,thoughtExpandedRef:W_,draftExpandedRef:V_}=C8(),[y_,w_]=I({}),[v_,F_]=I(null),[C_,S_]=I(null),[Y0,n_]=I(!1),[e_,B0]=I(null),[x_,_0]=I([]),[Q0,D0]=I([]),[F0,G0]=I(null),[E0,o_]=I([]),[T_,M0]=I(!1),[c_,d_]=I(()=>Z7()),k0=i_(()=>x_.find((L)=>L?.chat_jid===$)||null,[x_,$]),B_=i_(()=>Q0.find((L)=>L?.chat_jid===$)||k0||null,[k0,Q0,$]),M=B_?.root_chat_jid||k0?.root_chat_jid||$,e=j7(T),[A_,R_]=I(()=>({status:Z?"running":"idle",message:Z?"Preparing a new chat branch…":""})),b0=E0.length,a0=y(new Set),$0=y([]),O0=y(new Set),W0=y({inFlight:!1,lastAttemptAt:0,turnId:null});a0.current=new Set(E0.map((L)=>L.row_id)),$0.current=E0;let{notificationsEnabled:t0,notificationPermission:C0,toggleNotifications:R0,notify:g0}=P8(),[n2,b2]=I(()=>new Set),[c0,j0]=I(()=>p2("workspaceOpen",!0)),K2=y(null),{editorOpen:L0,tabStripTabs:Z0,tabStripActiveId:I_,previewTabs:I0,openEditor:p0,closeEditor:z2,handleTabClose:J2,handleTabActivate:w2,handleTabCloseOthers:H0,handleTabCloseAll:A2,handleTabTogglePin:o2,handleTabTogglePreview:y2,revealInExplorer:m2}=S8({onTabClosed:(L)=>K2.current?.(L)}),q2=y(null),a_=y(null),P2=y(null),x0=y(null),S0=X0.getDockPanes().length>0,[Y2,X$]=I(!1),U0=P(()=>X$((L)=>!L),[]),M2=!j&&(L0||S0&&Y2);u(()=>{let L=q2.current;if(!L)return;if(a_.current)a_.current.dispose(),a_.current=null;let Y=I_;if(!Y)return;let J={path:Y,mode:"edit"},w=X0.resolve(J)||X0.get("editor");if(!w){L.innerHTML='<div style="padding:2em;color:var(--text-secondary);text-align:center;">No editor available for this file.</div>';return}let k=w.mount(L,J);a_.current=k,k.onDirtyChange?.(($_)=>{m_.setDirty(Y,$_)}),k.onSaveRequest?.(()=>{}),k.onClose?.(()=>{z2()});let h=m_.getViewState(Y);if(h&&typeof k.restoreViewState==="function")requestAnimationFrame(()=>k.restoreViewState(h));if(typeof k.onViewStateChange==="function")k.onViewStateChange(($_)=>{m_.saveViewState(Y,$_)});return requestAnimationFrame(()=>k.focus()),()=>{if(a_.current===k)k.dispose(),a_.current=null}},[I_,z2]),u(()=>{let L=P2.current;if(x0.current)x0.current.dispose(),x0.current=null;if(!L||!S0||!Y2)return;let Y=X0.getDockPanes()[0];if(!Y){L.innerHTML='<div class="terminal-placeholder">No dock pane available.</div>';return}let J=Y.mount(L,{mode:"view"});return x0.current=J,requestAnimationFrame(()=>J.focus?.()),()=>{if(x0.current===J)J.dispose(),x0.current=null}},[S0,Y2]);let[G2,O2]=I({name:"You",avatar_url:null,avatar_background:null}),C2=y(!1),h0=y(!1),e0=y(null),f0=y($),_2=y(new Map),x2=y($),S2=y(0),$2=y(0),U2=y({}),d2=y({name:null,avatar_url:null}),w0=y({currentHashtag:null,searchQuery:null}),B=y(null),S=y(null),n=y(0),Y_=y(0),P_=y(0),t_=y(null),M_=y(null),N0=y(null),i0=y(null),D2=y(0),m0=y({title:null,avatarBase:null}),L2=y(null),X2=P(()=>{if(L2.current)clearTimeout(L2.current),L2.current=null;a(null)},[]);u8(30000),u(()=>{return H4()},[]),u(()=>{return d8(X)},[]),u(()=>{s_("workspaceOpen",String(c0))},[c0]),u(()=>{return o8()},[]),u(()=>{return()=>{X2()}},[X2]),u(()=>{if(!c_){s_(H1,"");return}s_(H1,JSON.stringify({question:c_.question||"",answer:c_.answer||"",thinking:c_.thinking||"",error:c_.error||null,status:c_.status||"success"}))},[c_]),u(()=>{U2.current=y_||{}},[y_]),u(()=>{f0.current=$},[$]),u(()=>{d2.current=G2||{name:"You",avatar_url:null,avatar_background:null}},[G2]);let l0=P((L,Y,J=null)=>{if(typeof document>"u")return;let w=(L||"").trim()||"PiClaw";if(m0.current.title!==w){document.title=w;let R=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(R&&R.getAttribute("content")!==w)R.setAttribute("content",w);m0.current.title=w}let k=document.getElementById("dynamic-favicon");if(!k)return;let h=k.getAttribute("data-default")||k.getAttribute("href")||"/favicon.ico",$_=Y||h,H_=Y?`${$_}|${J||""}`:$_;if(m0.current.avatarBase!==H_){let R=Y?`${$_}${$_.includes("?")?"&":"?"}v=${J||Date.now()}`:$_;k.setAttribute("href",R),m0.current.avatarBase=H_}},[]),W2=P((L)=>{if(!L)return;F((Y)=>Y.includes(L)?Y:[...Y,L])},[]),J0=P((L)=>{F((Y)=>Y.filter((J)=>J!==L))},[]);K2.current=J0;let f2=P(()=>{F([])},[]),W=P((L,Y=null,J="info",w=3000)=>{X2(),a({title:L,detail:Y||null,kind:J||"info"}),L2.current=setTimeout(()=>{a((k)=>k?.title===L?null:k)},w)},[X2]),Q=P((L)=>{let Y=v8(L,{editorOpen:L0,resolvePane:(J)=>X0.resolve(J)});if(Y.kind==="open"){p0(Y.path);return}if(Y.kind==="toast")W(Y.title,Y.detail,Y.level)},[L0,p0,W]),E=P(()=>{let L=I_;if(L)W2(L)},[I_,W2]),D=P((L)=>{if(!L)return;C((Y)=>Y.includes(L)?Y:[...Y,L])},[]),m=P(async(L,Y=null)=>{let J=(k)=>{k.scrollIntoView({behavior:"smooth",block:"center"}),k.classList.add("post-highlight"),setTimeout(()=>k.classList.remove("post-highlight"),2000)},w=document.getElementById("post-"+L);if(w){J(w);return}try{let k=typeof Y==="string"&&Y.trim()?Y.trim():$,$_=(await U3(L,k))?.thread?.[0];if(!$_)return;q0((H_)=>{if(!H_)return[$_];if(H_.some((R)=>R.id===$_.id))return H_;return[...H_,$_]}),requestAnimationFrame(()=>{setTimeout(()=>{let H_=document.getElementById("post-"+L);if(H_)J(H_)},50)})}catch(k){console.error("[scrollToMessage] Failed to fetch message",L,k)}},[$]),d=P((L)=>{C((Y)=>Y.filter((J)=>J!==L))},[]),r=P(()=>{C([])},[]),o=P((L={})=>{let Y=Date.now();if(D_.current=Y,L.running)r_.current=!0,M0((J)=>J?J:!0);if(L.clearSilence)k_.current=0},[M0]),g=P(()=>{if(i0.current)clearTimeout(i0.current),i0.current=null;D2.current=0},[]);u(()=>()=>{g()},[g]);let z_=P(()=>{g(),l((L)=>{if(!L)return L;if(!(L.last_activity||L.lastActivity))return L;let{last_activity:Y,lastActivity:J,...w}=L;return w})},[g]),y0=P((L)=>{if(!L)return;g();let Y=Date.now();D2.current=Y,l({type:L.type||"active",last_activity:!0}),i0.current=setTimeout(()=>{if(D2.current!==Y)return;l((J)=>{if(!J||!(J.last_activity||J.lastActivity))return J;return null})},T8)},[g]),V0=P(()=>{r_.current=!1,M0(!1),D_.current=null,k_.current=0,E_.current="",u_.current="",b_.current=null,M_.current=null,p.current=null,L_.current=null,e0.current=null,W0.current={inFlight:!1,lastAttemptAt:0,turnId:null},g(),X_(null),Z_(null),W_.current=!1,V_.current=!1},[g,X_,Z_,M0]),A0=P((L)=>{if(!l8({remainingQueueCount:L,currentTurnId:p.current,isAgentTurnActive:T_}))return;L_.current=null,Z_(null)},[T_,Z_]),J1=P(()=>({agentStatus:null,agentDraft:{text:"",totalLines:0},agentPlan:"",agentThought:{text:"",totalLines:0},pendingRequest:null,currentTurnId:null,steerQueuedTurnId:null,isAgentTurnActive:!1,followupQueueItems:[],activeModel:null,activeThinkingLevel:null,supportsThinking:!1,activeModelUsage:null,contextUsage:null,isAgentRunning:!1,wasAgentActive:!1,draftBuffer:"",thoughtBuffer:"",lastAgentEvent:null,lastSilenceNotice:0,lastAgentResponse:null,currentTurnIdRef:null,steerQueuedTurnIdRef:null,thoughtExpanded:!1,draftExpanded:!1,agentStatusRef:null,silentRecovery:{inFlight:!1,lastAttemptAt:0,turnId:null}}),[]),W$=P(()=>({agentStatus:t,agentDraft:N_?{...N_}:{text:"",totalLines:0},agentPlan:Q_||"",agentThought:s?{...s}:{text:"",totalLines:0},pendingRequest:K_,currentTurnId:G_,steerQueuedTurnId:J_,isAgentTurnActive:Boolean(T_),followupQueueItems:Array.isArray(E0)?E0.map((L)=>({...L})):[],activeModel:v_,activeThinkingLevel:C_,supportsThinking:Boolean(Y0),activeModelUsage:e_,contextUsage:F0,isAgentRunning:Boolean(r_.current),wasAgentActive:Boolean(h0.current),draftBuffer:E_.current||"",thoughtBuffer:u_.current||"",lastAgentEvent:D_.current||null,lastSilenceNotice:k_.current||0,lastAgentResponse:M_.current||null,currentTurnIdRef:p.current||null,steerQueuedTurnIdRef:L_.current||null,thoughtExpanded:Boolean(W_.current),draftExpanded:Boolean(V_.current),agentStatusRef:e0.current||null,silentRecovery:{...W0.current||{inFlight:!1,lastAttemptAt:0,turnId:null}}}),[v_,e_,C_,N_,Q_,t,s,F0,G_,E0,T_,K_,J_,Y0]),y1=P((L)=>{let Y=L||J1();g(),r_.current=Boolean(Y.isAgentRunning),h0.current=Boolean(Y.wasAgentActive),M0(Boolean(Y.isAgentTurnActive)),D_.current=Y.lastAgentEvent||null,k_.current=Number(Y.lastSilenceNotice||0),E_.current=Y.draftBuffer||"",u_.current=Y.thoughtBuffer||"",b_.current=Y.pendingRequest||null,M_.current=Y.lastAgentResponse||null,p.current=Y.currentTurnIdRef||null,L_.current=Y.steerQueuedTurnIdRef||null,e0.current=Y.agentStatusRef||null,W0.current=Y.silentRecovery||{inFlight:!1,lastAttemptAt:0,turnId:null},W_.current=Boolean(Y.thoughtExpanded),V_.current=Boolean(Y.draftExpanded),l(Y.agentStatus||null),i(Y.agentDraft?{...Y.agentDraft}:{text:"",totalLines:0}),U_(Y.agentPlan||""),__(Y.agentThought?{...Y.agentThought}:{text:"",totalLines:0}),j_(Y.pendingRequest||null),X_(Y.currentTurnId||null),Z_(Y.steerQueuedTurnId||null),o_(Array.isArray(Y.followupQueueItems)?Y.followupQueueItems.map((J)=>({...J})):[]),F_(Y.activeModel||null),S_(Y.activeThinkingLevel||null),n_(Boolean(Y.supportsThinking)),B0(Y.activeModelUsage??null),G0(Y.contextUsage??null)},[g,J1,X_,o_,M0,Z_]),V2=P((L)=>{if(!L)return;if(p.current===L)return;p.current=L,W0.current={inFlight:!1,lastAttemptAt:0,turnId:L},X_(L),L_.current=null,Z_(null),E_.current="",u_.current="",i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null),b_.current=null,M_.current=null,W_.current=!1,V_.current=!1},[X_,Z_]),D1=P((L)=>{if(typeof document<"u"){let R=typeof document.hasFocus==="function"?document.hasFocus():!0;if(!document.hidden&&R)return}let Y=M_.current;if(!Y||!Y.post)return;if(L&&Y.turnId&&Y.turnId!==L)return;let J=Y.post;if(J.id&&t_.current===J.id)return;let w=String(J?.data?.content||"").trim();if(!w)return;t_.current=J.id||t_.current,M_.current=null;let k=w.replace(/\s+/g," ").slice(0,200),h=U2.current||{},H_=(J?.data?.agent_id?h[J.data.agent_id]:null)?.name||"Pi";g0(H_,k)},[g0]),j6=P(async(L,Y)=>{if(L!=="thought"&&L!=="draft")return;let J=p.current;if(L==="thought"){if(W_.current=Y,J)try{await e8(J,"thought",Y)}catch(w){console.warn("Failed to update thought visibility:",w)}if(!Y)return;try{let w=J?await t8(J,"thought"):null;if(w?.text)u_.current=w.text;__((k)=>({...k||{text:"",totalLines:0},fullText:u_.current||k?.fullText||"",totalLines:Number.isFinite(w?.total_lines)?w.total_lines:k?.totalLines||0}))}catch(w){console.warn("Failed to fetch full thought:",w)}return}if(V_.current=Y,J)try{await e8(J,"draft",Y)}catch(w){console.warn("Failed to update draft visibility:",w)}if(!Y)return;try{let w=J?await t8(J,"draft"):null;if(w?.text)E_.current=w.text;i((k)=>({...k||{text:"",totalLines:0},fullText:E_.current||k?.fullText||"",totalLines:Number.isFinite(w?.total_lines)?w.total_lines:k?.totalLines||0}))}catch(w){console.warn("Failed to fetch full draft:",w)}},[]),o$=y(null),d$=P(()=>{let L=B.current;if(!L)return;if(!(Math.abs(L.scrollTop)>150))L.scrollTop=0},[]);o$.current=d$;let Z6=P((L)=>{let Y=B.current;if(!Y||typeof L!=="function"){L?.();return}let{currentHashtag:J,searchQuery:w}=w0.current||{},k=!(w&&!J),h=k?Y.scrollHeight-Y.scrollTop:Y.scrollTop;L(),requestAnimationFrame(()=>{let $_=B.current;if(!$_)return;if(k){let H_=Math.max($_.scrollHeight-h,0);$_.scrollTop=H_}else{let H_=Math.max($_.scrollHeight-$_.clientHeight,0),R=Math.min(h,H_);$_.scrollTop=R}})},[]),s2=P((L)=>{let Y=B.current;if(!Y||typeof L!=="function"){L?.();return}let J=Y.scrollTop;L(),requestAnimationFrame(()=>{let w=B.current;if(!w)return;let k=Math.max(w.scrollHeight-w.clientHeight,0);w.scrollTop=Math.min(J,k)})},[]),N6="Queued as a follow-up (one-at-a-time).",K6="⁣",E1=P((L)=>{if(!L||!Array.isArray(L))return L;let Y=a0.current,J=new Set(Y),w=L.filter((k)=>{if(J.has(k?.id))return!1;if(k?.data?.is_bot_message){let h=k?.data?.content;if(h===N6||h===K6)return!1}return!0});return w.length===L.length?L:w},[]),{posts:k1,setPosts:q0,hasMore:z6,setHasMore:V$,hasMoreRef:b1,loadPosts:j2,refreshTimeline:P0,loadMore:Y6,loadMoreRef:s$}=M8({preserveTimelineScroll:Z6,preserveTimelineScrollTop:s2,chatJid:$}),u2=i_(()=>E1(k1),[k1,E0,E1]),q$=P(()=>{let L=l_.current;if(!L)return;q0((Y)=>Y?Y.filter((J)=>J.id!==L):Y),l_.current=null},[q0]),{handleSplitterMouseDown:G6,handleSplitterTouchStart:O6,handleEditorSplitterMouseDown:L6,handleEditorSplitterTouchStart:X6,handleDockSplitterMouseDown:W6,handleDockSplitterTouchStart:V6}=x8({appShellRef:S,sidebarWidthRef:n,editorWidthRef:Y_,dockHeightRef:P_}),w1=P(()=>{if(!r_.current)return;r_.current=!1,k_.current=0,D_.current=null,p.current=null,X_(null),W_.current=!1,V_.current=!1;let L=(E_.current||"").trim();if(E_.current="",u_.current="",i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null),b_.current=null,M_.current=null,!L){l({type:"error",title:"Response stalled - No content received"});return}let J=`${L}${`

⚠️ Response may be incomplete - the model stopped responding`}`,w=Date.now(),k=new Date().toISOString(),h={id:w,timestamp:k,data:{type:"agent_response",content:J,agent_id:"default",is_local_stall:!0}};l_.current=w,q0(($_)=>$_?G$([...$_,h]):[h]),o$.current?.(),l(null)},[X_]);u(()=>{w0.current={currentHashtag:O,searchQuery:V}},[O,V]);let g_=P(()=>{let L=$;O7(L).then((Y)=>{if(f0.current!==L)return;let J=O0.current,w=Array.isArray(Y?.items)?Y.items.map((k)=>({...k})).filter((k)=>!J.has(k.row_id)):[];if(w.length){o_((k)=>{if(k.length===w.length&&k.every((h,$_)=>h.row_id===w[$_].row_id))return k;return w});return}J.clear(),A0(0),o_((k)=>k.length===0?k:[])}).catch(()=>{if(f0.current!==L)return;o_((Y)=>Y.length===0?Y:[])})},[A0,$,o_]),n0=P(async()=>{let L=$;try{let Y=await F1(L);if(f0.current!==L)return;if(Y)G0(Y)}catch(Y){if(f0.current!==L)return;console.warn("Failed to fetch agent context:",Y)}},[$]),o0=P(async()=>{let L=$;try{let Y=await _6(L);if(f0.current!==L)return null;if(!Y||Y.status!=="active"||!Y.data){if(h0.current){let{currentHashtag:k,searchQuery:h}=w0.current||{};if(!k&&!h)P0()}return h0.current=!1,V0(),e0.current=null,l(null),i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null),b_.current=null,Y??null}h0.current=!0;let J=Y.data;e0.current=J;let w=J.turn_id||J.turnId;if(w)V2(w);if(o({running:!0,clearSilence:!0}),z_(),l(J),Y.thought&&Y.thought.text)__((k)=>{if(k&&k.text&&k.text.length>=Y.thought.text.length)return k;return u_.current=Y.thought.text,{text:Y.thought.text,totalLines:Y.thought.totalLines||0}});if(Y.draft&&Y.draft.text)i((k)=>{if(k&&k.text&&k.text.length>=Y.draft.text.length)return k;return E_.current=Y.draft.text,{text:Y.draft.text,totalLines:Y.draft.totalLines||0}});return Y}catch(Y){return console.warn("Failed to fetch agent status:",Y),null}},[V0,z_,o,P0,V2]),r$=P(async()=>{if(!r_.current)return null;if(b_.current)return null;let L=p.current||null,Y=W0.current,J=Date.now();if(Y.inFlight)return null;if(Y.turnId===L&&J-Y.lastAttemptAt<V1)return null;Y.inFlight=!0,Y.lastAttemptAt=J,Y.turnId=L;try{let{currentHashtag:w,searchQuery:k}=w0.current||{};if(!w&&!k)await P0();return await g_(),await o0()}finally{Y.inFlight=!1}},[o0,g_,P0]);u(()=>{let L=Math.min(1000,Math.max(100,Math.floor(W1/2))),Y=setInterval(()=>{if(!r_.current)return;if(b_.current)return;let J=D_.current;if(!J)return;let w=Date.now(),k=w-J,h=R$(e0.current);if(k>=f8){if(!h)l({type:"waiting",title:"Re-syncing after a quiet period…"});r$();return}if(k>=W1){if(w-k_.current>=V1){if(!h){let $_=Math.floor(k/1000);l({type:"waiting",title:`Waiting for model… No events for ${$_}s`})}k_.current=w,r$()}}},L);return()=>clearInterval(Y)},[r$]);let q6=P((L)=>{if(z(L),L!=="connected"){l(null),i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null),b_.current=null,V0();return}if(!C2.current){C2.current=!0,o0(),n0();return}let{currentHashtag:Y,searchQuery:J}=w0.current;if(!Y&&!J)P0();o0(),n0()},[V0,P0,o0,n0]),U6=P(async(L)=>{q(L),q0(null),await j2(L)},[j2]),B6=P(async()=>{q(null),H(null),q0(null),await j2()},[j2]),Q6=P(async(L,Y=T)=>{if(!L||!L.trim())return;let J=Y==="root"||Y==="all"?Y:"current";v(J),H(L.trim()),q(null),q0(null);try{let w=await r8(L.trim(),50,0,$,J,M);q0(w.results),V$(!1)}catch(w){console.error("Failed to search:",w),q0([])}},[$,M,T]),F6=P(()=>{f(!0),H(null),q(null),v("current"),q0([])},[]),H6=P(()=>{f(!1),H(null),j2()},[j2]),U7=P(()=>{},[]),J6=P(async(L)=>{if(!L)return;let Y=L.id,J=typeof L?.chat_jid==="string"&&L.chat_jid.trim()?L.chat_jid.trim():$,w=u2?.filter((h)=>h?.data?.thread_id===Y&&h?.id!==Y).length||0;if(w>0){if(!window.confirm(`Delete this message and its ${w} replies?`))return}let k=(h)=>{if(!h.length)return;b2((H_)=>{let R=new Set(H_);return h.forEach((O_)=>R.add(O_)),R}),setTimeout(()=>{if(s2(()=>{q0((H_)=>H_?H_.filter((R)=>!h.includes(R.id)):H_)}),b2((H_)=>{let R=new Set(H_);return h.forEach((O_)=>R.delete(O_)),R}),b1.current)s$.current?.({preserveScroll:!0,preserveMode:"top"})},180)};try{let h=await a8(Y,w>0,J);if(h?.ids?.length)k(h.ids)}catch(h){let $_=h?.message||"";if(w===0&&$_.includes("Replies exist")){if(!window.confirm("Delete this message and its replies?"))return;let R=await a8(Y,!0,J);if(R?.ids?.length)k(R.ids);return}console.error("Failed to delete post:",h),alert(`Failed to delete message: ${$_}`)}},[$,u2,s2]),A1=P(async()=>{try{let L=await N7();w_(I8(L));let Y=L?.user||{};O2((w)=>{let k=typeof Y.name==="string"&&Y.name.trim()?Y.name.trim():"You",h=typeof Y.avatar_url==="string"?Y.avatar_url.trim():null,$_=typeof Y.avatar_background==="string"&&Y.avatar_background.trim()?Y.avatar_background.trim():null;if(w.name===k&&w.avatar_url===h&&w.avatar_background===$_)return w;return{name:k,avatar_url:h,avatar_background:$_}});let J=(L?.agents||[]).find((w)=>w.id==="default");l0(J?.name,J?.avatar_url)}catch(L){console.warn("Failed to load agents:",L)}try{let L=$,Y=await F1(L);if(f0.current!==L)return;if(Y)G0(Y)}catch{}},[l0,$]);u(()=>{A1();let L=h2("sidebarWidth",null),Y=Number.isFinite(L)?Math.min(Math.max(L,160),600):280;if(n.current=Y,S.current)S.current.style.setProperty("--sidebar-width",`${Y}px`)},[A1]);let a$=T_||t!==null,P1=P((L)=>{if(!L||typeof L!=="object")return;let Y=L.agent_id;if(!Y)return;let{agent_name:J,agent_avatar:w}=L;if(!J&&w===void 0)return;let k=U2.current?.[Y]||{id:Y},h=k.name||null,$_=k.avatar_url??k.avatarUrl??k.avatar??null,H_=!1,R=!1;if(J&&J!==k.name)h=J,R=!0;if(w!==void 0){let O_=typeof w==="string"?w.trim():null,p_=typeof $_==="string"?$_.trim():null,q_=O_||null;if(q_!==(p_||null))$_=q_,H_=!0}if(!R&&!H_)return;if(w_((O_)=>{let q_={...O_[Y]||{id:Y}};if(R)q_.name=h;if(H_)q_.avatar_url=$_;return{...O_,[Y]:q_}}),Y==="default")l0(h,$_,H_?Date.now():null)},[l0]),M1=P((L)=>{if(!L||typeof L!=="object")return;let Y=L.user_name??L.userName,J=L.user_avatar??L.userAvatar,w=L.user_avatar_background??L.userAvatarBackground;if(Y===void 0&&J===void 0&&w===void 0)return;O2((k)=>{let h=typeof Y==="string"&&Y.trim()?Y.trim():k.name||"You",$_=J===void 0?k.avatar_url:typeof J==="string"&&J.trim()?J.trim():null,H_=w===void 0?k.avatar_background:typeof w==="string"&&w.trim()?w.trim():null;if(k.name===h&&k.avatar_url===$_&&k.avatar_background===H_)return k;return{name:h,avatar_url:$_,avatar_background:H_}})},[]),t$=P((L)=>{if(!L||typeof L!=="object")return;let Y=L.model??L.current;if(Y!==void 0)F_(Y);if(L.thinking_level!==void 0)S_(L.thinking_level??null);if(L.supports_thinking!==void 0)n_(Boolean(L.supports_thinking));if(L.provider_usage!==void 0)B0(L.provider_usage??null)},[]),r2=P(()=>{let L=$;K7(L).then((Y)=>{if(f0.current!==L)return;if(Y)t$(Y)}).catch(()=>{})},[t$,$]),u0=P(()=>{z7().then((L)=>{let Y=Array.isArray(L?.chats)?L.chats.filter((J)=>J&&typeof J.agent_name==="string"&&J.agent_name.trim()):[];_0(Y)}).catch(()=>{})},[]),T0=P(()=>{$6(M).then((L)=>{let Y=Array.isArray(L?.chats)?L.chats.filter((J)=>J&&typeof J.chat_jid==="string"&&typeof J.agent_name==="string"):[];D0(Y)}).catch(()=>{})},[M]),y6=P((L)=>{let Y=L?.row_id;if(Y==null)return;O0.current.add(Y),o_((J)=>J.filter((w)=>w?.row_id!==Y)),L7(Y,Q1($)).then(()=>{g_()}).catch((J)=>{console.warn("[queue] Failed to steer queued item:",J),W("Failed to steer message","The queued message could not be sent as steering.","warning"),O0.current.delete(Y),g_()})},[$,g_,o_,W]),D6=P((L)=>{let Y=L?.row_id;if(Y==null)return;let J=$0.current.filter((w)=>w?.row_id!==Y).length;O0.current.add(Y),A0(J),o_((w)=>w.filter((k)=>k?.row_id!==Y)),X7(Y,Q1($)).then(()=>{g_()}).catch((w)=>{console.warn("[queue] Failed to remove queued item:",w),W("Failed to remove message","The queued message could not be removed.","warning"),O0.current.delete(Y),g_()})},[A0,$,g_,o_,W]),e$=P((L)=>{if(!L||typeof L!=="object")return;if(u0(),T0(),L?.queued==="followup"||L?.queued==="steer"){g_();return}let Y=L?.command;if(Y&&typeof Y==="object"&&(Y?.queued_followup||Y?.queued_steer))g_()},[u0,T0,g_]),_3=P(()=>{if(N0.current)N0.current.abort(),N0.current=null;d_(null)},[]),U$=P(async(L)=>{let Y=String(L||"").trim();if(!Y)return W("BTW needs a question","Usage: /btw <question>","warning"),!0;if(N0.current)N0.current.abort();let J=new AbortController;N0.current=J,d_({question:Y,answer:"",thinking:"",error:null,model:null,status:"running"});try{let w=await W7(Y,{signal:J.signal,chatJid:C4($),systemPrompt:"Answer the user briefly and directly. This is a side conversation that should not affect the main chat until explicitly injected.",onEvent:(k,h)=>{if(k==="side_prompt_start")d_(($_)=>$_?{...$_,status:"running"}:$_)},onThinkingDelta:(k)=>{d_((h)=>h?{...h,thinking:`${h.thinking||""}${k||""}`}:h)},onTextDelta:(k)=>{d_((h)=>h?{...h,answer:`${h.answer||""}${k||""}`}:h)}});if(N0.current!==J)return!0;d_((k)=>k?{...k,answer:w?.result||k.answer||"",thinking:w?.thinking||k.thinking||"",model:w?.model||null,status:"success",error:null}:k)}catch(w){if(J.signal.aborted)return!0;d_((k)=>k?{...k,status:"error",error:w?.payload?.error||w?.message||"BTW request failed."}:k)}finally{if(N0.current===J)N0.current=null}return!0},[$,W]),E6=P(async({content:L})=>{let Y=M4(L);if(!Y)return!1;if(Y.type==="help")return W("BTW usage","Use /btw <question> to open a side conversation.","info",4000),!0;if(Y.type==="clear")return _3(),W("BTW cleared","Closed the side conversation panel.","info"),!0;if(Y.type==="ask")return await U$(Y.question),!0;return!1},[_3,U$,W]),k6=P(()=>{if(c_?.question)U$(c_.question)},[c_,U$]),b6=P(async()=>{let L=f4(c_);if(!L)return;try{let Y=await g2("default",L,null,[],a$?"queue":null,$);e$(Y),W(Y?.queued==="followup"?"BTW queued":"BTW injected",Y?.queued==="followup"?"The BTW summary was queued as a follow-up because the agent is busy.":"The BTW summary was sent to the main chat.","info",3500)}catch(Y){W("BTW inject failed",Y?.message||"Could not inject BTW answer into chat.","warning")}},[c_,e$,a$,W]),a2=P(()=>{r2(),u0(),T0(),g_(),n0()},[r2,u0,T0,g_,n0]);u(()=>{a2();let L=setInterval(()=>{r2(),u0(),T0(),g_()},60000);return()=>clearInterval(L)},[a2,r2,u0,T0,g_]),u(()=>{T0()},[T0]),u(()=>{let L=!1;if(q0(null),O)return j2(O),()=>{L=!0};if(V)return r8(V,50,0,$,T,M).then((Y)=>{if(L)return;q0(Y.results),V$(!1)}).catch((Y)=>{if(L)return;console.error("Failed to search:",Y),q0([]),V$(!1)}),()=>{L=!0};return j2(),()=>{L=!0}},[$,O,V,T,M,j2,V$,q0]),u(()=>{let L=x2.current||$;_2.current.set(L,W$())},[$,W$]),u(()=>{let L=x2.current||$;if(L===$)return;_2.current.set(L,W$()),x2.current=$,O0.current.clear(),y1(_2.current.get($)||null),g_(),o0(),n0()},[$,o0,n0,g_,y1,W$]);let w6=P(()=>{let{currentHashtag:L,searchQuery:Y}=w0.current||{};if(!L&&!Y)P0();a2()},[a2,P0]),$3=P((L,Y)=>{let J=Y?.turn_id,w=typeof Y?.chat_jid==="string"&&Y.chat_jid.trim()?Y.chat_jid.trim():null,h=w?w===$:L==="connected"||L==="workspace_update";if(h)P1(Y),M1(Y);if(L==="ui_theme"){J4(Y);return}if(L?.startsWith("agent_")){if(!(L==="agent_draft_delta"||L==="agent_thought_delta"||L==="agent_draft"||L==="agent_thought"))z_()}if(L==="connected"){l(null),i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null),b_.current=null,V0();let R=$;_6(R).then((q_)=>{if(f0.current!==R)return;if(!q_||q_.status!=="active"||!q_.data)return;let v2=q_.data,C1=v2.turn_id||v2.turnId;if(C1)V2(C1);if(o({clearSilence:!0}),y0(v2),q_.thought&&q_.thought.text)u_.current=q_.thought.text,__({text:q_.thought.text,totalLines:q_.thought.totalLines||0});if(q_.draft&&q_.draft.text)E_.current=q_.draft.text,i({text:q_.draft.text,totalLines:q_.draft.totalLines||0})}).catch((q_)=>{console.warn("Failed to fetch agent status:",q_)});let{currentHashtag:O_,searchQuery:p_}=w0.current||{};if(!O_&&!p_)P0();a2();return}if(L==="agent_status"){if(!h){if(Y?.type==="done"||Y?.type==="error")u0(),T0();return}if(Y.type==="done"||Y.type==="error"){if(J&&p.current&&J!==p.current)return;if(Y.type==="done"){D1(J||p.current);let{currentHashtag:R,searchQuery:O_}=w0.current||{};if(!R&&!O_)P0();if(Y.context_usage)G0(Y.context_usage)}if(h0.current=!1,V0(),O0.current.clear(),u0(),g_(),i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null),Y.type==="error")l({type:"error",title:Y.title||"Agent error"}),setTimeout(()=>l(null),8000);else l(null)}else{if(J)V2(J);if(o({running:!0,clearSilence:!0}),Y.type==="thinking")E_.current="",u_.current="",i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0});e0.current=Y,l((R)=>{if(R&&R.type===Y.type&&R.title===Y.title)return R;return Y})}return}if(L==="agent_steer_queued"){if(!h)return;if(J&&p.current&&J!==p.current)return;let R=J||p.current;if(!R)return;L_.current=R,Z_(R);return}if(L==="agent_followup_queued"){if(!h)return;let R=Y?.row_id,O_=Y?.content;if(R!=null&&typeof O_==="string"&&O_.trim())o_((p_)=>{if(p_.some((q_)=>q_?.row_id===R))return p_;return[...p_,{row_id:R,content:O_,timestamp:Y?.timestamp||null,thread_id:Y?.thread_id??null}]});g_();return}if(L==="agent_followup_consumed"){if(!h)return;let R=Y?.row_id;if(R!=null){let O_=$0.current.filter((p_)=>p_.row_id!==R).length;A0(O_),o_((p_)=>p_.filter((q_)=>q_.row_id!==R))}g_(),P0();return}if(L==="agent_followup_removed"){if(!h)return;let R=Y?.row_id;if(R!=null){let O_=$0.current.filter((p_)=>p_.row_id!==R).length;O0.current.add(R),A0(O_),o_((p_)=>p_.filter((q_)=>q_.row_id!==R))}g_();return}if(L==="agent_draft_delta"){if(!h)return;if(J&&p.current&&J!==p.current)return;if(J&&!p.current)V2(J);if(o({running:!0,clearSilence:!0}),Y?.reset)E_.current="";if(Y?.delta)E_.current+=Y.delta;let R=Date.now();if(!S2.current||R-S2.current>=100){S2.current=R;let O_=E_.current,p_=q1(O_);if(V_.current)i((q_)=>({text:q_?.text||"",totalLines:p_,fullText:O_}));else i({text:O_,totalLines:p_})}return}if(L==="agent_draft"){if(!h)return;if(J&&p.current&&J!==p.current)return;if(J&&!p.current)V2(J);o({running:!0,clearSilence:!0});let R=Y.text||"",O_=Y.mode||(Y.kind==="plan"?"replace":"append"),p_=Number.isFinite(Y.total_lines)?Y.total_lines:R?R.replace(/\r\n/g,`
`).split(`
`).length:0;if(Y.kind==="plan")if(O_==="replace")U_(R);else U_((q_)=>(q_||"")+R);else if(!V_.current)E_.current=R,i({text:R,totalLines:p_});return}if(L==="agent_thought_delta"){if(!h)return;if(J&&p.current&&J!==p.current)return;if(J&&!p.current)V2(J);if(o({running:!0,clearSilence:!0}),Y?.reset)u_.current="";if(typeof Y?.delta==="string")u_.current+=Y.delta;let R=Date.now();if(W_.current&&(!$2.current||R-$2.current>=100)){$2.current=R;let O_=u_.current;__((p_)=>({text:p_?.text||"",totalLines:q1(O_),fullText:O_}))}return}if(L==="agent_thought"){if(!h)return;if(J&&p.current&&J!==p.current)return;if(J&&!p.current)V2(J);o({running:!0,clearSilence:!0});let R=Y.text||"",O_=Number.isFinite(Y.total_lines)?Y.total_lines:R?R.replace(/\r\n/g,`
`).split(`
`).length:0;if(!W_.current)u_.current=R,__({text:R,totalLines:O_});return}if(L==="agent_request"){if(!h)return;if(console.log("Agent request:",Y),J&&p.current&&J!==p.current)return;if(J)V2(J);o({running:!0,clearSilence:!0}),j_(Y),b_.current=Y;return}if(L==="agent_request_timeout"){if(!h)return;if(console.log("Agent request timeout:",Y),J&&p.current&&J!==p.current)return;j_(null),b_.current=null,V0(),l({type:"error",title:"Permission request timed out"});return}if(L==="model_changed"){if(!h)return;if(Y?.model!==void 0)F_(Y.model);if(Y?.thinking_level!==void 0)S_(Y.thinking_level??null);if(Y?.supports_thinking!==void 0)n_(Boolean(Y.supports_thinking));let R=$;F1(R).then((O_)=>{if(f0.current!==R)return;if(O_)G0(O_)}).catch(()=>{});return}if(L==="workspace_update"){if(typeof window<"u")window.dispatchEvent(new CustomEvent("workspace-update",{detail:Y}));return}let{currentHashtag:$_,searchQuery:H_}=w0.current;if(L==="agent_response"){if(!h)return;q$(),M_.current={post:Y,turnId:p.current}}if(!$_&&!H_&&h&&(L==="new_post"||L==="new_reply"||L==="agent_response"))q0((R)=>{if(!R)return[Y];if(R.some((O_)=>O_.id===Y.id))return R;return[...R,Y]}),o$.current?.();if(L==="interaction_updated"){if(!h)return;q0((R)=>{if(!R)return R;if(!R.some((O_)=>O_.id===Y.id))return R;return R.map((O_)=>O_.id===Y.id?Y:O_)})}if(L==="interaction_deleted"){if(!h)return;let R=Y?.ids||[];if(R.length){s2(()=>{q0((q_)=>q_?q_.filter((v2)=>!R.includes(v2.id)):q_)});let{currentHashtag:O_,searchQuery:p_}=w0.current;if(b1.current&&!O_&&!p_)s$.current?.({preserveScroll:!0,preserveMode:"top"})}}},[V0,z_,$,s$,o,D1,s2,u0,T0,P0,q$,V2,y0,P1,M1,r2,g_,o_]);u(()=>{if(typeof window>"u")return;let L=window.__PICLAW_TEST_API||{};return L.emit=$3,L.reset=()=>{q$(),V0(),l(null),i({text:"",totalLines:0}),U_(""),__({text:"",totalLines:0}),j_(null)},L.finalize=()=>w1(),window.__PICLAW_TEST_API=L,()=>{if(window.__PICLAW_TEST_API===L)window.__PICLAW_TEST_API=void 0}},[V0,w1,$3,q$]),A8({handleSseEvent:$3,handleConnectionStatusChange:q6,loadPosts:j2,onWake:w6,chatJid:$}),u(()=>{if(!u2||u2.length===0)return;let L=location.hash;if(!L||!L.startsWith("#msg-"))return;let Y=L.slice(5);m(Y),history.replaceState(null,"",location.pathname+location.search)},[u2,m]);let j3=t!==null;u(()=>{if(K!=="connected")return;let Y=setInterval(()=>{let{currentHashtag:J,searchQuery:w}=w0.current||{},k=!J&&!w;if(j3){if(k)P0();g_(),o0(),n0()}else{if(k)P0();o0(),n0()}},j3?15000:60000);return()=>clearInterval(Y)},[K,j3,o0,n0,g_,P0]),u(()=>{return s8(()=>{o0(),n0(),g_()})},[o0,n0,g_]);let A6=P(()=>{j0((L)=>!L)},[]),P6=P((L)=>{if(typeof window>"u")return;let Y=String(L||"").trim();if(!Y||Y===$)return;let J=L$(window.location.href,Y,{chatOnly:j});window.location.assign(J)},[j,$]),M6=P(async()=>{if(typeof window>"u"||!B_?.chat_jid)return;let L=B_.agent_name||"",Y=B_.display_name||"",J=window.prompt("Branch display name",Y);if(J===null)return;let w=window.prompt("Agent handle (without @)",L);if(w===null)return;try{let k=await Y7(B_.chat_jid,{displayName:J,agentName:w});await Promise.allSettled([u0(),T0()]);let h=k?.branch?.agent_name||String(w||"").trim()||L;W("Branch renamed",`This chat is now @${h}.`,"info",3500)}catch(k){let h=k instanceof Error?k.message:String(k||"Could not rename branch.");W("Could not rename branch",h||"Could not rename branch.","warning",5000)}},[B_,u0,T0,W]),C6=P(async()=>{if(typeof window>"u"||!B_?.chat_jid)return;if(B_.chat_jid===(B_.root_chat_jid||B_.chat_jid)){W("Cannot prune branch","The root chat branch cannot be pruned.","warning",4000);return}let Y=B_.display_name||`@${B_.agent_name||B_.chat_jid}`;if(!window.confirm(`Prune ${Y}?

This archives the branch agent and removes it from the branch picker. Chat history is preserved.`))return;try{await G7(B_.chat_jid),await Promise.allSettled([u0(),T0()]);let w=B_.root_chat_jid||"web:default";W("Branch pruned",`${Y} has been archived.`,"info",3000);let k=L$(window.location.href,w,{chatOnly:j});window.location.assign(k)}catch(w){let k=w instanceof Error?w.message:String(w||"Could not prune branch.");W("Could not prune branch",k||"Could not prune branch.","warning",5000)}},[j,B_,u0,T0,W]);u(()=>{if(!Z||typeof window>"u")return;let L=!1;return(async()=>{try{R_({status:"running",message:"Preparing a new chat branch…"});let Y=await w$(N);if(L)return;let J=Y?.branch,w=typeof J?.chat_jid==="string"&&J.chat_jid.trim()?J.chat_jid.trim():null;if(!w)throw Error("Branch fork did not return a chat id.");let k=L$(window.location.href,w,{chatOnly:!0});window.location.replace(k)}catch(Y){if(L)return;R_({status:"error",message:B1(Y)})}})(),()=>{L=!0}},[Z,N]);let x6=P(async()=>{if(typeof window>"u"||G)return;let L=R8($);if(!L){W("Could not open branch window","Opening branch windows is unavailable in standalone webapp mode.","warning",5000);return}if(L.mode==="tab"){let J=i8(window.location.href,$,{chatOnly:!0});if(!window.open(J,L.target))W("Could not open branch window","The browser blocked opening a new tab or window.","warning",5000);return}let Y=g8(L);if(!Y){W("Could not open branch window","The browser blocked opening a new tab or window.","warning",5000);return}c8(Y,{title:"Opening branch…",message:"Preparing a new chat branch. This should only take a moment."});try{let w=(await w$($))?.branch,k=typeof w?.chat_jid==="string"&&w.chat_jid.trim()?w.chat_jid.trim():null;if(!k)throw Error("Branch fork did not return a chat id.");try{let $_=await Q3();_0(Array.isArray($_?.chats)?$_.chats:[])}catch{}try{let $_=await $6(M);D0(Array.isArray($_?.chats)?$_.chats:[])}catch{}let h=L$(window.location.href,k,{chatOnly:!0});p8(Y,h)}catch(J){h8(Y),W("Could not open branch window",B1(J),"error",5000)}},[$,M,G,W]);u(()=>{if(!L0)return;if(typeof window>"u")return;let L=S.current;if(!L)return;if(!Y_.current){let Y=h2("editorWidth",null),J=n.current||280;Y_.current=Number.isFinite(Y)?Y:J}if(L.style.setProperty("--editor-width",`${Y_.current}px`),!P_.current){let Y=h2("dockHeight",null);P_.current=Number.isFinite(Y)?Y:200}L.style.setProperty("--dock-height",`${P_.current}px`)},[L0]),u(()=>{if(!S0||j)return;let L=(Y)=>{if(Y.ctrlKey&&Y.key==="`")Y.preventDefault(),U0()};return document.addEventListener("keydown",L),()=>document.removeEventListener("keydown",L)},[U0,S0,j]);let S6=Boolean(J_&&J_===(t?.turn_id||G_));if(Z)return U`
        ...
        <div class=${`app-shell${c0?"":" workspace-collapsed"}${L0?" editor-open":""}${j?" chat-only":""}`} ref=${S}>
            ...
            `}
            <div class="container">
                ${V&&m8()&&U`<div class="search-results-spacer"></div>`}
                ${j&&U`
                            ...
                            ${Q0.length>1&&U`
                                <label class="chat-window-branch-picker-wrap">
                                    ...
                                    <select
                                        ...
                                        value=${$}
                                        onChange=${(L)=>P6(L.currentTarget.value)}
                                    >
                                        ${Q0.map((L)=>U`
                                            <option key=${L.chat_jid} value=${L.chat_jid}>
                                                ${`@${L.agent_name}${L.display_name?` — ${L.display_name}`:""}${L.is_active?" • active":""}`}
                                            </option>
                                        `)}
                                    </select>
                                </label>
...
//# debugId=72732C45542A4CDF64756E2164756E21
