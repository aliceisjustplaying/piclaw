...
function fetchByRowId(db, rowId, chatJid) {
    if (chatJid) {
        const row = db
            .prepare("SELECT rowid, chat_jid, sender, sender_name, content, timestamp, is_bot_message FROM messages WHERE rowid = ? AND chat_jid = ?")
            .get(rowId, chatJid);
        return row ?? null;
    }
    const row = db
        .prepare("SELECT rowid, chat_jid, sender, sender_name, content, timestamp, is_bot_message FROM messages WHERE rowid = ?")
        .get(rowId);
    return row ?? null;
}
function searchMessages(db, query, chatJid, limit, offset) {
    ...
    if (trimmed.startsWith("#")) {
        ...
        return db
            .prepare("SELECT rowid, chat_jid, sender, sender_name, content, timestamp, is_bot_message FROM messages WHERE content LIKE ? COLLATE NOCASE ORDER BY rowid DESC LIMIT ? OFFSET ?")
...
export function createMessageSearchTool(defaultChatJid) {
    return {
