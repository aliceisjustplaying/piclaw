...
export function getDataRateLimitRule(method: string, pathname: string): DataRateLimitRule | null {
  ...
  if (method === "POST" && (pathname === "/agent/branch-fork" || pathname === "/agent/branch-rename" || pathname === "/agent/branch-merge-parent" || pathname === "/agent/branch-prune" || pathname === "/agent/branch-purge" || pathname === "/agent/branch-restore")) {
    return {
