...
${G.slice(W).trimStart()}`.trim();return{settings:$,content:O}}function p(G){if(G.startsWith("\\#")||G.startsWith("\\---"))return G;if(G.startsWith("#")||G.startsWith("---"))return`\\${G}`;return G}function i(G){if(G.startsWith("\\#")||G.startsWith("\\---"))return G.slice(1);return G}function Nj(G){return String(G||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Pj(G){let j=String(G||"").trim();if(!j)return"#";if(j.startsWith("#")||j.startsWith("/"))return j;try{let J=new URL(j,window.location.origin);if(["http:","https:","mailto:","tel:"].includes(J.protocol))return J.toString()}catch{}return"#"}function n(G){return G.replace(/\n/g,"<br>")}function Qj(G){let j=String(G||""),J=t.get(j);if(J)return J;let P=Nj(j),Q=n(P),$=globalThis?.marked;try{if($?.parse)Q=String($.parse(P,{gfm:!0,breaks:!0,headerIds:!1,mangle:!1})||"")}catch{Q=n(P)}return Q=Q.replace(/<a\s+([^>]*?)href=(['"])(.*?)\2([^>]*)>/gi,(Z,W,O,U,z)=>{let q=Pj(U);return`<a ${W}href=${O}${q}${O}${z} target="_blank" rel="noopener noreferrer">`}),t.set(j,Q),Q}function a(G){let j={lanes:[],archive:[],settings:{}},J=Gj(G),P=Jj(J);j.settings=P.settings,J=P.content;let Q=J.split(/\n---\n/);if(j.lanes=e(Q[0]),Q.length>1){let $=Q.slice(1).join(`
---
...
`)}function Wj({checked:G,onChange:j}){return y`
    ...
    </div>`}function Xj({onArchive:G,isEditing:j,onCancelEdit:J}){return y`
      ...
      <div class="kanban-plugin__item ${G.checked?"is-complete":""} ${Z?"is-editing":""}"
        ...
        <div class="kanban-plugin__item-content-wrapper">
          <div class="kanban-plugin__item-title-wrapper">
            <${Wj} checked=${G.checked} onChange=${B} />
            ${Z?y`
              <textarea ref=${z} class="kanban-plugin__item-edit-textarea"
                value=${O} onInput=${I}
                onBlur=${()=>{if(Z)D()}}
                onKeyDown=${R} />
            `:y`<div class="kanban-plugin__item-title kanban-plugin__item-markdown" dangerouslySetInnerHTML=${{__html:Qj(G.title)}}></div>`}
            <${Xj} isEditing=${Z}
              onArchive=${()=>$(G)} onCancelEdit=${M} />
          </div>
        </div>
      </div>
    </div>`}function Zj({onAdd:G,onCancel:j}){let[J,P]=V(""),Q=u(null);x(()=>{Q.current?.focus()},[]);let $=()=>{if(J.trim())G(J.trim()),P("")};return y`
    ...
    <div class="kanban-plugin__lane-wrapper ${H?"is-lane-drop-target":""} ${g?"is-lane-dragging":""}"
      ...
      <div class="kanban-plugin__lane ${I?"is-dropping":""}">
        ...
        <div class="kanban-plugin__lane-items">
          ${G.cards.map((K,E)=>y`
            <${Yj} key=${K.id} card=${K} laneId=${G.id} cardIndex=${E}
              onUpdate=${(l)=>$(G.id,l)}
              onDelete=${(l)=>Z(G.id,l)}
              onArchive=${W} />`)}
        </div>
        ${D?y`<${Zj} onAdd=${c} onCancel=${()=>R(!1)} />`:null}
      </div>
    </div>`}function Hj({onAdd:G,onCancel:j}){let[J,P]=V(""),Q=u(null);x(()=>{Q.current?.focus()},[]);let $=()=>{if(J.trim())G(J.trim())};return y`
    ...
    </div>`}function yj({cards:G,onRestore:j}){let[J,P]=V(!0);if(G.length===0)return null;return y`
...
`)[0]}</span>
    ...
    <div class="kanban-plugin" tabindex="-1">
      ...
      <div class="kanban-plugin__board"><div>
        ${j.lanes.map((N,Y)=>y`
          <${$j} key=${N.id} lane=${N} laneIndex=${Y} onUpdate=${R} onDelete=${I}
            onAddCard=${H} onUpdateCard=${k} onDeleteCard=${g}
            onArchiveCard=${m} onMoveCard=${f} onMoveLane=${M} />`)}
        ${P&&y`<${Hj} onAdd=${D} onCancel=${()=>Q(!1)} />`}
      </div></div>
      <${yj} cards=${j.archive} onRestore=${s} />
    </div>`}window.__kanbanEditor={mount(G,j){if(L=G,C=j.onEdit,T=null,v=null,!j.isDark)G.classList.add("light");d(y`<${Kj} initialContent=${j.content} />`,G)},update(G){v=G,h++},setTheme(G){L?.classList.toggle("light",!G)},destroy(){if(L)d(null,L);L=null,C=null,v=null,T=null}};})();
