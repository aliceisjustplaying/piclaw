...
import { archiveChatBranch, ensureChatBranch, getChatBranchByAgentName, getChatBranchByChatJid, listChatBranches, renameChatBranchIdentity, renameChatJid as renameChatJidDb, restoreChatBranchIdentity, storeChatMetadata, } from "../db.js";
...
export class AgentBranchManager {
    ...
    async renameChatJid(oldJid, newJid) {
        ...
        if (!old)
            throw new Error("Old JID is required.");
        if (!next)
            throw new Error("New JID is required.");
        if (old === next)
        ...
        if (mainEntry) {
            try {
                ...
                log.debug("best-effort dispose failed for main session", { jid: old, err: e });
            }
        ...
        if (sideEntry) {
            try {
                ...
                debugSuppressedError(log, "best-effort dispose failed for side session", e, { jid: old });
            }
        ...
        for (const [jid, entry] of this.options.pool) {
            if (jid.startsWith(childPrefix)) {
                try {
                    ...
                    debugSuppressedError(log, "best-effort dispose failed for child pool session", e, { jid });
                }
        ...
        for (const [jid, entry] of this.options.sidePool) {
            if (jid.startsWith(childPrefix)) {
                try {
                    ...
                    debugSuppressedError(log, "best-effort dispose failed for child side-pool session", e, { jid });
                }
        ...
        try {
            ...
            debugSuppressedError(log, "best-effort child branch directory rename failed", e, { old, next });
        }
        return result;
    }
