...
function deriveBaseAgentName(chatJid) {
...
export function getChatBranchByChatJid(chatJid) {
    const db = getDb();
    const row = db.prepare(`SELECT branch_id, chat_jid, root_chat_jid, parent_branch_id, agent_name, created_at, updated_at, archived_at
       FROM chat_branches
...
export function listChatBranches(rootChatJid, options) {
    ...
    const rows = rootChatJid
        ? db.prepare(`SELECT branch_id, chat_jid, root_chat_jid, parent_branch_id, agent_name, created_at, updated_at, archived_at
         FROM chat_branches
        WHERE root_chat_jid = ?
...
export function ensureChatBranch(input) {
    ...
    if (existing) {
        ...
        if (rootChatJid !== existing.root_chat_jid ||
            ...
            SET root_chat_jid = ?,
                ...
                updated_at = ?,
                archived_at = NULL
          WHERE branch_id = ?`).run(rootChatJid, parentBranchId, nextAgentName, now, existing.branch_id);
        }
    ...
    ) VALUES (?, ?, ?, ?, ?, ?, ?, NULL)`).run(branchId, chatJid, rootChatJid, parentBranchId, agentName, now, now);
    return getChatBranchByChatJid(chatJid);
}
export function renameChatBranchIdentity(input) {
    ...
    db.prepare(`UPDATE chat_branches
        SET agent_name = ?,
            updated_at = ?
      WHERE branch_id = ?`).run(nextAgentName, now, existing.branch_id);
    return getChatBranchByChatJid(chatJid);
}
 ...
 */
export function renameChatJid(oldJid, newJid) {
    const old = String(oldJid || "").trim();
    const next = String(newJid || "").trim();
    if (!old)
        throw new Error("oldJid is required");
    if (!next)
        throw new Error("newJid is required");
    if (old === next)
        throw new Error("Old and new JID are identical.");
    // Disallow reserved characters that would break the branch-tree model.
    if (/[\s]/.test(next))
        throw new Error("JID must not contain whitespace.");
    const existing = getChatBranchByChatJid(old);
    if (!existing)
        throw new Error(`Unknown chat: ${old}`);
    // Make sure the target JID is not already in use.
    const collision = getChatBranchByChatJid(next);
    if (collision)
        throw new Error(`Target JID already exists: ${next}`);
    const db = getDb();
    const now = new Date().toISOString();
    const updated = [];
    ...
    try {
        // --- chats (PK = jid) -----------------------------------------------
        const chatsRows = db.prepare(`UPDATE chats SET jid = ? WHERE jid = ?`).run(next, old);
        // Also update any children whose JID starts with the old prefix.
        db.prepare(`UPDATE chats SET jid = ? || substr(jid, ?) WHERE jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, childPrefix + "*");
        if (chatsRows?.changes > 0)
            updated.push("chats");
        // --- messages -------------------------------------------------------
        const msgRows = db.prepare(`UPDATE messages SET chat_jid = ? WHERE chat_jid = ?`).run(next, old);
        db.prepare(`UPDATE messages SET chat_jid = ? || substr(chat_jid, ?) WHERE chat_jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, childPrefix + "*");
        if (msgRows?.changes > 0)
            updated.push("messages");
        // --- chat_cursors ---------------------------------------------------
        const cursorRows = db.prepare(`UPDATE chat_cursors SET chat_jid = ? WHERE chat_jid = ?`).run(next, old);
        db.prepare(`UPDATE chat_cursors SET chat_jid = ? || substr(chat_jid, ?) WHERE chat_jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, childPrefix + "*");
        if (cursorRows?.changes > 0)
            updated.push("chat_cursors");
        // --- chat_branches (chat_jid + root_chat_jid) ----------------------
        db.prepare(`UPDATE chat_branches SET chat_jid = ?, root_chat_jid = CASE WHEN root_chat_jid = ? THEN ? ELSE root_chat_jid END, updated_at = ? WHERE chat_jid = ?`).run(next, old, next, now, old);
        ...
        db.prepare(`UPDATE chat_branches
         SET chat_jid = ? || substr(chat_jid, ?),
             ...
             updated_at = ?
       WHERE chat_jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, old, next, now, childPrefix + "*");
        updated.push("chat_branches");
        // --- token_usage ----------------------------------------------------
        db.prepare(`UPDATE token_usage SET chat_jid = ? WHERE chat_jid = ?`).run(next, old);
        db.prepare(`UPDATE token_usage SET chat_jid = ? || substr(chat_jid, ?) WHERE chat_jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, childPrefix + "*");
        updated.push("token_usage");
        // --- scheduled_tasks ------------------------------------------------
        db.prepare(`UPDATE scheduled_tasks SET chat_jid = ? WHERE chat_jid = ?`).run(next, old);
        db.prepare(`UPDATE scheduled_tasks SET chat_jid = ? || substr(chat_jid, ?) WHERE chat_jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, childPrefix + "*");
        ...
        for (const tbl of ["ssh_configs", "proxmox_configs", "portainer_configs"]) {
            ...
            db.prepare(`UPDATE ${tbl} SET chat_jid = ? || substr(chat_jid, ?) WHERE chat_jid GLOB ?`).run(childNextPrefix, childPrefix.length + 1, childPrefix + "*");
        }
        db.exec("COMMIT");
    }
    catch (err) {
