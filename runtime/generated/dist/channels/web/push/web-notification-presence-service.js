...
function isLikelyIosWebKitClient(userAgent) {
...
export function normalizeWebNotificationPresence(value, options = {}) {
...
export class WebNotificationPresenceService {
    ...
    getDeviceChatState(deviceId, chatJid, nowMs = this.now()) {
    ...
    shouldSendWebPush(deviceId, chatJid, nowMs = this.now()) {
        const normalizedDeviceId = normalizeTrimmedString(deviceId);
        const normalizedChatJid = normalizeTrimmedString(chatJid);
        if (!normalizedDeviceId || !normalizedChatJid)
            return true;
        const state = this.getDeviceChatState(normalizedDeviceId, normalizedChatJid, nowMs);
        if (!state.hasLiveClient)
            return true;
        if (state.hasVisibleClient)
            return false;
