...
function resolvePushDir(baseDir = DEFAULT_PUSH_DIR) {
    return resolve(baseDir);
}
function resolveVapidKeysPath(baseDir = DEFAULT_PUSH_DIR) {
    return resolve(resolvePushDir(baseDir), VAPID_FILE_NAME);
}
function resolveSubscriptionsPath(baseDir = DEFAULT_PUSH_DIR) {
...
function readStoredVapidKeys(baseDir = DEFAULT_PUSH_DIR) {
    const path = resolveVapidKeysPath(baseDir);
    const parsed = readJsonFile(path);
    if (!parsed)
        return null;
    if (!parsed.publicKey || !parsed.publicKeyPem || !parsed.privateKeyPem)
        return null;
    return parsed;
}
export function ensureStoredVapidKeys(baseDir = DEFAULT_PUSH_DIR) {
...
export function getStoredVapidPublicKey(baseDir = DEFAULT_PUSH_DIR) {
    return ensureStoredVapidKeys(baseDir).publicKey;
}
export function normalizeStoredWebPushSubscription(value, options = {}) {
...
export function listStoredWebPushSubscriptions(baseDir = DEFAULT_PUSH_DIR) {
...
function writeStoredWebPushSubscriptions(entries, baseDir = DEFAULT_PUSH_DIR) {
    writeJsonFile(resolveSubscriptionsPath(baseDir), entries);
}
function getMaxStoredWebPushSubscriptions() {
    return Math.max(1, Number.parseInt(String(process.env.PICLAW_WEB_PUSH_SUBSCRIPTION_CAP || "32"), 10) || 32);
}
function capStoredWebPushSubscriptions(entries) {
...
export function upsertStoredWebPushSubscription(value, options = {}) {
...
export function removeStoredWebPushSubscription(endpoint, baseDir = DEFAULT_PUSH_DIR) {
